...
搜索功能包含筛选器，可根据您的需求（如产品、类别、可信内容以及发布者）来缩小结果范围。这确保您能快速找到并访问最适合您项目的资源。

### 产品

Docker Hub 的内容库包含多种产品，每种都旨在满足开发者和组织的特定需求。这些产品包括：
...
Docker Hub 中的 Helm 图表提供了一种简化的方式来打包、配置和部署 Kubernetes 应用。Helm 是 Kubernetes 的包管理器，而图表是预配置的模板，定义了在 Kubernetes 集群中运行应用所需的资源。Docker Hub 托管各种 Helm 图表，它们提供：

- 应用打包：将 Kubernetes 清单、配置和依赖项打包到单个可复用的图表中。
- 版本管理：跟踪和管理应用部署的不同版本。
- 配置模板化：使用 values 文件自定义部署，使相同应用能够轻松跨不同环境部署。
- 依赖管理：自动处理图表依赖项，确保所有必需组件一起部署。

Helm 图表降低了 Kubernetes 部署的复杂性，使团队能够更轻松地在生产环境中部署、升级和管理应用。

#### Compose

Docker Hub 中的 Docker Compose 文件通过简单的 YAML 配置文件实现多容器应用编排。Compose 是用于定义和运行多容器 Docker 应用的工具，Docker Hub 托管的 Compose 文件可帮助您：

- 多容器编排：使用单个命令定义并运行由多个互联容器组成的应用。
- 服务配置：以声明式格式指定容器镜像、环境变量、网络、卷和依赖项。
- 开发和测试：快速启动完整的应用栈，用于本地开发、测试或演示。
- 环境一致性：确保应用在开发、预发和生产环境中行为一致。

Compose 文件通过提供清晰、受版本控制的整个应用栈定义，简化了管理复杂应用的过程。

#### AI 模型

Docker Hub 以容器化格式托管 AI 和机器学习模型，使其更易于跨不同环境部署、共享和运行 AI 应用。这些容器化的 AI 模型提供：

- 预训练模型：访问用于常见任务（如图像识别、自然语言处理和预测分析）的即用型机器学习模型。
- 模型服务：将模型部署为可轻松集成到应用中并根据需要扩展的容器化服务。
- 可复现环境：将模型与其依赖项一起打包，确保开发和生产环境中行为一致。
- 框架支持：查找使用 TensorFlow、PyTorch、scikit-learn 等流行框架构建的模型。

容器化的 AI 模型消除了管理基础架构依赖的需要，让您能够跨不同环境部署模型，并随着应用需求变化进行扩展。
...
Docker Hub 的可信内容提供了一系列精选的高质量、安全镜像，旨在让开发者对其所用资源的可靠性和安全性充满信心。这些镜像稳定、定期更新，并遵循行业最佳实践，是构建和部署应用程序的坚实基础。Docker Hub 的可信内容包括 Docker 强化镜像、Docker 官方镜像、经过验证的发布者镜像以及 Docker 赞助的开源软件镜像。

更多详情，请参阅 [可信内容](./trusted-content.md)。

### 发布者

**发布者** 筛选器可让您按发布该镜像的组织来缩小镜像结果范围。

可信内容的发布者会排在前面。仅显示精选数量的发布者。
