...
> [!IMPORTANT]
> containerd 使用与 Docker 数据目录不同的存储路径。如果您之前为 Docker 配置了自定义数据目录（例如为了使用不同的分区），containerd 的存储不会自动移动到该位置。您需要单独配置 containerd 的数据目录，以避免占满您的根分区。
>
> 要配置 containerd 的数据目录，请参阅[配置数据目录位置](../daemon/_index.md#configure-the-data-directory-location)。
