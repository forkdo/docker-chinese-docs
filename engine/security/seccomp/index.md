...
只有在 Docker 使用 `seccomp` 构建且内核配置启用了 `CONFIG_SECCOMP` 时，此功能才可用。要检查内核是否支持 `seccomp`：
...
为使用 seccomp 运行容器提供了一个合理的默认值，禁用了大约 44 个系统调用（总共 300 多个）。它在提供广泛应用程序兼容性的同时，具有适度的保护作用。

实际上，该配置文件是一个白名单，默认情况下拒绝访问系统调用，然后允许特定的系统调用。该配置文件通过定义 `defaultAction` 为 `SCMP_ACT_ERRNO` 来工作，并仅为特定系统调用覆盖该操作。`SCMP_ACT_ERRNO` 的效果是导致“权限被拒绝”错误。接下来，配置文件定义了一个特定的系统调用列表，这些调用被完全允许，因为它们的 `action` 被覆盖为 `SCMP_ACT_ALLOW`。最后，一些特定规则针对个别系统调用（如 `personality` 等），允许具有特定参数的这些系统调用的变体。

`seccomp` 对于以最小权限运行 Docker 容器至关重要。不建议更改默认的 `seccomp` 配置文件。

运行容器时，除非使用 `--security-opt` 选项覆盖，否则它使用默认配置文件。例如，以下命令显式指定了一个策略：
...
| 系统调用             | 描述                                                                                                                                                                                                                                    |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `acct`              | 会计系统调用，可能让容器禁用自身的资源限制或进程会计。也受 `CAP_SYS_PACCT` 限制。                                                                                                                                                                          |
| `add_key`           | 防止容器使用内核密钥环，因为密钥环未被命名空间化。                                                                                                                                                                     |
| `bpf`               | 拒绝将潜在的持久性 BPF 程序加载到内核，已通过 `CAP_SYS_ADMIN` 限制。                                                                                                                                                |
...
| `create_module`     | 拒绝对内核模块的操作和函数。已过时。也受 `CAP_SYS_MODULE` 限制。                                                                                                                                                                           |
...
| `keyctl`            | 防止容器使用内核密钥环，因为密钥环未被命名空间化。                                                                                                                                                                     |
...
| `personality`       | 防止容器启用 BSD 仿真。本身并不危险，但测试不足，可能存在大量内核漏洞。                                                                                                     |
...
| `query_module`      | 拒绝对内核模块的操作和函数。已过时。                                                                                                                                                                                   |
| `quotactl`          | 配额系统调用，可能让容器禁用自身的资源限制或进程会计。也受 `CAP_SYS_ADMIN` 限制。                                                                                                               |
...
你可以传递 `unconfined` 来在不使用默认 seccomp 配置文件的情况下运行容器。
