...
默认情况下，Compose 会根据包含 Compose 文件的目录名称来分配项目名称。您可以通过多种方法覆盖此设置。

本页提供了自定义项目名称可能有帮助的场景示例，概述了设置项目名称的各种方法，并提供了每种方法的优先级顺序。
...
Compose 使用项目名称来隔离不同的环境。在多种情况下，项目名称都很有用：
...
项目名称必须仅包含小写字母、十进制数字、短横线和下划线，并且必须以小写字母或十进制数字开头。如果项目目录或当前目录的基本名称违反此约束，则可以使用替代机制。
...
- 了解[使用多个 Compose 文件](multiple-compose-files/_index.md)。
