...
启用后，Compose 会在更新服务或堆栈时自动删除孤立容器。孤立容器是由先前配置创建但当前 `compose.yaml` 文件中不再定义的容器。
...
- `COMPOSE_API_VERSION`
   默认情况下，API 版本会与服务器协商。使用 `DOCKER_API_VERSION`。
