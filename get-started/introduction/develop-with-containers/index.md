...
1. 在文本编辑器中打开 `backend/src/routes/getGreeting.js` 文件（在克隆的项目目录中，位于您的本地机器上）。此文件提供 API 端点的处理程序。您的更改将自动同步到正在运行的容器中。
...
1. 在您的本地项目目录中打开 `client/src/components/AddNewItemForm.jsx` 文件。此文件提供向待办事项列表添加新项的组件。
...
1. 在您的本地项目目录中打开 `client/src/index.scss` 文件。
...
- 进行更改并立即看到它们。这是通过以下方式实现的：1) 在每个容器中运行的进程正在监视并响应文件更改，2) 您本地项目目录中的文件与容器化环境共享，因此您在本地所做的编辑会自动同步到容器中。
