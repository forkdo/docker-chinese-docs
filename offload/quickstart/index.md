...
![Offload 模式图标](./images/cloud-mode.png))，Docker Desktop 仪表板将显示为紫色。您可以在终端中运行 `docker offload status` 命令来检查 Docker Offload 的状态。
...
![Offload 会话时长](./images/hourglass-icon.png)) 旁边看到当前会话时长估算。
