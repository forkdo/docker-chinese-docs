...
|--------|---------|-------------|
| `--ca-cert` |  |  Path to the PEM-formatted root CA certificate to use for the new cluster<br> |
...
| `-d`, `--detach` |  |  Exit immediately instead of waiting for the root rotation to converge<br> |
| `--external-ca` |  |  Specifications of one or more certificate signing endpoints |
| `-q`, `--quiet` |  |  Suppress progress output |
| `--rotate` |  |  Rotate the swarm CA - if no certificate or key are provided, new ones will be generated<br> |
