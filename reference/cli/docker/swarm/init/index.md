...
| `--autolock` |  |  Enable manager autolocking (requiring an unlock key to start a stopped manager)<br> |
...
| `--data-path-addr` |  | API 1.31+ Address or interface to use for data path traffic (format: `<ip|interface>`)<br> |
| `--data-path-port` |  | API 1.40+ Port number to use for data path traffic (1024 - 49151). If no value is set or is set to 0, the default port (4789) is used.<br> |
