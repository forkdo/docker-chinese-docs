# 扩展 API 参考

## 仪表板接口

- [Host](Host.md)
- [NavigationIntents](NavigationIntents.md)
- [Toast](Toast.md)

## 其他接口

- [ExecResultV0](ExecResultV0.md)
- [RequestConfigV0](RequestConfigV0.md)
- [BackendV0](BackendV0.md)
- [OpenDialogResult](OpenDialogResult.md)
- [Dialog](Dialog.md)
- [Docker](Docker.md)
- [DockerCommand](DockerCommand.md)
- [ExecOptions](ExecOptions.md)
- [SpawnOptions](SpawnOptions.md)
- [Exec](Exec.md)
- [ExecProcess](ExecProcess.md)
- [ExecStreamOptions](ExecStreamOptions.md)
- [ExecResult](ExecResult.md)
- [RawExecResult](RawExecResult.md)
- [Extension](Extension.md)
- [DesktopUI](DesktopUI.md)
- [ExtensionVM](ExtensionVM.md)
- [ExtensionHost](ExtensionHost.md)
- [ExtensionCli](ExtensionCli.md)
- [HttpService](HttpService.md)
- [RequestConfig](RequestConfig.md)
- [ServiceError](ServiceError.md)
- [DockerDesktopClient](DockerDesktopClient.md)

- [](/reference/api/extensions-sdk/ExecResult/)

- [Interface: ExecOptions](/reference/api/extensions-sdk/ExecOptions/)

- [Interface: ExecResultV0](/reference/api/extensions-sdk/ExecResultV0/)

- [Interface: ExtensionHost](/reference/api/extensions-sdk/ExtensionHost/)

- [Interface: OpenDialogResult](/reference/api/extensions-sdk/OpenDialogResult/)

- [Interface: Toast](/reference/api/extensions-sdk/Toast/)

- [接口: BackendV0](/reference/api/extensions-sdk/BackendV0/)

- [接口: Docker](/reference/api/extensions-sdk/Docker/)

- [接口: Exec](/reference/api/extensions-sdk/Exec/)

- [接口: Extension](/reference/api/extensions-sdk/Extension/)

- [接口: ExtensionVM](/reference/api/extensions-sdk/ExtensionVM/)

- [接口: NavigationIntents](/reference/api/extensions-sdk/NavigationIntents/)

- [接口: RawExecResult](/reference/api/extensions-sdk/RawExecResult/)

- [接口: RequestConfigV0](/reference/api/extensions-sdk/RequestConfigV0/)

- [接口: SpawnOptions](/reference/api/extensions-sdk/SpawnOptions/)

- [接口：DesktopUI](/reference/api/extensions-sdk/DesktopUI/)

- [接口：Dialog](/reference/api/extensions-sdk/Dialog/)

- [接口：DockerCommand](/reference/api/extensions-sdk/DockerCommand/)

- [接口：DockerDesktopClient](/reference/api/extensions-sdk/DockerDesktopClient/)

- [接口：ExecProcess](/reference/api/extensions-sdk/ExecProcess/)

- [接口：ExecStreamOptions](/reference/api/extensions-sdk/ExecStreamOptions/)

- [接口：ExtensionCli](/reference/api/extensions-sdk/ExtensionCli/)

- [接口：Host](/reference/api/extensions-sdk/Host/)

- [接口：HttpService](/reference/api/extensions-sdk/HttpService/)

- [接口：RequestConfig](/reference/api/extensions-sdk/RequestConfig/)

- [接口：ServiceError](/reference/api/extensions-sdk/ServiceError/)

