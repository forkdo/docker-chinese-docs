# 气隙容器
  ...
  <a class="link" href="https://docs.docker.com/desktop/release-notes/#4290">4.29.0</a> and later</span>
        <span class="icon-svg">
...
气隙容器允许您通过控制容器可以发送和接收数据的位置来限制容器网络访问。此功能将自定义代理规则应用于容器网络流量，有助于保护容器不应具有无限制互联网访问权限的环境。
...
| `exclude` | 对这些地址绕过代理 | 主机名/IP 数组 |
| `pac` | 代理自动配置文件 URL | PAC 文件 URL |
...
PAC 文件通过为不同目标定义规则，提供对容器网络访问的细粒度控制。
...
# 如果 internal.company.com 在您的排除列表或 PAC 规则中，则应成功
```
...
- 网络策略执行：气隙容器在 Docker Desktop 级别工作。高级用户可能通过各种方式绕过限制，因此对于高安全环境，请考虑额外的网络级控制。
