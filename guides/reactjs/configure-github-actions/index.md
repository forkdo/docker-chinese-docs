...
完成本指南中从[容器化 React.js 应用程序](containerize.md)开始的所有先前部分。
...
2. 在 [Docker Hub](https://hub.docker.com/repositories/) 中创建一个仓库
   ...
   2. 对于仓库名称，请使用描述性名称 —— 例如：`reactjs-sample`。
...
5. 将你的本地项目连接到 GitHub
   ...
   通过从项目根目录运行以下命令，将你的本地项目 `docker-reactjs-sample` 链接到你刚创建的 GitHub 仓库：
...
1. 进入你在 GitHub 上的仓库，并选择顶部菜单中的 **Actions** 选项卡。
...
此工作流为您的 React.js 应用程序执行以下任务：
- 在每次针对 `main` 分支的 `push` 或 `pull request` 时触发。
- 使用 `Dockerfile.dev` 构建一个针对测试优化的开发 Docker 镜像。
- 在干净的容器化环境中使用 Vitest 执行单元测试，确保一致性。
...
2. 监控工作流执行
   ...
   2. 点击进入工作流运行，以跟踪每个步骤：**构建**、**测试**，以及（如果成功）**推送**。
...
3. 在 Docker Hub 上验证 Docker 镜像
   ...
   - 您应该会在仓库中看到一个新镜像，包含：
      ...
      - 标签包括：
         ...
         - `<short-sha>` —— 基于提交哈希的唯一标识符，对版本跟踪、回滚和追踪非常有用。
