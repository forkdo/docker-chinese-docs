...
- 为 `docker scout attest get` 添加 `--skip-tlog` 标志，以跳过针对透明度日志的签名验证。
...
<em class="text-gray-400 italic dark:text-gray-500">2025-05-26</em>


### 错误修复

- 修复本地镜像的 `docker scout attest list` 和 `docker scout attest get` 问题。

## 1.18.0

<em class="text-gray-400 italic dark:text-gray-500">2025-05-13</em>
...
- 为 `docker scout sbom` 命令添加密钥扫描功能。
...
- 为 `docker scout sbom` 添加新的 `--format=cyclonedx` 标志，以 CycloneDX 格式输出 SBOM。
...
- 改进分析没有证明的 `oci` 目录时的消息提示。
  仅支持单平台镜像和带有证明的多平台镜像。不支持没有证明的多平台镜像。
- 改进分类器和 SBOM 索引器：
...
<em class="text-gray-400 italic dark:text-gray-500">2024-09-24</em>
...
<em class="text-gray-400 italic dark:text-gray-500">2024-08-05</em>


### 新增功能

- 为 `docker scout quickview`、`docker scout policy` 和 `docker scout compare` 命令添加 `--only-policy` 过滤器选项。
- 为 `docker scout cves` 和 `docker scout quickview` 命令添加 `--ignore-suppressed` 过滤器选项，以过滤掉受[例外](/scout/explore/exceptions/)影响的 CVE。
...
- 将 `github.com/docker/docker` 更新至 `v26.1.5+incompatible` 以修复 CVE-2024-41110。
- 将 Syft 更新至 1.10.0。
...
<em class="text-gray-400 italic dark:text-gray-500">2024-06-26</em>
...
<em class="text-gray-400 italic dark:text-gray-500">2024-05-28</em>
...
<em class="text-gray-400 italic dark:text-gray-500">2024-05-27</em>


### 新增功能

- 通过在 `docker scout cves` 命令上使用 `--format gitlab`，添加对 [GitLab 容器扫描文件格式](https://docs.gitlab.com/ee/development/integrations/secure.html#container-scanning) 的支持。
...
- 为 `docker scout attest add` 命令支持单架构镜像
- 在 `docker scout quickview` 和 `docker scout recommendations` 命令上指示镜像来源是否未使用 `mode=max` 创建。
  如果没有 `mode=max`，基础镜像可能会被错误检测，导致结果不准确。

## 1.9.0

<em class="text-gray-400 italic dark:text-gray-500">2024-05-24</em>
...
<em class="text-gray-400 italic dark:text-gray-500">2024-04-25</em>
...
<em class="text-gray-400 italic dark:text-gray-500">2024-04-15</em>


### 新增功能

- [`docker scout push` 命令](/reference/cli/docker/scout/push/) 现已完全可用：在本地分析镜像并将 SBOM 推送到 Docker Scout。
...
<em class="text-gray-400 italic dark:text-gray-500">2024-03-26</em>
...
<em class="text-gray-400 italic dark:text-gray-500">2024-03-22</em>
...
- 按 EPSS 分数优先考虑 CVE
  ...
  - `--epss` 用于显示和优先考虑 CVE
  - `--epss-score` 和 `--epss-percentile` 用于按分数和百分位数过滤
...
- 如果使用[设置管理](/manuals/enterprise/security/hardened-desktop/settings-management/_index.md)功能禁用了索引，则现在会在 CLI 中阻止索引。
...
Docker Scout CLI 插件早期版本的发布说明可在 [GitHub](https://github.com/docker/scout-cli/releases) 上获取。
