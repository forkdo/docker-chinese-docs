...
- **文件服务器**：处理容器对主机文件系统的文件访问。
  ...
  - 使用 `virtiofs` 或 `krun` 时，文件访问由相应的守护进程处理，而不是后端进程。
...
Docker Desktop 中的每个 Linux 容器都在 Docker 管理的小型虚拟网络中运行，每个容器都连接到 Docker 管理的网络并接收其自己的内部 IP 地址。您可以使用 `docker network ls`、`docker network create` 和 `docker network inspect` 查看和管理这些网络。它们由 [`daemon.json`](/manuals/engine/daemon/_index.md) 管理。
...
Docker Desktop 可以使用系统的默认代理设置，也可以使用您在 [Docker Desktop 的代理设置](/manuals/desktop/settings-and-maintenance/settings.md#proxies) 中配置的自定义设置。

Docker Desktop 通过两个独立的代理路由流量。容器代理（Containers proxy）管理所有 `docker image pull` 操作，以及在启用气隙容器强制（air-gapped container enforcement）时运行的容器流量。Docker Desktop 代理管理主机级流量——即 Desktop 应用程序、Docker CLI 和扩展——并且仅在未显式配置容器代理时作为镜像拉取的回退。
