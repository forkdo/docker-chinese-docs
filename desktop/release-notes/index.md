...
>
  ...
  <div class="leading-tight">
    <div class="text-base">订阅 Docker Desktop RSS 源</div>
  </div>
</button>
...
- 为 Docker Desktop 添加了新的端口绑定设置。管理员也可以通过 `admin-settings.json` 文件使用设置管理来控制此功能。
...
- Docker Desktop 现在会检测并尝试避免“Docker 子网”与使用 RFC1918 地址的物理网络发生冲突。例如，如果主机有一个与 `192.168.65.0/24` 重叠的非默认路由，则将自动选择替代网络。您仍然可以通过 Docker Desktop 设置和管理员设置覆盖此选择。
...
- 修复了 [CVE-2025-9164](https://www.cve.org/cverecord?id=CVE-2025-9164)，该问题导致 Windows 版 Docker Desktop 安装程序因不安全的 DLL 搜索顺序而容易受到 DLL 劫持。安装程序在检查系统目录之前会在用户的下载文件夹中搜索所需的 DLL，从而允许通过恶意 DLL 放置进行本地提权。
...
- 移除了过时的 `mcp` 键，以符合最新的 VS Code MCP 服务器更改。
...
- 修复了 [CVE-2025-10657](https://www.cve.org/CVERecord?id=CVE-2025-10657)，该问题导致增强容器隔离的 [Docker 套接字命令限制](../enterprise/security/hardened-desktop/enhanced-container-isolation/config.md#command-restrictions) 功能仅在 Docker Desktop 4.46.0 中无法正常工作（其配置被忽略）。

### 新增功能

- 为 Docker 的 MCP 目录添加了动态 MCP 服务器发现和支持。
- 使用增强容器隔离，管理员现在可以在挂载了 Docker 套接字的容器中阻止 `docker plugin` 和 `docker login` 命令。
...
- 增强了 Docker Model Runner，CLI 中具有丰富的响应渲染，Docker Desktop 仪表板中具有对话上下文，并且支持可恢复的下载。
...
- 修复了在写入本地内容存储时（不带 `--push` 标志）`docker model package` 命令有时会挂起的错误。
- 修复了使用 `unless-stopped` 重启策略启动的容器永远不会重启的错误。修复 [docker/for-mac#7744](https://github.com/docker/for-mac/issues/7744)。

#### 对于 Windows

- 修复了 Windows 上 Goose MCP 客户端与 Docker MCP Toolkit 的连接问题。
...
- 重新设计了更新体验，使 Docker Desktop 及其组件的更新更易于理解和管理。
...
- 修复了 [CVE-2025-9074](https://www.cve.org/CVERecord?id=CVE-2025-9074)，该问题允许在 Docker Desktop 上运行的恶意容器访问 Docker Engine 并启动其他容器，而无需挂载 Docker 套接字。这可能允许未经授权访问主机系统上的用户文件。增强容器隔离 (ECI) 无法缓解此漏洞。
...
- 修复了 4.44.0 版本中发现的一个问题，该问题导致当 `vpnkit` CIDR 在桌面设置管理中未指定值而被锁定时启动失败。

#### 对于 Windows

- 修复了从使用旧版 `version-pack-data` 目录结构的发行版升级后，卷和容器不可见的问题。
- 解决了 WSL 2 中一个罕见的问题，该问题导致 Docker CLI 因 **需要代理身份验证** 错误而失败。
...
我们注意到 [CVE-2025-23266](https://nvd.nist.gov/vuln/detail/CVE-2025-23266)，这是一个影响 CDI 模式下 NVIDIA Container Toolkit（最高至 1.17.7 版本）的关键漏洞。Docker Desktop 包含 1.17.8 版本，不受影响。但是，如果手动启用了 CDI 模式，则捆绑了早期工具包版本的旧版 Docker Desktop 可能会受到影响。请升级到 Docker Desktop 4.44 或更高版本，以确保您使用的是已修补的版本。
...
- 改进了[ Kubernetes 自定义注册表镜像](/manuals/desktop/use-desktop/kubernetes.md#configuring-a-custom-image-registry-for-kubernetes-control-plane-images) 与增强容器隔离 (ECI) 之间的交互，因此在使用自定义注册表作为 Kubernetes 控制平面镜像时，不再需要手动更新 [ECI Docker 套接字镜像列表](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/config.md)。
...
- 更新了首次启动时的服务条款文本。
- 在解析新发布的 GGUF 格式方面增强了健壮性。

#### 对于 Mac

- 修复了在回收磁盘空间时 DockerVMM 上的磁盘损坏问题。
- 通过重新引入常规用法的性能提升，修复了自 4.42.0 以来的回归问题。
...
- 修复了 [CVE-2025-6587](https://www.cve.org/CVERecord?id=CVE-2025-6587)，该问题导致敏感的系统环境变量包含在 Docker Desktop 诊断日志中，可能导致秘密泄露。
...
- `docker buildx bake` 将不会在具有顶级模型属性的 Compose 文件中构建镜像。请改用 `docker compose build`。
...
- Docker Model Runner 现已可用于运行在 Qualcomm/ARM GPU 上的 Windows 系统。
- 为模型视图添加了 **日志** 选项卡，以便您可以实时查看推理引擎的输出。
...
- 修复了当 Docker Desktop 内存不足时会挂起的错误。
- 在容器中阻止 `io_uring` 系统调用。
- 添加了直接从 Docker Hub 拉取模型的支持，简化了访问和使用模型的过程。
- Docker Desktop 现在将磁盘使用限制设置为全新安装时物理磁盘的大小，并在 Mac 和 Linux 上重置为默认值。
...
- 修复了仪表板容器日志屏幕中的一个错误，该错误导致滚动条在鼠标接近时消失。
...
- 此版本包含 `docker port` 的回归，导致在使用 testcontainers-node 时出现 "No host port found for host IP" 错误。请参阅 [testcontainers/testcontainers-node#818](https://github.com/testcontainers/testcontainers-node/issues/818#issuecomment-2941575369)
...
- 通过避免将 `llama.cpp` DLL 放置在系统 `PATH` 中包含的目录中，修复了与第三方工具（例如 Ollama）的潜在冲突。
...
- 现在可以使用 Docker Model Runner 将[模型推送到 Docker Hub](/manuals/ai/model-runner.md#push-a-model-to-docker-hub)。
- 为 Mac 和 Windows 上的 Docker Desktop 添加了对 Docker Model Runner 的模型管理和聊天界面的支持（在支持 Docker Model Runner 的硬件上）。用户现在可以通过新的专用界面查看、交互和管理本地 AI 模型。
...
- 修复了 [CVE-2025-3224](https://www.cve.org/CVERecord?id=CVE-2025-3224)，该问题允许在 Docker Desktop 更新时，拥有用户机器访问权限的攻击者执行提权。
- 修复了 [CVE-2025-4095](https://www.cve.org/CVERecord?id=CVE-2025-4095)，该问题导致在使用 MacOS 配置描述文件时，注册表访问管理 (RAM) 策略未强制执行，允许用户从未经批准的注册表拉取镜像。
- 修复了 [CVE-2025-3911](https://www.cve.org/CVERecord?id=CVE-2025-3911)，该问题允许拥有用户机器读取权限的攻击者从 Docker Desktop 日志文件中获取敏感信息，包括为正在运行的容器配置的环境变量。
...
- 修复了在 Mac、Windows Hyper-V 或 Linux 上启用[增强容器隔离](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/_index.md) 时，容器访问 `/proc/sys/kernel/shm*` sysctl 的问题。
...
- 修复了当 `/Library/Application Support/com.docker.docker/` 目录不存在时，无法应用 RAM 策略限制的问题。

#### 对于 Windows

- Windows `.exe` 安装程序现在包含对锁定文件的改进处理。修复 [docker/for-win#14299](https://github.com/docker/for-win/issues/14299) 和 [docker/for-win#14316](https://github.com/docker/for-win/issues/14316)。
...
- 如果在 `allowedOrgs` Windows 注册表项中使用空格分隔的格式指定了多个组织，登录将失败，用户将被注销。作为解决方法，请在注册表项值中将每个组织单独列在一行上。
...
- 用户现在可以在生成私有扩展市场时使用新的 `--timeout` 标志指定超时。
...
- 在显示特权辅助错误消息时切换到 Windows 容器可能会导致状态不一致。作为解决方法，请退出 Docker Desktop，将 `settings-store.json` 中的 `UseWindowsContainers` 更改为 `false`，然后重新启动 Docker Desktop。
...
- 通过在 `docker history` 中启用用户选择特定平台，支持多平台镜像。
- 修复了当存在带有端口映射的容器时，导致 CLI 和 Docker Desktop 以外的客户端出现 3 秒延迟的问题。请参阅 [docker/for-mac#7575](https://github.com/docker/for-mac/issues/7575)
- 修复了 ECI Docker 套接字权限中的一个错误，该错误有时会阻止在允许的镜像或派生自允许镜像的镜像上挂载 Docker 套接字。
- 修复了在引擎重启后 Docker Desktop 无法再次立即进入资源节省模式的错误。
- 修复了由于过期的 PKI 证书导致 Kubernetes 集群停止工作的问题。

#### 对于 Mac

- 将 Linux 内核降级到 `v6.1.14`，以修复 OpenJDK 中的一个错误，该错误导致 Java 容器因 cgroups 控制器错误识别而终止。请参阅 [docker/for-mac#7573](https://github.com/docker/for-mac/issues/7573)。
...
- 修复了增强容器隔离中的一个错误，即在将 Docker 套接字挂载到容器中，然后从该容器内部使用绑定挂载创建 Docker 容器时。
...
- 修复了 `admin-settings.json` 中 `pac` 设置被忽略的错误。
- 构建 UI：
  - 导入构建时添加了进度状态。
  - 修复了用户无法导入构建的错误。
...
- 防止了一个导致 Docker Desktop 无法将 `com.docker.vmnetd` 或 `com.docker.socket` 更新到较新版本的错误。

### 已知问题

#### 对于 Mac

- 如果您看到关于 `com.docker.vmnetd` 或 `com.docker.socket` 恶意软件的安全弹出窗口，请按照 [docker/for-mac#7527](https://github.com/docker/for-mac/issues/7527) 中记录的步骤操作。
...
- 修复了在使用[增强容器隔离](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/_index.md) 时，Docker Desktop 因 `index out of range [0] with length 0` 而发生恐慌的问题。
...
- Docker Engine 新安装的默认磁盘使用限制现在为 1TB。
- 修复了容器无法建立环回 `AF_VSOCK` 连接的问题。
- 修复了重置默认设置也会重置 CLI 上下文的错误。
- 修复了在资源节省模式下（仅限使用 WSL2 后端的 Windows）或切换引擎后（macOS），重启引擎时 Docker Desktop 仪表板与 Docker 守护进程失去同步的错误。
...
- 修复了当使用 `sudo` 两次运行卸载程序二进制文件时，某些用户目录会以 root 权限创建的错误。

#### 对于 Windows

- 添加了对使用 WSL 2 版本 2.3.24 及更高版本的 ARM 上 Windows 的支持，以实现 WSL 2 上的单发行版模式。
...
- QEMU（旧版）作为 Apple Silicon 上的 VMM 将在未来的版本中被移除。建议您切换到 Apple 虚拟化框架以提高性能和稳定性。如果遇到问题，请[联系 Docker 支持](https://www.docker.com/support/) 或[提交 GitHub 问题](https://github.com/docker/for-mac/issues)。
...
- 使用 WSL2 引擎的 Windows 上现有的 Docker Desktop 安装现在会自动迁移到统一的单发行版架构，以增强一致性和性能。
- 管理员现在可以：
...
- 修复了当 Docker Desktop 配置为使用代理时，由于在获取身份验证令牌时内部超时而无法启动的问题。
- 添加了恢复横幅，以便在下载失败时重试更新。
- 修复了当 `umask` 设置为 `577` 时会导致 `rpmbuild` 失败的问题。修复 [docker/for-mac#6511](https://github.com/docker/for-mac/issues/6511)。
...
- Docker Desktop 在手动暂停后不会自动取消暂停。系统将保持暂停状态，直到您手动恢复 Docker 引擎。这修复了一个错误，即其他软件可能会通过在后台运行 CLI 命令而意外触发恢复。修复 [for-mac/#6908](https://github.com/docker/for-mac/issues/6908)
- 构建 UI：
  - **源** 选项卡现在支持多个源文件。
...
- 修复了 Docker VMM 中的一个错误，该错误阻止了 MySQL 和其他数据库容器的启动。修复来自 [docker/for-mac#7464](https://github.com/docker/for-mac/issues/7464) 的报告。
- Docker VMM 的最低内存要求现在会自动调整，改善了用户体验并解决了来自 [docker/for-mac#7464](https://github.com/docker/for-mac/issues/7464)、[docker/for-mac#7482](https://github.com/docker/for-mac/issues/7482) 的报告。
...
- 修复了当 Docker Desktop 由非管理员用户安装或当前用户以前是管理员时，应用内更新会失败的错误。修复 [for-mac/#7403](https://github.com/docker/for-mac/issues/7403) 和 [for-mac/#6920](https://github.com/docker/for-mac/issues/6920)
...
- 对 [Red Hat Enterprise Linux 上的 Docker Desktop](/manuals/desktop/setup/install/linux/rhel.md) 的支持现已正式可用。
...
- 修复了当 ICMPv6 设置未成功时 Docker Desktop 启动失败的问题。
- 添加了允许 USB/IP 工作的驱动程序。
- 修复了增强容器隔离 (ECI) [派生镜像的 Docker 套接字挂载权限](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/config.md) 中的一个错误，该错误在 Docker Desktop 使用 containerd 镜像存储时，错误地拒绝了某些镜像的 Docker 套接字挂载。
...
- 修复了在启动时不会填充 `~/.docker/cli-plugins` 的错误。
...
- 修复了如果在内核配置中或通过内核命令行在 WSL 中禁用了 IPv6，则会导致启动失败的问题。修复 [docker/for-win#14240](https://github.com/docker/for-win/issues/14240)
...
- Windows 现在支持在使用[托管虚拟硬盘](/manuals/desktop/features/wsl/best-practices.md) 的 WSL2 安装中自动回收 Docker Desktop 的磁盘空间。
...
- 修复了当容器使用 AutoRemove (`--rm`) 启动但其端口绑定被 Docker Desktop 在启动时拒绝时，导致 CLI 变得空闲的错误。
- 修复了在 **支持** 屏幕上诊断收集会随机失败的错误。
- 修复了容器的 **文件** 选项卡中文件夹无法展开的错误。修复 [docker/for-win#14204](https://github.com/docker/for-win/issues/14204)。
- 应用内更新现在尊重代理设置。
- 扩展了 ECI Docker 套接字挂载权限功能，可选择允许派生自允许镜像的子镜像。这允许 ECI 与创建使用 Docker 套接字挂载的临时本地镜像的 buildpack（例如 Paketo）一起使用。
...
- 修复了触发配置完整性检查功能时偶尔出现 `Partial repair error` 的错误。
- 配置完整性检查功能现在显示有关 Docker 套接字配置错误原因的信息。
- 修复了当 Docker Desktop 作为 `用户` 安装时，配置完整性检查功能会报告系统路径而不是用户路径的问题。
- 修复了尝试从绑定挂载卷读取扩展属性的应用程序可能会失败的错误。修复 [docker/for-mac#7377](https://github.com/docker/for-mac/issues/7377)。
...
- Compose Bridge 在 **实验性** 设置选项卡中启用后不会自动工作。需要几分钟时间才会通知您必须“修复” Docker Desktop，然后安装 `compose-bridge` 二进制文件。
...
- 在 Docker CLI 中针对注册表进行身份验证时存在一个已知问题 (`docker login [registry address]`)，如果提供的注册表地址包含存储库/镜像名称（例如 `docker login index.docker.io/docker/welcome-to-docker`），则存储库部分 (`docker/welcome-to-docker`) 不会被规范化，导致凭据存储不正确，这会导致后续从注册表拉取 (`docker pull index.docker.io/docker/welcome-to-docker`) 时无法进行身份验证。为防止这种情况，在运行 `docker login` 时不要在注册表地址中包含任何额外的后缀。
  > [!NOTE]
  > 使用包含 URL 路径段的地址进行 `docker login` 不是文档化的用例，被视为不受支持。推荐的用法是仅将注册表主机名（以及可选的端口）指定为 `docker login` 的地址。
...
- BuildKit 现在会评估 Dockerfile 规则以告知您潜在问题。
...
- Docker Engine 的 `default-network-opts` 参数现在得到正确验证。
...
- 配置完整性检查功能现在提供更多有关 Docker Desktop 配置更改内容的上下文。更多信息，请参阅[常见问题解答](/manuals/desktop/troubleshoot-and-support/faqs/macfaqs.md)。
...
- 修复了某些 `wincred` 值在卸载后仍然存在的问题。由 Javier Yong [@Javiery3889](https://github.com/Javiery3889) 报告。
...
- 移除了对旧版 Visual Studio Code 的临时兼容性修复。
- 构建视图：
...
- 修复了一个回归问题，即当 `/var/run/docker.socket` 指向意外路径时，总是会出现 **另一个应用程序更改了您的桌面配置** 警告消息。
- 将配置检查菜单项和横幅更改为通知。
- 改进了绑定挂载上读写操作的性能。
...
- 修复了导致绑定挂载失败的问题。修复 [docker/for-mac#7274](https://github.com/docker/for-mac/issues/7274)。

### 已知问题

#### 所有平台

- **使用 Compose 管理同步文件共享** 设置会自动为所有选择加入 **访问实验性功能** 的用户启用。这会将所有绑定挂载转换为同步文件共享。要禁用此行为，请取消选中 **访问实验性功能**。然后，手动删除任何文件共享：转到 **资源** 中的 **文件共享** 选项卡，导航到 **同步文件共享** 部分，选择要删除的文件共享，然后选择 **删除**。
...
- Docker Compose 文件查看器以语法高亮显示您的 Compose YAML，并提供指向相关文档的上下文链接（Beta，逐步推出）。
- 新的侧边栏用户体验。
...
- 修复了在 macOS 和使用 Hyper-V 的 Windows 上使用 gRPC FUSE 文件共享时，主机上修改的绑定挂载文件在容器重启后未更新的错误。修复 [docker/for-mac#7274](https://github.com/docker/for-mac/issues/7274)、[docker/for-win#14060](https://github.com/docker/for-win/issues/14060)。
- 构建视图：
  - 新的[导入构建](use-desktop/builds.md#import-builds) 功能，允许您导入其他人或 CI 环境中的构建记录。
...
- 修复了增强容器隔离 (ECI) 模式中的一个安全漏洞，该漏洞允许用户从 Docker Desktop VM 内的受限目录创建 Docker 卷，并将其挂载到容器中，从而让容器访问此类受限的 VM 目录。
- 默认情况下，只有列出在市场中的扩展才能在 Docker Desktop 中安装。这可以在 Docker Desktop 的设置中更改。扩展开发人员需要更改此选项才能测试他们的扩展。

### 对于 Windows

- 修复了 [CVE-2024-5652](https://www.cve.org/cverecord?id=CVE-2024-5652)，该问题允许 `docker-users` 组中的用户通过 Windows 容器模式下的 `exec-path` Docker 守护进程配置选项导致 Windows 拒绝服务。此漏洞由 Hashim Jawad ([@ihack4falafel](https://github.com/ihack4falafel)) 与 Trend Micro Zero Day Initiative 合作发现。

### 弃用

#### 所有平台

- 以前作为 `com.docker.cli` 发送的 CLI 二进制文件现在简单地作为 `docker` 发送。此版本将 CLI 二进制文件保留为 `com.docker.cli`，但将在下一个版本中移除。
...
- 尚不支持 Ubuntu 20.04 LTS，Docker Desktop 将无法启动。由于最新 Ubuntu 版本限制非特权命名空间的方式发生了变化，需要至少运行一次 `sudo sysctl -w kernel.apparmor_restrict_unprivileged_userns=0`。请参阅 [Ubuntu 博客](https://ubuntu.com/blog/)。
...
- 添加了一个新设置来管理[设置管理](/manuals/enterprise/security/hardened-desktop/settings-management/_index.md) 中的入门调查。
...
- 修复了自诊断 CLI 中的一个错误，该错误错误地显示 VM 未启动。修复 [docker/for-mac#7241](https://github.com/docker/for-mac/issues/7241)。
- 修复了高吞吐量端口转发传输可能停滞的错误。修复 [docker/for-mac#7207](https://github.com/docker/for-mac/issues/7207)。
- 修复了在移除 CLI 应用程序时未移除 CLI 插件符号链接的错误。
- 修复了共享端口抽屉中的错误，以显示本地引擎的正确消息。
- Dev Environments 正在逐步淘汰，并已移至 **开发中功能** 中的 **Beta** 选项卡。
- 构建视图：
  - 更好的批量删除构建记录。
  - 为容器镜像和 Git 源的构建依赖项添加了打开相关网页的操作。
  - 添加了以 Jaeger 或 OTLP 格式下载溯源和 OpenTelemetry 迹线的操作。
  - 修复了远程构建调用的源详细信息。
  - 修复了当使用云构建器时，多平台构建显示为单独记录的错误。
...
- 修复了 Docker Desktop 4.28 中的一个回归问题，该问题导致在 WSL 上使用 Docker-in-Docker（通过挂载 `/var/run/docker.sock`）时，主机文件绑定挂载到容器中无法在容器内正确显示。
...
- 如果您启用了 Docker Desktop 中需要登录的功能，例如 **主机网络**，您必须保持登录状态才能使用 Docker Desktop。要继续使用 Docker Desktop 或修改这些设置，请确保您已登录。
...
- 当以非管理员用户身份启动 Docker Desktop 时，如果用户不是 **docker-users** 组的成员，可能会触发连接 `ENOENT \\.\pipe\errorReporter` 错误。
...
- 尚不支持 Ubuntu 20.04 LTS，Docker Desktop 将无法启动。由于最新 Ubuntu 版本限制非特权命名空间的方式发生了变化，需要至少运行一次 `sudo sysctl -w kernel.apparmor_restrict_unprivileged_userns=0`。有关更多详细信息，请参阅 [Ubuntu 博客](https://ubuntu.com/blog/ubuntu-23-10-restricted-unprivileged-user-namespaces)。
...
- 修复了在选择 **导出到本地镜像** 字段时显示警告“_提供给自动完成的值无效。_”的问题。
...
- 修复了使用 QEMU 启动 Docker Desktop 时出现的竞争条件。
...
- 修复了 [CVE-2024-6222](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2024-6222)，该问题允许通过容器逃逸获得 Docker Desktop VM 访问权限的攻击者通过传递扩展和仪表板相关的 IPC 消息进一步逃逸到主机。由 Billy Jheng Bing-Jhong、Đỗ Minh Tuấn、Muhammad Alifa Ramdhan 与 Trend Micro Zero Day Initiative 合作报告。

### 已知问题

#### 对于 Mac

- 如果未安装 Rosetta，Apple Silicon 上的 Docker Desktop 将无法启动。这将在未来的版本中修复。请参阅 [docker/for-mac#7243](https://github.com/docker/for-mac/issues/7243)。
...
- 修复了 `Qemu` 下的 `postgis` 问题。修复 [docker/for-mac#7172](https://github.com/docker/for-mac/issues/7172)。
- 重新添加了 `kpartx` 所需的 `CONFIG_BLK_DEV_DM` 内核配置。修复 [docker/for-mac#7197](https://github.com/docker/for-mac/issues/7197)。
- 允许通过代理自动配置 `pac 文件` 设置 `SOCKS` 代理。
- 重新添加了 `CONFIG_AUDIT` 内核配置。
- 修复了在 `virtiofs` 上 Rust 构建的错误。请参阅 [rust-lang/docker-rust#161](https://github.com/rust-lang/docker-rust/issues/161)。
- 修复了拉取 Kubernetes 镜像时出现 `missing registry authentication` 错误的问题。
...
- 修复了即使禁用了 **在 Docker Desktop 启动时打开 Docker 仪表板** 选项，仪表板在启动时仍会显示的错误。修复 [docker/for-win#13887](https://github.com/docker/for-win/issues/13887)。
...
- [同步文件共享](/manuals/desktop/features/synchronized-file-sharing.md) 在 Docker Desktop 内提供快速灵活的主机到 VM 文件共享。利用 [Docker 收购 Mutagen](https://www.docker.com/blog/mutagen-acquisition/) 背后的技术，此功能提供了使用同步文件系统缓存的替代虚拟绑定挂载，从而提高了处理大型代码库的开发人员的性能。
...
- 修复了即使选中了 **始终下载更新** 复选框，自动更新也不会下载的错误。
...
- 容器使用情况图表已移至垂直的 **资源使用情况** 侧面板，以便在容器列表中留出更多空间。通过 **显示图表** 按钮访问使用情况图表的方式保持不变。
...
- 修复了在某些区域设置下无法将 UTF-16 字符串编码为 UTF-8 的错误。修复 [docker/for-win#13868](https://github.com/docker/for-win/issues/13868)。
- 修复了在应用程序重启时使用 WSL 集成时凭据存储配置会重置的错误。修复 [docker/for-win#13529](https://github.com/docker/for-win/issues/13529)。
...
- 缓解了 Windows 上 Docker Desktop 安装程序中的几个 DLL 侧加载漏洞，由 Suman Kumar Chakraborty ([@Hijack-Everything](https://github.com/Hijack-Everything)) 报告。

### 已知问题

#### 所有平台

- 当使用设置管理时，未在 `admin-settings.json` 中设置的设置将在 Docker Desktop 启动时重置为默认值。

#### 对于 Mac

- 从 **软件更新** 更新到 4.27.0 有时会挂起。作为解决方法，请使用此页面的 4.27.0 安装程序。
...
- 更新了 Docker Desktop 内部的反馈链接以确保其继续正常工作
...
- [**构建** 视图](use-desktop/builds.md) 现已 GA。您现在可以检查构建、排除错误并优化构建速度。
...
- containerd 集成：
  ...
  - 修复了在配置了 `--userns-remap` 的守护程序下 `docker commit` 的问题。
...
- 修复了 Rosetta 无法与 PHP 一起使用的问题。修复 [docker/for-mac#6773](https://github.com/docker/for-mac/issues/6773) 和 [docker/for-mac#7037](https://github.com/docker/for-mac/issues/7037)。
- 修复了多个与 Rosetta 不工作相关的问题。修复 [[docker/for-mac#6973](https://github.com/docker/for-mac/issues/6973)、[[docker/for-mac#7009](https://github.com/docker/for-mac/issues/7009)、[[docker/for-mac#7068](https://github.com/docker/for-mac/issues/7068) 和 [[docker/for-mac#7075](https://github.com/docker/for-mac/issues/7075)
...
- 解决了当通过设置管理禁用分析时 Docker Desktop 变得无响应的问题。
...
- 当在较旧的 Linux 发行版（例如 Ubuntu 20.04）上使用 WSL 2 集成时，Docker CLI 不起作用，该发行版使用的 `glibc` 版本早于 `2.32`。这将在未来的版本中修复。请参阅 [docker/for-win#13824](https://github.com/docker/for-win/issues/13824)。
...
- 修复了在 WSL 2 上 Docker Desktop 在空闲时会意外关闭 dockerd 的错误。修复 [docker/for-win#13789](https://github.com/docker/for-win/issues/13789)
...
- 修复了 **切换到 Windows 容器** 选项未在某些用户的托盘菜单中显示的问题。请参阅 [docker/for-win#13761](https://github.com/docker/for-win/issues/13761)。
...
- Docker Desktop 现在会检测 WSL 版本是否过时。如果检测到过时的 WSL 版本，您可以允许 Docker Desktop 自动更新安装，也可以在 Docker Desktop 外手动更新 WSL。
...
- 修复了 Docker Desktop 退出[资源节省模式](https://docs.docker.com/desktop/use-desktop/resource-saver/) 的错误，该模式每 15 分钟退出一次，或者，如果计时器设置为超过 15 分钟，资源节省模式永远不会启动。
...
- 资源节省模式现已对所有用户可用，并且默认启用。要配置此功能，请导航到 **设置** 中的 **资源** 选项卡。有关更多信息，请参阅 [Docker Desktop 的资源节省模式](use-desktop/resource-saver.md)。
...
- 修复了在早于 12.5 的 macOS 版本的 Apple Silicon Mac 上出现内核恐慌的问题。修复 [docker/for-mac#6975](https://github.com/docker/for-mac/issues/6975)。
...
- 修复了 4.23 中引入的资源泄漏问题。与 [docker/for-mac#6953](https://github.com/docker/for-mac/issues/6953) 相关。
...
- 创建端口为 53 的容器会失败，并显示地址 `already in use` 错误。作为解决方法，请通过在 `settings.json` 文件（位于 `~/Library/Group Containers/group.com.docker/settings.json`）中添加 `"kernelForUDP": false` 来停用网络加速。
...
- containerd 镜像存储：
  ...
  - 修复了使用旧式经典构建器 (`DOCKER_BUILDKIT=0`) 构建镜像时出现的 `mismatched image rootfs errors`。
  - 修复了 `ONBUILD` 和 `MAINTAINER` Dockerfile 指令
  - 修复了健康检查。

#### 对于 Mac

- 所有使用 macOS 12.5 或更高版本的用户现在默认启用 VirtioFS。您可以在 **常规** 选项卡的 **设置** 中恢复此设置。
- 改进了单流 TCP 吞吐量。
- 恢复了 macOS 的健康检查，当系统上发生可能导致运行 Docker 二进制文件出现问题的更改时会通知您。
...
- 修复了当非管理员用户切换到 Windows 容器或禁用 WSL 并切换到 Hyper-V 引擎时，会提示用户输入凭据的错误。
...
- 修复了当 vpnkit 预期未运行时，自诊断工具显示误报失败的错误。修复 [docker/for-win#13479](https://github.com/docker/for-win/issues/13479)。
- 修复了搜索栏中的无效正则表达式导致错误的错误。修复 [docker/for-win#13592](https://github.com/docker/for-win/issues/13592)。
...
- 修复了使用 SSH 的 Docker 上下文的连接泄漏问题 ([docker/for-mac#6834](https://github.com/docker/for-mac/issues/6834) 和 [docker/for-win#13564](https://github.com/docker/for-win/issues/13564))
...
- VirtioFS 现在是在 macOS 12.5 及更高版本上全新安装 Docker Desktop 的默认文件共享实现。
- 改进了使用 OpenTelemetry 的产品使用情况报告（实验性）。
- 修复了 Docker 套接字权限问题。修复 [docker/for-win#13447](https://github.com/docker/for-win/issues/13447) 和 [docker/for-mac#6823](https://github.com/docker/for-mac/issues/6823)。
...
- 添加了 macOS 的健康检查，当系统上发生可能导致运行 Docker 二进制文件出现问题的更改时通知用户。

#### 对于 Windows

- 修复了 WSL 2 上的一个错误，即如果 Desktop 被暂停、终止然后重启，除非首先使用 `wsl --shutdown` 关闭 WSL，否则启动会挂起。
- 修复了当 wsl.exe 不在 PATH 中时 WSL 引擎的问题 [docker/for-win#13547](https://github.com/docker/for-win/issues/13547)。
- 修复了 WSL 引擎检测 Docker Desktop 发行版的驱动器缺失的情况的能力 [docker/for-win#13554](https://github.com/docker/for-win/issues/13554)。
...
- 在 WSL-2 上启用了 Docker Desktop 的增强容器隔离 (ECI) 功能。此功能随 Docker Business 订阅提供。
- 修复了 **容器** 表中的一个错误，即在全新安装 Docker Desktop 后，以前隐藏的列会再次显示。
...
- 修复了 `com.docker.diagnose check` 中的一个错误，即使 vpnkit 预期未运行，它也会抱怨缺少 vpnkit。与 [docker/for-mac#6825](https://github.com/docker/for-mac/issues/6825) 相关。
...
- 添加了在安装期间设置 HTTP 代理配置的支持。这可以通过 `--proxy-http-mode`、`--overrider-proxy-http`、`--override-proxy-https` 和 `--override-proxy-exclude` 安装程序标志在 [Mac](/manuals/desktop/setup/install/mac-install.md#install-from-the-command-line) 和 [Windows](/manuals/desktop/setup/install/windows-install.md#install-from-the-command-line) 上从 CLI 安装时完成，或者通过在 `install-settings.json` 文件中设置值来完成。
- Docker Desktop 现在停止在应用程序启动时覆盖 .docker/config.json `credsStore` 键。请注意，如果您使用自定义凭据助手，则 CLI `docker login` 和 `docker logout` 不会影响 UI 是否登录到 Docker。通常，最好通过 UI 登录到 Docker，因为 UI 支持多因素身份验证。
...
- 修复了端口转发中的各种错误。
- 修复了 HTTP 代理中的一个错误，即没有服务器名称指示 (SNI) 记录的 HTTP 请求会被拒绝并显示错误。
...
- 修复了更新 Docker Desktop 时桌面上出现虚假的 `courgette.log` 文件的错误。修复 [docker/for-win#12468](https://github.com/docker/for-win/issues/12468)。
...
- 提高了在 macOS Ventura 及更高版本上使用虚拟化框架时的 VM 网络性能。Mac 版 Docker Desktop 现在使用 gVisor 代替 VPNKit。要继续使用 VPNKit，请将 `"networkType":"vpnkit"` 添加到位于 `~/Library/Group Containers/group.com.docker/settings.json` 的 `settings.json` 文件中。
- 修复了卸载时显示错误窗口的错误。
...
- 修复了一个安全问题，该问题允许用户通过从其 Docker CLI 配置文件中删除 `credsStore` 键来绕过其组织配置的镜像访问管理 (IAM) 限制。仅影响 Docker Business 客户。
...
- 添加了 Docker Compose 的实验性文件监视命令，可在您编辑和保存代码时自动更新正在运行的 Compose 服务。
...
- 修复了当服务器名称指示字段未设置时透明 TLS 代理中的错误。
...
- 修复了 [CVE-2023-1802](https://www.cve.org/cverecord?id=CVE-2023-1802)，该问题导致 Artifactory 集成中的安全问题会在 HTTPS 检查失败时回退到通过纯 HTTP 发送注册表凭据。仅启用了 **访问实验性功能** 的用户会受到影响。修复 [docker/for-win#13344](https://github.com/docker/for-win/issues/13344)。
...
- **容器文件浏览器** 作为实验性功能可用。直接从 GUI 调试容器内的文件系统。
- 您现在可以在 **全局搜索** 中搜索卷。
...
- 修复了 vmnetd 安装在区分大小写的文件系统上无法读取 `Info.Plist` 的错误。实际文件名是 `Info.plist`。修复 [docker/for-mac#6677](https://github.com/docker/for-mac/issues/6677)。
...
- 改进了 kubectl 符号链接逻辑，以尊重 `/usr/local/bin` 中现有的二进制文件。修复 [docker/for-mac#6328](https://github.com/docker/for-mac/issues/6328)。
...
- 修复了在 WSL 2 模式下（在 4.16 中引入）Docker Desktop 在“启动”阶段卡住的问题。
...
- 修复了当目标路径有空格时，在 Visual Studio Code 中打开失败的错误。
...
- 修复了 [CVE-2023-0628](https://www.cve.org/cverecord?id=CVE-2023-0628)，该问题允许攻击者在 Dev Environments 容器初始化期间通过诱骗用户打开精心制作的恶意 `docker-desktop://` URL 在容器内执行任意命令。
- 修复了 [CVE-2023-0629](https://www.cve.org/cverecord?id=CVE-2023-0629)，该问题允许非特权用户通过将 Docker 主机设置为 `docker.raw.sock` 或 Windows 上的 `npipe:////.pipe/docker_engine_linux` 来绕过增强容器隔离 (ECI) 限制，或者通过 `-H` (`--host`) CLI 标志或 `DOCKER_HOST` 环境变量，并在没有 ECI 提供的额外强化功能的情况下启动容器。这不会影响已在运行的容器，也不会影响通过常规方法（不使用 Docker 原始套接字）启动的容器。
...
- 修复了 Docker Desktop 4.16 中引入的回归问题，导致在 WSL2 模式下卡在“启动”阶段。修复 [docker/for-win/13165](https://github.com/docker/for-win/issues/13165)
...
- 修复了容器内的 `sudo` 因某些镜像的安全相关错误而失败的问题。修复 [docker/for-mac/6675](https://github.com/docker/for-mac/issues/6675) 和 [docker/for-win/13161](https://github.com/docker/for-win/issues/13161)。
...
- 修复了在使用 `containerd` 集成时 `docker inspect` 未显示镜像标签的问题。
- 增加了正在运行和已停止容器图标之间的对比度，使色盲用户更容易扫描容器列表。
- 修复了在 Docker Desktop 重启之前，用户被重复提示输入新 HTTP 代理凭据的错误。
...
- 在所有实验性功能旁边添加了信息横幅，说明如何选择退出实验性功能访问。
...
- 修复了在每次重启后提示用户安装 `/var/run/docker.sock` 符号链接的问题。
- 确保安装 `/var/run/docker.sock` 符号链接的登录项已签名。
- 修复了在恢复出厂设置时删除 `$HOME/.docker` 的错误。

### 对于 Windows

- 修复了 `docker build` 在打印 "load metadata for" 时挂起的错误。修复 [docker/for-win#10247](https://github.com/docker/for-win/issues/10247)。
...
- 对 macOS 用户进行了实质性性能改进，可以选择启用新的 VirtioFS 文件共享技术。适用于 macOS 12.5 及更高版本。
...
- Mac 和 Linux 上的 Docker Desktop 仪表板页脚中现在显示 VM 的可用磁盘空间。
- 如果可用空间低于 3GB，页脚中会显示磁盘空间警告。
- 随着我们变得更加 ADA 可访问和视觉统一，Docker Desktop 界面的更改。
...
- Dev Environments 允许将 Git 存储库克隆到本地绑定挂载，因此您可以使用任何本地编辑器或 IDE。
- 更多 Dev Environments 改进：自定义名称、更好的私有存储库支持、改进的端口处理。
...
- 为镜像实现了 "reference" 过滤器（使用 containerd 集成）。
- 添加了通过 `proxy.pac` 在容器、`docker pull` 等中自动选择上游 HTTP/HTTPS 代理的支持。
- 修复了在拉取时解析镜像引用的回归问题。修复 [docker/for-win#13053](https://github.com/docker/for-win/issues/13053)、[docker/for-mac#6560](https://github.com/docker/for-mac/issues/6560) 和 [docker/for-mac#6540](https://github.com/docker/for-mac/issues/6540)。
...
- 修复了当 Docker 启动且开发人员登录时，未使用系统 HTTP 代理的问题。
...
- 修复了如果 `/usr/local/lib` 尚不存在，则不会为用户创建符号链接的错误。修复 [docker/for-mac#6569](https://github.com/docker/for-mac/issues/6569)
...
- 将 qemu 恢复到 6.2.0，以修复在运行模拟 amd64 代码时出现 `PR_SET_CHILD_SUBREAPER is unavailable` 等错误。
- 在扩展中启用了 [contextIsolation](https://www.electronjs.org/docs/latest/tutorial/context-isolation) 和 [sandbox](https://www.electronjs.org/docs/latest/tutorial/sandbox) 模式。现在扩展在单独的上下文中运行，这通过限制对大多数系统资源的访问来限制恶意代码可能造成的损害。
...
- 移除了容器列表上的无限微调器。[修复 https://github.com/docker/for-mac/issues/6486](https://github.com/docker/for-mac/issues/6486)
- 修复了在 **设置** 中显示已用空间值不正确的错误。
- 修复了在使用 containerd 集成时导致 Kubernetes 无法启动的错误。
- 修复了在使用 containerd 集成时导致 `kind` 无法启动的错误。
- 修复了在使用 containerd 集成时导致 Dev Environments 无法工作的错误。
...
- 修复了虚拟化框架用户的启动失败问题。
- 重新添加了 Mac 上的 `/var/run/docker.sock`，以增加与 `tilt` 和 `docker-py` 等工具的兼容性。
- 修复了在全新 Mac 安装上无法创建 Dev Environments 的问题（错误 "Cannot connect to the Docker daemon at unix:///var/run/docker.sock. Is the docker daemon running?"）。
...
- 修复了 Compose v2 在 Windows 上并非始终默认启用的问题。
- Docker Desktop 现在会在卸载时删除 `C:\Program Files\Docker` 文件夹。

### 已知问题

- 对于 Mac OS 上的某些用户，安装程序存在一个已知问题，会阻止安装 Docker Desktop 中漏洞和包发现实验性功能所需的新辅助工具。要解决此问题，需要一个符号链接，可以使用以下命令创建：`sudo ln -s /Applications/Docker.app/Contents/Resources/bin/docker-index /usr/local/bin/docker-index`
...
- 重新添加了 Mac 上的 `/var/run/docker.sock` 符号链接，以增加与 `tilt` 和 `docker-py` 等工具的兼容性。修复 [docker/for-mac#6529](https://github.com/docker/for-mac/issues/6529)。
- 修复了在全新 Mac 安装上无法创建 Dev Environments 并导致 `error "Cannot connect to the Docker daemon at unix:///var/run/docker.sock. Is the docker daemon running?")` 的问题。
...
- 为 Docker Business 用户引入了两个新的安全功能：设置管理和增强容器隔离。阅读有关 Docker Desktop 新的[强化 Docker Desktop 安全模型](/manuals/enterprise/security/hardened-desktop/_index.md) 的更多信息。
- 添加了新的 Dev Environments CLI `docker dev`，因此您可以通过命令行创建、列出和运行 Dev Env。现在更容易将 Dev Env 集成到自定义脚本中。
...
- [Linux kernel 5.15.49](https://hub.docker.com/layers/docker/for-desktop-kernel/5.15.49-13422a825f833d125942948cf8a8688cef721ead/images/sha256-ebf1f6f0cb58c70eaa260e9d55df7c43968874d62daced966ef6a5cd96b493?context=explore)
...
- Docker Desktop 现在将 HTTP 和 HTTPS 代理密码存储在 OS 凭据存储中。
...
- 为扩展市场添加了类别。
- 在鲸鱼菜单和 **扩展** 选项卡上添加了指示器，指示何时有扩展更新可用。
- 修复了卸载具有无命名空间镜像名称（例如 'my-extension'）的扩展失败的问题。
- 在 **容器** 选项卡中显式显示端口映射。
- 将镜像磁盘使用信息的刷新率更改为每天自动发生一次。
- 使 **容器** 和 **卷** 选项卡的选项卡样式保持一致。
- 修复了 **设置** 中 Grpcfuse 文件共享模式的启用问题。修复 [docker/for-mac#6467](https://github.com/docker/for-mac/issues/6467)
...
- 添加了一个小改进，以便在侧边栏中单击 **扩展** 时导航到扩展市场。
- 添加了一个徽章以识别市场中的新扩展。
- 修复了在使用 containerd 集成时 Kubernetes 无法启动的问题。
- 修复了在使用 containerd 集成时 `kind` 无法启动的问题。
- 修复了在使用 containerd 集成时 Dev Environments 无法工作的问题。
- 在 containerd 集成中实现了 `docker diff`。
- 在 containerd 集成中实现了 `docker run —-platform`。
- 修复了在使用 containerd 集成时不安全注册表无法工作的问题。
- 修复了在 **设置** 中显示已用空间值不正确的错误。
- Docker Desktop 现在从 Github 发布版安装凭据助手。参见 [docker/for-win#10247](https://github.com/docker/for-win/issues/10247)、[docker/for-win#12995](https://github.com/docker/for-win/issues/12995)。
...
- Docker Desktop 现在运行未标记的镜像。修复 [docker/for-mac#6425](https://github.com/docker/for-mac/issues/6425)。
- 为 Docker 扩展的市场添加了搜索功能。修复 [docker/roadmap#346](https://github.com/docker/roadmap/issues/346)。
- 添加了放大、缩小或将 Docker Desktop 设置为实际大小的能力。这可以通过键盘快捷键 ⌘ + / CTRL +, ⌘ - / CTRL -, ⌘ 0 / CTRL 0（在 Mac 和 Windows 上）或通过 Mac 上的视图菜单完成。
- 如果任何相关容器可停止，则添加 compose 停止按钮。
- 现在可以从 **容器** 视图中删除单个 compose 容器。
- 移除了针对 Fedora 35 上的 virtiofsd <-> qemu 协议不匹配的解决方法，因为不再需要。Fedora 35 用户应将 qemu 包升级到最新版本（截至撰写本文时为 qemu-6.1.0-15.fc35）。
- 为容器实现了集成终端。
...
- 修复了 [CVE-2023-0626](https://www.cve.org/cverecord?id=CVE-2023-0626)，该问题允许通过消息框路由中的查询参数在 Electron 客户端中进行 RCE。
- 修复了 [CVE-2023-0625](https://www.cve.org/cverecord?id=CVE-2023-0625)，该问题允许通过扩展描述/变更日志进行 RCE，可能被恶意扩展滥用。

#### 对于 Windows

- 修复了 [CVE-2023-0627](https://www.cve.org/cverecord?id=CVE-2023-0627)，该问题允许绕过在 4.11 版本中引入的 `--no-windows-containers` 安装标志。此标志允许管理员禁用 Windows 容器的使用。
- 修复了 [CVE-2023-0633](https://www.cve.org/cverecord?id=CVE-2023-0633)，该问题允许对 Docker Desktop 安装程序进行参数注入，可能导致本地提权。

### 错误修复和次要增强功能

#### 所有平台

- 恢复出厂设置后，Compose V2 现在已启用。
- 在 Docker Desktop 的全新安装中，Compose V2 现在默认启用。
- Compose 中环境变量的优先级顺序更加一致，并已明确[记录](/manuals/compose/how-tos/environment-variables/envvars-precedence.md)。
- 将内核升级到 5.10.124。
- 改进了因计算磁盘大小而导致的整体性能问题。与 [docker/for-win#9401](https://github.com/docker/for-win/issues/9401) 相关。
- Docker Desktop 现在会阻止未安装 Rosetta 的 ARM mac 用户切换回 Compose V1，后者仅具有 intel 二进制文件。
- 将卷大小和 **创建** 列的默认排序顺序更改为降序，以及容器的 **启动** 列。
- 重新组织了容器行操作，仅保留开始/停止和删除操作始终可见，而允许通过行菜单项访问其余操作。
- 快速入门指南现在立即运行每个命令。
- 定义了容器/compose **状态** 列的排序顺序：运行 > 部分运行 > 已暂停 > 部分暂停 > 已退出 > 部分退出 > 已创建。
- 修复了镜像列表在 Docker Desktop 中显示为空的问题，即使存在镜像。与 [docker/for-win#12693](https://github.com/docker/for-win/issues/12693) 和 [docker/for-mac#6347](https://github.com/docker/for-mac/issues/6347) 相关。
- 根据是否显示系统容器定义了哪些镜像“正在使用”。如果未显示与 Kubernetes 和扩展相关的系统容器，则相关镜像不被定义为“正在使用”。
- 修复了在某些语言中 Docker 客户端在 `docker exec` 上挂起的错误。修复 [https://github.com/apocas/dockerode/issues/534](https://github.com/apocas/dockerode/issues/534)。
- 当构建扩展时，生成的命令失败不再导致 Docker Desktop 意外退出。
- 修复了当扩展未禁用时，在左侧菜单中显示为禁用的错误。
- 修复了当注册表访问管理已启用且对 Docker Hub 的访问被阻止时，`docker login` 到私有注册表的问题。
...
- 在 macOS 上安装或更新 Docker Desktop 的最低版本现在是 10.15。修复 [docker/for-mac#6007](https://github.com/docker/for-mac/issues/6007)。
- 修复了在下载更新后，托盘菜单错误地显示“下载即将开始...”的错误。修复 [for-mac/issues#5677](https://github.com/docker/for-mac/issues/5677) 中报告的一些问题
- 修复了在应用更新后不会重启 Docker Desktop 的错误。
- 修复了当计算机睡眠时，如果用户使用 virtualization.framework 和限制性防火墙软件，会导致与 Docker 的连接丢失的错误。
- 修复了即使在用户退出应用程序后，Docker Desktop 仍在后台运行的错误。修复 [docker/for-mac##6440](https://github.com/docker/for-mac/issues/6440)
- 对于运行 macOS < 12.5 的用户，禁用了虚拟化框架和 VirtioFS。

#### 对于 Windows

- 修复了在更新期间显示的版本可能不正确的错误。修复 [for-win/issues#12822](https://github.com/docker/for-win/issues/12822)。
...
- 修复了从 WSL2 发行版登录到私有注册表的问题 [docker/for-win#12871](https://github.com/docker/for-win/issues/12871)
...
- Docker Desktop 现在在 VMware ESXi 和 Azure VM 中得到 Docker Business 客户的全面支持。有关更多信息，请参阅[在 VM 或 VDI 环境中运行 Docker Desktop](/manuals/desktop/setup/vm-vdi.md)
...
- 扩展市场中添加了排序扩展的功能。
- 修复了导致某些用户被过于频繁地要求提供反馈的错误。您现在每年只会被要求提供两次反馈。
- 为 Docker Desktop 添加了自定义主题设置。这允许您为 Docker Desktop 指定深色或浅色模式，独立于您的设备设置。修复 [docker/for-win#12747](https://github.com/docker/for-win/issues/12747)
...
- 更新了 Docker 主题调色板值以匹配我们的设计系统。
- 改进了当注册表访问管理阻止 Docker 引擎访问 Docker Hub 时 `docker login` 的错误消息。
- 提高了主机和 Docker 之间的吞吐量。例如提高了 `docker cp` 的性能。
- 收集诊断所需的时间更少。
- 在容器概览上选择或取消选择 compose 应用程序现在会选择/取消选择其所有容器。
- 容器概览镜像列中的标签名称可见。
- 在终端的滚动条中添加了搜索装饰，以便在视口之外可见匹配项。
- 修复了容器页面上搜索无法正常工作的问题 [docker/for-win#13828](https://github.com/docker/for-win/issues/13828)。
- 修复了导致 **卷** 屏幕无限加载的问题 [docker/for-win#12789](https://github.com/docker/for-win/issues/12789)。
- 修复了容器 UI 中调整大小或隐藏列不起作用的问题。修复 [docker/for-mac#6391](https://github.com/docker/for-mac/issues/6391)。
- 修复了在离开市场屏幕时，安装、更新或同时卸载多个扩展的状态丢失的错误。
- 修复了关于页面中的 compose 版本仅在重启 Docker Desktop 后才会从 v2 更新到 v1 的问题。
- 修复了用户无法看到日志视图的问题，因为其底层硬件不支持 WebGL2 渲染。修复 [docker/for-win#12825](https://github.com/docker/for-win/issues/12825)。
- 修复了容器和镜像的 UI 不同步的错误。
- 修复了启用实验性虚拟化框架时的启动竞争条件。
...
- 添加了 `--backend=windows` 安装程序选项以将 Windows 容器设置为默认后端。

#### 对于 Linux

- 修复了在路径中包含空格时设置文件共享的错误。
...
- 修复了在从 WSL 创建的 Compose 应用程序上 UI 操作失败的错误。修复 [docker/for-win#12806](https://github.com/docker/for-win/issues/12806)。

#### 对于 Mac

- 修复了因路径未初始化而导致安装命令失败的错误。修复 [docker/for-mac#6384](https://github.com/docker/for-mac/issues/6384)。
...
- 添加了使处理容器日志更轻松的功能，例如正则表达式搜索以及在容器仍在运行时清除容器日志的能力。
- 在容器表上实现了反馈。添加了端口并分隔了容器和镜像名称。
- 向扩展市场添加了两个新扩展：Ddosify 和 Lacework。

### 移除

- 在设计新设计时移除了主页。您可以在此处提供[反馈](https://docs.google.com/forms/d/e/1FAIpQLSfYueBkJHdgxqsWcQn4VzBn2swu4u_rMQRIMa8LExYb_72mmQ/viewform?entry.1237514594=4.10)。
...
- 在 **容器** 选项卡中为开始/暂停/停止选定容器添加了额外的批量操作。
- 在 **容器** 选项卡中为 compose 项目添加了暂停和重启操作。
- 在 **容器** 选项卡中添加了图标以及暴露的端口或退出代码信息。
- 外部 URL 现在可以使用诸如 `docker-desktop://extensions/marketplace?extensionId=docker/logs-explorer-extension` 之类的链接引用扩展市场中的扩展详细信息。
- Compose 应用程序的展开或折叠状态现在已保留。
- `docker extension` CLI 命令默认随 Docker Desktop 提供。
- 增加了扩展市场中显示的屏幕截图大小。
- 修复了如果其后端容器停止，Docker 扩展无法加载的错误。修复 [docker/extensions-sdk#16](https://github.com/docker/extensions-sdk/issues/162)。
- 修复了镜像搜索字段无故被清除的错误。修复 [docker/for-win#12738](https://github.com/docker/for-win/issues/12738)。
- 修复了许可证协议不显示并静默阻止 Docker Desktop 启动的错误。
- 修复了未发布的扩展显示的镜像和标签，以实际显示来自已安装的未发布扩展的镜像和标签。
- 修复了支持屏幕上重复的页脚。
- Dev Environments 可以从 GitHub 存储库的子目录创建。
- 移除了当使用 Docker Desktop 离线时无法加载每日提示的错误消息。修复 [docker/for-mac#6366](https://github.com/docker/for-mac/issues/6366)。

#### 对于 Mac

- 修复了 macOS 上 bash 补全文件位置的错误。修复 [docker/for-mac#6343](https://github.com/docker/for-mac/issues/6343)。
- 修复了当用户名超过 25 个字符时 Docker Desktop 无法启动的错误。修复 [docker/for-mac#6122](https://github.com/docker/for-mac/issues/6122)。
- 修复了当系统代理配置无效时 Docker Desktop 无法启动的错误。修复 [docker/for-mac#6289](https://github.com/docker/for-mac/issues/6289) 中报告的一些问题。
- 修复了启用实验性虚拟化框架时 Docker Desktop 无法启动的错误。
- 修复了卸载 Docker Desktop 后托盘图标仍然显示的问题。

#### 对于 Windows

- 修复了在 Hyper-V 上导致高 CPU 使用率的错误。修复 [docker/for-win#12780](https://github.com/docker/for-win/issues/12780)。
- 修复了 Windows 版 Docker Desktop 无法启动的错误。修复 [docker/for-win#12784](https://github.com/docker/for-win/issues/12784)。
- 修复了 `--backend=wsl-2` 安装程序标志未将后端设置为 WSL 2 的错误。修复 [docker/for-win#12746](https://github.com/docker/for-win/issues/12746)。
...
- 偶尔 Docker 引擎会在 `docker system prune` 期间重启。这是当前引擎中使用的 buildkit 版本的[已知问题](https://github.com/moby/buildkit/pull/2177)，将在未来的版本中修复。
...
- 在主页上添加了关于以下内容的附加指南：Elasticsearch、MariaDB、Memcached、MySQL、RabbitMQ 和 Ubuntu。
- 向 Docker Desktop 仪表板添加了页脚，其中包含有关 Docker Desktop 更新状态和 Docker 引擎统计信息的一般信息
- 重新设计了容器表，添加了：
  - 将容器 ID 复制到剪贴板的按钮
  - 每个容器的暂停按钮
  - 容器表的列调整大小
...
- 改进了 Docker Desktop 仪表板中的日志搜索功能以允许空格。
- 仪表板中按钮上的鼠标中键点击现在行为与左键点击相同，而不是打开空白窗口。
...
- 修复了 WSL 2 集成中的一个错误，即如果文件或目录被绑定挂载到容器中，并且容器退出，则该文件或目录将被同名的其他类型对象替换。例如，如果文件被目录替换或目录被文件替换，则尝试绑定挂载新对象会失败。
- 修复了托盘图标和仪表板 UI 不显示且 Docker Desktop 未完全启动的错误。修复 [docker/for-win#12622](https://github.com/docker/for-win/issues/12622)。

### 已知问题

#### 对于 Linux

- 更改绑定挂载中文件的所有权失败。这是由于我们在主机和运行 Docker 引擎的 VM 之间实现文件共享的方式所致。我们旨在在下一个版本中解决此问题。
...
- 修复了禁用扩展时高 CPU 使用率的问题。修复 [docker/for-mac#6310](https://github.com/docker/for-mac/issues/6310)。
...
- [Docker 扩展](/manuals/extensions/_index.md) 和扩展 SDK 的 Beta 版发布。
- 创建了一个 Docker 主页，您可以在其中运行流行镜像并发现如何使用它们。
- [Compose V2 现已 GA](https://www.docker.com/blog/announcing-compose-v2-general-availability/)

### 错误修复和增强功能

- 修复了在更新 Docker Desktop 时删除 Kubernetes 集群的错误。
...
- 引入了读取系统代理。除非与您的操作系统级代理不同，否则您不再需要手动配置代理。
- 修复了在代理后面运行时在仪表板中显示远程存储库的错误。
- 修复了 vpnkit 建立并阻止客户端连接的问题，即使服务器已消失。参见 [docker/for-mac#6235](https://github.com/docker/for-mac/issues/6235)
- 改进了 Docker Desktop 中的卷选项卡：
  - 显示卷大小。
  - 可以调整列的大小、隐藏和重新排序。
  - 列的排序顺序和隐藏状态会保留，即使在 Docker Desktop 重启后也是如此。
  - 在选项卡之间切换时，行选择会保留，即使在 Docker Desktop 重启后也是如此。
- 修复了 Dev Environments 选项卡中的错误，该错误在向屏幕添加更多项目时未添加滚动条。
- 标准化仪表板中的标题标题和操作。
- 添加了通过 HTTP 代理下载注册表访问管理策略的支持。
- 修复了当计算机长时间处于睡眠模式时远程存储库为空的问题。
- 修复了在清理过程中未选择悬空镜像的错误，如果其名称未标记为 "&lt;none>" 但其标签是。
- 改进了当 `docker pull` 因需要 HTTP 代理而失败时的错误消息。
- 添加了在 Docker Desktop 中轻松清除搜索栏的功能。
- 将“容器/应用”选项卡重命名为“容器”。
- 修复了当 `C:\ProgramData\DockerDesktop` 是文件或符号链接时，Docker Desktop 安装程序中静默崩溃的问题。
- 修复了当镜像没有命名空间时（例如 `docker pull <private registry>/image`），除非在设置中启用了对 Docker Hub 的访问，否则会被注册表访问管理错误地阻止的错误。

#### 对于 Mac

- Docker Desktop 的图标现在符合 Big Sur 风格指南。参见 [docker/for-mac#5536](https://github.com/docker/for-mac/issues/5536)
- 修复了 Dock 图标重复和 Dock 图标无法按预期工作的问题。修复 [docker/for-mac#6189](https://github.com/docker/for-mac/issues/6189)。
- 改进了对 `Cmd+Q` 快捷键的支持。
...
- 目前，如果您正在运行 Kubernetes 集群，它将在升级到 Docker Desktop 4.8.0 时被删除。我们旨在在下一个版本中修复此问题。
...
- 修复了更新因符号链接错误而失败的错误。修复 [docker/for-win#12650](https://github.com/docker/for-win/issues/12650)。
...
- 对于新 Kubernetes 集群使用 [cri-dockerd](https://github.com/Mirantis/cri-dockerd) 代替 `dockershim`。此更改对用户来说是透明的，Kubernetes 容器像以前一样在 Docker 引擎上运行。`cri-dockerd` 允许 Kubernetes 使用标准的[容器运行时接口](https://github.com/kubernetes/cri-api#readme) 管理 Docker 容器，这是用于控制其他容器运行时的相同接口。有关更多信息，请参阅 [Dockershim 的未来是 cri-dockerd](https://www.mirantis.com/blog/the-future-of-dockershim-is-cri-dockerd/)。
...
- 修复了注册表访问管理策略在失败后永远不会刷新的错误。
- 日志和终端在 UI 中现在尊重您的 OS 主题（浅色和深色模式）。
- 通过多选复选框轻松清理多个卷。
- 改进了登录反馈。
...
- 修复了在使用 Hyperkit 时，从睡眠状态唤醒后 gettimeofday() 性能下降的问题。修复 [docker/for-mac#3455](https://github.com/docker/for-mac/issues/3455)。
...
- 防止 vpnkit-forwarder 旋转填充日志和错误消息。
- 修复了未设置 HTTP 代理时诊断上传的问题。修复 [docker/for-mac#6234](https://github.com/docker/for-mac/issues/6234)。
- 从自诊断中移除了虚假的 "vm is not running" 错误。修复 [docker/for-mac#6233](https://github.com/docker/for-mac/issues/6233)。
...
- Docker Desktop 仪表板卷管理功能现在提供使用多选复选框高效清理卷的能力。

#### 对于 Mac

- Docker Desktop 4.6.0 为 macOS 用户提供了启用名为 VirtioFS 的新实验性文件共享技术的选项。在测试期间，VirtioFS 已被证明可以显著减少主机和 VM 之间同步更改所需的时间，从而带来显著的性能改进。有关更多信息，请参阅 [VirtioFS](/manuals/desktop/settings-and-maintenance/settings.md#beta-features)。
...
- 修复了 [CVE-2022-0847](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-0847)，又名“Dirty Pipe”，一个可能允许攻击者从容器内部修改主机上容器镜像中的文件的问题。
  如果使用 WSL 2 后端，您必须通过运行 `wsl --update` 更新 WSL 2。

#### 对于 Windows

- 修复了 [CVE-2022-26659](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-26659)，该问题可能允许攻击者在安装或更新 Docker Desktop 期间覆盖系统上任何管理员可写文件。
...
- 修复了在设置了 HTTPS 代理时上传诊断的问题。
- 使从系统托盘菜单检查更新打开软件更新设置部分。

#### 对于 Mac

- 修复了启动 Docker Desktop 后系统托盘菜单未显示所有菜单项的问题。修复 [docker/for-mac#6192](https://github.com/docker/for-mac/issues/6192)。
- 修复了 Docker Desktop 不再在后台启动的回归问题。修复 [docker/for-mac#6167](https://github.com/docker/for-mac/issues/6167)。
- 修复了缺少 Docker Desktop Dock 图标的问题。修复 [docker/for-mac#6173](https://github.com/docker/for-mac/issues/6173)。
- 使用实验性 `virtualization.framework` 时，加快了块设备访问速度。参见[基准测试](https://github.com/docker/roadmap/issues/7#issuecomment-1050626886)。
- 将默认 VM 内存分配增加到物理内存的一半（最小 2 GB，最大 8 GB），以获得更好的开箱即用性能。

#### 对于 Windows

- 修复了 UI 卡在 `starting` 状态的问题，尽管 Docker Desktop 从命令行工作正常。
- 修复了缺少 Docker Desktop 系统托盘图标的问题 [docker/for-win#12573](https://github.com/docker/for-win/issues/12573)
- 修复了在最新 5.10.60.1 内核下使用 WSL 2 时的注册表访问管理问题。
- 修复了在从 WSL 2 环境启动的 Compose 应用程序中选择容器时 UI 崩溃的问题。修复 [docker/for-win#12567](https://github.com/docker/for-win/issues/12567)。
- 修复了从快速入门指南的终端复制文本的问题。修复 [docker/for-win#12444](https://github.com/docker/for-win/issues/12444)。

### 已知问题

#### 对于 Mac

- 启用 VirtioFS 后，使用不同 Unix 用户 ID 运行进程的容器可能会遇到缓存问题。例如，如果以 `root` 身份运行的进程查询文件，而另一个以用户 `nginx` 身份运行的进程立即尝试访问同一文件，`nginx` 进程将收到“权限被拒绝”错误。
...
- Docker Desktop 4.5.0 引入了新版本的 Docker 菜单，在所有操作系统上创建一致的用户体验。有关更多信息，请参阅博客文章 [Docker Desktop 4.5 的新 Docker 菜单和改进的版本亮点](https://www.docker.com/blog/new-docker-menu-improved-release-highlights-with-docker-desktop-4-5/)
...
- 修复了 [CVE-2021-44719](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2021-44719)，该问题允许 Docker Desktop 用于从容器访问主机上的任何用户文件，绕过允许的共享文件夹列表。

#### 对于 Windows

- 修复了 [CVE-2022-23774](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-23774)，该问题允许 Docker Desktop 允许攻击者移动任意文件。
...
- 修复了如果应用程序在不同版本的 Docker Compose 中启动，则无法使用 Docker Desktop 仪表板停止 Docker Compose 应用程序的问题。例如，如果用户在 V1 中启动了 Docker Compose 应用程序，然后切换到 Docker Compose V2，则尝试停止 Docker Compose 应用程序将失败。
- 修复了在用户退出 Docker Desktop 并启动应用程序后，Docker Desktop 错误地提示用户登录的问题。
- 修复了 **关于 Docker Desktop** 窗口不再工作的问题。
- 在 Mac M1 上将 CPU 数量限制为 8，以修复启动问题。修复 [docker/for-mac#6063](https://github.com/docker/for-mac/issues/6063)。

#### 对于 Windows

- 修复了使用版本 2 启动的 Compose 应用程序的问题，但仪表板仅处理版本 1。

### 已知问题

#### 对于 Windows

从头安装 Docker Desktop 4.5.0 有一个错误，默认情况下 Docker Desktop 使用 Hyper-V 后端而不是 WSL 2。这意味着 Windows Home 用户将无法启动 Docker Desktop，因为 WSL 2 是唯一受支持的后端。要解决此问题，您必须从计算机中卸载 4.5.0，然后下载并安装 Docker Desktop 4.5.1 或更高版本。或者，您可以编辑位于 `%APPDATA%\Docker\settings.json` 的 Docker Desktop settings.json 文件，并将 `wslEngineEnabled` 字段的值手动切换为 `true`。
...
- 在浏览器中登录后单击 **继续到桌面** 有时不会将仪表板置于前台。
- 登录后，当仪表板获得焦点时，即使单击后台窗口，它有时也会保持在前台。作为解决方法，您需要在单击另一个应用程序窗口之前单击仪表板。
...
- 禁用了仪表板快捷键，以防止在最小化或未聚焦时捕获它们。修复 [docker/for-win#12495](https://github.com/docker/for-win/issues/12495)。
...
- 使用 Auth0 和单点登录轻松、安全地登录
  - 单点登录：拥有 Docker Business 订阅的用户现在可以配置 SSO，以使用其身份提供商 (IdP) 进行身份验证来访问 Docker。有关更多信息，请参阅[单点登录](/manuals/enterprise/security/single-sign-on/_index.md)。
  - 登录 Docker Desktop 现在会引导您通过浏览器，因此您可以获得密码管理器自动填充的所有好处。
...
- 修复了 [CVE-2021-45449](../security/_index.md#cve-2021-45449)，该问题影响当前在 Docker Desktop 版本 4.3.0 或 4.3.1 上的用户。

Docker Desktop 版本 4.3.0 和 4.3.1 有一个错误，可能会在登录期间将敏感信息（访问令牌或密码）记录在用户机器上。
...
- 如果 `registry.json` 在 `allowedOrgs` 字段中包含多个组织，Docker Desktop 会显示错误。如果您为不同的开发人员组使用多个组织，则必须为每个组提供单独的 `registry.json` 文件。
- 修复了 Compose 中的一个回归问题，该问题将容器名称分隔符从 `-` 恢复为 `_`。修复 [docker/compose-switch](https://github.com/docker/compose-switch/issues/24)。

#### 对于 Mac

- 修复了仪表板中容器的内存统计信息。修复 [docker/for-mac/#4774](https://github.com/docker/for-mac/issues/6076)。
- 在 `settings.json` 中添加了一个已弃用的选项：`"deprecatedCgroupv1": true`，它将 Linux 环境切换回 cgroups v1。如果您的软件需要 cgroups v1，您应该更新它以与 cgroups v2 兼容。尽管 cgroups v1 应该继续工作，但某些未来功能可能会依赖于 cgroups v2。同样，某些 Linux 内核错误可能只会在 cgroups v2 中修复。
- 修复了在暂停 Docker Desktop 后将计算机置于睡眠模式会导致 Docker Desktop 在计算机退出睡眠模式后无法从暂停状态恢复的问题。修复 [for-mac#6058](https://github.com/docker/for-mac/issues/6058)。
...
- 在浏览器中登录后单击 **继续到桌面** 有时不会将仪表板置于前台。
- 登录后，当仪表板获得焦点时，即使单击后台窗口，它有时也会保持在前台。作为解决方法，您需要在单击另一个应用程序窗口之前单击仪表板。
- 当仪表板打开时，即使它没有焦点或被最小化，它仍然会捕获键盘快捷键（例如 ctrl-r 用于重启）
...
Docker Desktop 4.3.0 及更早版本中的旧版 `docker scan` 无法检测 [Log4j 2
...
- 添加了自诊断警告，如果主机缺少 Internet 连接。
- 修复了阻止用户使用卷 UI 中的“另存为”选项从卷保存文件的问题。修复 [docker/for-win#12407](https://github.com/docker/for-win/issues/12407)。
...
- Mac 版 Docker Desktop 不再需要 Rosetta 2，除了[三个可选的命令行工具
