...
- 修复了支持诊断工具无意中捕获了过期的 Docker Hub 授权持有者令牌 (bearer tokens) 的问题。
...
- 添加了一个新的 Docker Model Runner 命令。使用 `docker model purge` 可以删除所有模型。
...
- Ask Gordon 不再显示带有用户提供的 URL 的镜像。
...
- 修复了 [CVE-2025-9164](https://www.cve.org/cverecord?id=CVE-2025-9164)，其中 Docker Desktop for Windows 安装程序由于不安全的 DLL 搜索顺序而容易受到 DLL 劫持。安装程序在检查系统目录之前，会在用户的下载文件夹中搜索所需的 DLL，从而允许通过恶意 DLL 放置进行本地提权。

### 新功能

- [cagent](/manuals/ai/cagent/_index.md) 现在可通过 Docker Desktop 使用。
...
- 修复了在等待用户输入新密码时 Docker Desktop 使用过期的代理密码的问题。
- 修复了使用 Docker Debug 启动时显示的 'chown' 错误。
...
- 修复了当 Desktop 重启时 kind 集群状态有时会重置的问题。修复 [docker/for-mac#77445](https://github.com/docker/for-mac/issues/7745)。
- 删除了过时的 `mcp` 密钥，以符合最新的 VS Code MCP 服务器更改。
- 更新凭证助手至 [v0.9.4](https://github.com/docker/docker-credential-helpers/releases/tag/v0.9.4)。
- 修复了在等待用户输入新密码时 Docker Desktop 使用过期的代理密码的问题。
- 修复了在某些条件下 Docker Desktop 会定期使用 Docker CLI 工具创建新进程的错误。修复 [docker/for-win#14944](https://github.com/docker/for-win/issues/14944)。
- 修复了通过 Compose 使用 Docker Model Runner 时模型未配置为嵌入的错误。要指定模型应配置为嵌入，必须显式添加 `--embeddings` 运行时标志，如[在 Docker Compose 中使用 AI 模型](https://docs.docker.com/ai/compose/models-and-compose/#model-configuration-options)中所述。修复 [docker/model-runner#166](https://github.com/docker/model-runner/issues/166)。

#### For Windows

- 删除了 `HKLM\SOFTWARE\Docker Inc.\Docker\1.0` 注册表项。改为在路径中查找 `docker.exe` 以确定 Docker Desktop 的安装位置。
...
- 修复了 [CVE-2025-10657](https://www.cve.org/CVERecord?id=CVE-2025-10657)，其中增强容器隔离的 [Docker Socket 命令限制](../enterprise/security/hardened-desktop/enhanced-container-isolation/config.md#command-restrictions) 功能仅在 Docker Desktop 4.46.0 中无法正常工作（其配置被忽略）。

### 新功能

- 为 Docker 的 MCP 目录添加了动态 MCP 服务器发现和支持。
- 使用增强容器隔离时，管理员现在可以阻止在挂载了 Docker 套接字的容器中运行 `docker plugin` 和 `docker login` 命令。
...
- 现在可以通过过滤器、排序和改进的搜索功能更轻松地搜索 MCP 服务器。
- Docker Debug 在调试环境变量设置为空值的容器时不再挂起。
- 增强了 Docker Model Runner，CLI 中具有丰富的响应渲染，Docker Desktop 仪表板中具有对话上下文，以及可恢复的下载。

#### For Mac

- 删除了 `com.apple.security.cs.allow-dyld-environment-variables` 权限，该权限允许通过 `DYLD_INSERT_LIBRARIES` 环境变量将签名的任意动态库加载到 Docker Desktop。
- 修复了配置描述文件登录强制执行在某些客户环境中失效的回归问题。
- 修复了在写入本地内容存储（不带 `--push` 标志）时 `docker model package` 命令有时会挂起的错误。
- 修复了使用重启策略 `unless-stopped` 启动的容器永远不会重启的错误。修复 [docker/for-mac#7744](https://github.com/docker/for-mac/issues/7744)。

#### For Windows

- 修复了 Windows 上 Docker MCP Toolkit 的 Goose MCP 客户端连接。
...
- 通过启用 `llama.cpp` 推理进程的沙箱，提高了 Docker Model Runner 的安全性。
- 修复了导致 Docker Desktop 启动缓慢并显示冻结的错误。修复 [docker/for-mac#7671](https://github.com/docker/for-mac/issues/7671)。
...
- 修复了 VM 有时会因错误 **The virtual machine stopped unexpectedly.** 而失败的问题。
- 修复了在容器启动后连接或断开与网络的连接时会破坏端口映射的错误。修复 [docker/for-mac#7693](https://github.com/docker/for-mac/issues/7693)。
...
- 修复了 [CVE-2025-9074](https://www.cve.org/CVERecord?id=CVE-2025-9074)，其中在 Docker Desktop 上运行的恶意容器可以访问 Docker Engine 并启动其他容器，而无需挂载 Docker 套接字。这可能允许未经授权访问主机系统上的用户文件。增强容器隔离 (ECI) 无法缓解此漏洞。
...
- 修复了在 4.44.0 版本中发现的问题，该问题导致在 Desktop 设置管理中锁定 `vpnkit` CIDR 而未指定值时启动失败。

#### For Windows

- 修复了从使用旧版 `version-pack-data` 目录结构的发行版升级后，卷和容器不可见的问题。
- 解决了 WSL 2 中的一个罕见问题，该问题导致 Docker CLI 因 **Proxy Authentication Required** 错误而失败。
...
- 现在可以在 **设置** 中搜索特定的配置选项。
- Apple 虚拟化 (Apple Virtualization) 现在是默认的 VMM，以获得更好的性能，并且移除了 QEMU 虚拟化。请参阅[博客文章](https://www.docker.com/blog/docker-desktop-for-mac-qemu-virtualization-option-to-be-deprecated-in-90-days/)。
...
我们知道 [CVE-2025-23266](https://nvd.nist.gov/vuln/detail/CVE-2025-23266)，这是一个影响 CDI 模式下 NVIDIA Container Toolkit（最高版本 1.17.7）的关键漏洞。Docker Desktop 包含版本 1.17.8，不受影响。但是，捆绑了早期工具包版本的旧版 Docker Desktop 如果手动启用了 CDI 模式，则可能受到影响。请升级到 Docker Desktop 4.44 或更高版本，以确保您使用的是已修补的版本。

### 错误修复和增强功能

#### 所有平台

- 修复了在启用 containerd 镜像存储时拉取带有 zstd 差异层的镜像的问题。
- 修复了使用 `--restart` 标志启动的容器在使用增强容器隔离时无法正确重启的错误。
- 改进了[自定义注册表镜像](/manuals/desktop/use-desktop/kubernetes.md#configuring-a-custom-image-registry-for-kubernetes-control-plane-images)与增强容器隔离 (ECI) 之间的交互，因此在为 Kubernetes 控制平面镜像使用自定义注册表时，不再需要手动更新 [ECI Docker 套接字镜像列表](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/config.md)。
- 修复了当用户需要登录但当前已注销时，kind 模式下的 Docker Desktop Kubernetes 集群在重启 Docker Desktop 后无法启动的错误。
- 修复了当[增强容器隔离](/enterprise/security/hardened-desktop/enhanced-container-isolation/) 启用时，阻止将 MCP 秘密挂载到容器中的错误。
...
- 通过重新引入常规使用性能提升，修复了自 4.42.0 以来在 DockerVMM 上的回归问题。
...
- 修复了在容器启动后连接或断开与网络的连接时会破坏端口映射的错误。修复 [docker/for-mac#7693](https://github.com/docker/for-mac/issues/7693#issuecomment-3131427879)。
...
- 重新添加了 `docker-users` 组到命名管道安全描述符中。
...
- 修复了在使用带有 `cgroups v1` 的 WSL 并启用增强容器隔离 (ECI) 时，阻止 Docker Desktop Kubernetes 在 kind 模式下启动的错误。
...
- 修复了 [CVE-2025-6587](https://www.cve.org/CVERecord?id=CVE-2025-6587)，其中敏感的系统环境变量被包含在 Docker Desktop 诊断日志中，可能导致潜在的密钥泄露。
...
- 修复了在 `fsck` 期间关闭 Docker 时磁盘会损坏的错误。
- 修复了在使用 `kind` Kubernetes 集群时导致 WSL2 中 `~/.kube/config` 不正确的错误。
- 如果 Docker Desktop 已被手动暂停，则向 Docker API / `docker` CLI 命令返回显式错误。
...
- `docker buildx bake` 将不会构建具有顶层 models 属性的 Compose 文件中的镜像。请改用 `docker compose build`。
...
- Docker Desktop 的“主机网络”功能与最新的 WSL 2 Linux 内核之间可能存在兼容性问题。如果您遇到此类问题，请将 WSL 2 降级到 2.5.7。
...
- 在 WSL 2 中禁用了资源节省模式，以防止 `docker` CLI 命令在 WSL 2 发行版中挂起。[docker/for-win#14656](https://github.com/docker/for-win/issues/14656#issuecomment-2960285463)
...
- 为团队订阅用户修复了[强制登录](/manuals/enterprise/security/enforce-sign-in/_index.md)。
...
- 在 Docker Desktop Linux VM 中禁用了内存保护密钥机制。这导致 VS Code Dev Containers 无法正常工作。请参阅 [docker/for-mac#7667](https://github.com/docker/for-mac/issues/7667)。
...
- 修复了在 WSL 2 启动时导致 `The network name cannot be found` 错误的错误。修复 [docker/for-win#14714](https://github.com/docker/for-win/issues/14714)。
- 修复了在卸载时 Docker Desktop 不会删除 hosts 文件中的条目的问题。
- 修复了在某些系统语言下读取自动启动注册表项时的问题。修复 [docker/for-win#14731](https://github.com/docker/for-win/issues/14731)。
...
- 修复了如果另一个 WSL 发行版仍在使用 Linux cgroups v1，Docker Desktop 在 WSL 2.5.7 上无法启动的错误。修复 [docker/for-win#14801](https://github.com/docker/for-win/issues/14801)
...
- 运行带有 Wasm 的容器会间歇性挂起。请参阅 [docker/for-mac#7666](https://github.com/docker/for-mac/issues/7666)。
...
- 修复了即使 Docker Model Runner 不受支持或未启用，GUI 中仍显示 `Models` 菜单的问题。
...
- Docker Model Runner 现在可用于带有 NVIDIA GPU 的 x86 Windows 机器。
...
- 修复了 [CVE-2025-3224](https://www.cve.org/CVERecord?id=CVE-2025-3224)，该漏洞允许获得用户机器访问权限的攻击者在 Docker Desktop 更新时执行提权。
- 修复了 [CVE-2025-4095](https://www.cve.org/CVERecord?id=CVE-2025-4095)，其中当使用 MacOS 配置描述文件时，注册表访问管理 (RAM) 策略未强制执行，允许用户从未批准的注册表拉取镜像。
...
- 修复了在 Mac、Windows Hyper-V 或 Linux 上启用[增强容器隔离](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/_index.md) 时容器访问 `/proc/sys/kernel/shm*` sysctls 的问题。
...
- 如果您使用 `desktop.plist`（在 macOS 上）或注册表项（在 Windows 上）强制执行登录，并且还有 `registry.json`，如果用户属于 `desktop.plist`/注册表项中列出的组织，但不属于 `registry.json` 中指定的任何组织，则登录将失败。要解决此问题，请删除 `registry.json` 文件。

#### For Windows

- 如果在 Windows 注册表项 `allowedOrgs` 中使用空格分隔格式指定了多个组织，登录将失败，用户将被注销。作为解决方法，请在注册表项值中每行指定一个组织。
...
- 您现在可以直接在 Docker Desktop 中使用 [Docker Model Runner (Beta)](/manuals/ai/model-runner.md) 从 Docker Hub 拉取、运行和管理 AI 模型。目前适用于带有 Apple Silicon 的 Docker Desktop for Mac。
...
- 为 Mac 和 Linux 移除了未使用的内部辅助工具 `com.docker.admin`。
...
- 在显示特权助手错误消息时切换到 Windows 容器可能会导致状态不一致。作为解决方法，请退出 Docker Desktop，在 `settings-store.json` 中将 `UseWindowsContainers` 更改为 `false`，然后重新启动 Docker Desktop。
...
- 修复了导致 CLI 和 Docker Desktop 以外的客户端在存在带有端口映射的容器时出现 3 秒延迟的问题。请参阅 [docker/for-mac#7575](https://github.com/docker/for-mac/issues/7575)
- 修复了 ECI Docker 套接字权限中的错误，该错误有时会阻止在允许的镜像或从允许的镜像派生的镜像的容器上挂载 Docker 套接字。
...
<em class="text-gray-400 italic dark:text-gray-500">2025-01-30</em>
...
- 修复了当[增强容器隔离](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/_index.md) 启用时，使用 `curl` 通过 Docker API 创建容器失败的错误。
- 修复了在刷新期过后 RAM 策略未刷新的错误。
- 修复了增强容器隔离中的一个错误，该错误发生在将 Docker 套接字挂载到容器中，然后从该容器内部使用绑定挂载创建 Docker 容器时。
...
- 添加了对 gvisor 网络的支持。使用不兼容版本 qemu (8.x) 的用户将保留在 qemu 网络上，其他用户将自动迁移。
...
- 如果您看到关于 `com.docker.vmnetd` 或 `com.docker.socket` 的恶意软件安全弹出窗口，请按照 [docker/for-mac#7527](https://github.com/docker/for-mac/issues/7527) 中记录的步骤操作。

## 4.37.1

<em class="text-gray-400 italic dark:text-gray-500">2024-12-17</em>
...
<em class="text-gray-400 italic dark:text-gray-500">2024-12-12</em>
...
- 修复了在资源节省模式下（仅限带有 WSL2 后端的 Windows）或在切换引擎后（macOS），重启引擎后 Docker Desktop 仪表板与 Docker 守护进程不同步的错误。
...
- 修复了当使用 `sudo` 两次运行卸载程序二进制文件时，会以 root 权限创建某些用户目录的错误。

#### For Windows

- 添加了对在 WSL 2 单发行版模式下使用 WSL 2 版本 2.3.24 及更高版本的 Windows on ARM 的支持。
...
- 在 Apple Silicon 上作为 VMM 的 QEMU（旧版）将在未来版本中被移除。建议您切换到 Apple 虚拟化框架以提高性能和稳定性。如果您遇到问题，请[联系 Docker 支持](https://www.docker.com/support/)或[提交 GitHub 问题](https://github.com/docker/for-mac/issues)。
- osxfs（旧版）将在未来版本中被移除。建议您切换到 VirtioFS 以提高性能。如果您遇到问题，请[联系 Docker 支持](https://www.docker.com/support/)或[提交 GitHub 问题](https://github.com/docker/for-mac/issues)。
...
<em class="text-gray-400 italic dark:text-gray-500">2024-11-18</em>
...
- 同步文件共享现在遵守 `admin-settings.json` 中的 `filesharingAllowedDirectories` 设置。
...
- Docker Desktop 在手动暂停后不会自动取消暂停。系统将保持暂停状态，直到您手动恢复 Docker 引擎。这修复了其他软件通过在后台运行 CLI 命令意外触发恢复的错误。修复 [for-mac/#6908](https://github.com/docker/for-mac/issues/6908)
- 构建 UI：
  - **源** 选项卡现在支持多个源文件。
...
- 修复了 Docker VMM 中的一个错误，该错误阻止了 MySQL 和其他数据库容器启动。修复来自 [docker/for-mac#7464](https://github.com/docker/for-mac/issues/7464) 的报告。
- 最低内存要求现在会针对 Docker VMM 自动调整，改善了用户体验并解决了来自 [docker/for-mac#7464](https://github.com/docker/for-mac/issues/7464) 和 [docker/for-mac#7482](https://github.com/docker/for-mac/issues/7482) 的报告。
...
- 修复了如果 Docker Desktop 由非管理员用户安装，或者当前用户以前是管理员，应用内更新会失败的错误。修复 [for-mac/#7403](https://github.com/docker/for-mac/issues/7403) 和 [for-mac/#6920](https://github.com/docker/for-mac/issues/6920)
...
<em class="text-gray-400 italic dark:text-gray-500">2024-10-24</em>


### 新功能

- 对[在 Red Hat Enterprise Linux 上使用 Docker Desktop](/manuals/desktop/setup/install/linux/rhel.md) 的支持现已正式可用。
- 卷备份和共享现已正式可用，可在 **卷** 视图中找到。
- 使用系统 shell 在 Docker Desktop 内进行终端支持现已正式可用。
...
- 移除了 [docker-index](https://github.com/docker/index-cli-plugin)，因为它现在已弃用，您可以使用 `docker scout cves fs://<path to binary>` 代替。
...
- 修复了在 **镜像** 视图中选择 **推送到 Docker Hub** 操作会导致 `invalid tag format` 错误的错误。修复 [docker/for-win#14258](https://github.com/docker/for-win/issues/14258)。
- 修复了当 ICMPv6 设置不成功时 Docker Desktop 启动失败的问题。
- 添加了允许 USB/IP 工作的驱动程序。
- 修复了增强容器隔离 (ECI) [派生镜像的 Docker 套接字挂载权限](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/config.md) 中的错误，当 Docker Desktop 使用 containerd 镜像存储时，该错误会错误地拒绝某些镜像的 Docker 套接字挂载。
...
- 修复了在启动时 `~/.docker/cli-plugins` 未被填充的错误。
...
- Docker Desktop for Windows 的新安装现在需要 Windows 版本 19045 或更高版本。
- 修复了如果在 WSL 中通过内核配置或内核命令行禁用了 IPv6 会导致启动失败的问题。修复 [docker/for-win#14240](https://github.com/docker/for-win/issues/14240)
...
- 自版本 4.34.0 起，高级设置中的切换“允许特权端口映射”不起作用。更多信息，请参阅 [docker/for-mac#7460](https://github.com/docker/for-mac/issues/7460)。
...
- 修复了 [CVE-2024-9348](https://www.cve.org/cverecord?id=CVE-2024-9348)，该漏洞允许通过精心制作的扩展描述/变更日志进行 RCE。
...
- 修复了 Docker Desktop 无法启动（通常在首次启动时）错误地认为另一个应用程序实例正在运行的错误。([docker/for-win#14294](https://github.com/docker/for-win/issues/14294) 和 [docker/for-win#14034](https://github.com/docker/for-win/issues/14034))。

## 4.34.0

<em class="text-gray-400 italic dark:text-gray-500">2024-08-29</em>


### 新功能

- Docker Desktop 上的[主机网络](/manuals/engine/network/drivers/host.md#docker-desktop) 支持现已正式可用。
...
- 修复了当容器以 AutoRemove (`--rm`) 启动但其端口绑定被 Docker Desktop 在启动时拒绝时，CLI 会变为空闲的错误。
- 修复了在 **支持** 屏幕上诊断收集会随机失败的错误。
- 修复了容器的 **文件** 选项卡中文件夹无法展开的错误。修复 [docker/for-win#14204](https://github.com/docker/for-win/issues/14204)。
- 应用内更新现在尊重代理设置。
- 将 ECI Docker 套接字挂载权限功能扩展为可选择地允许来自允许镜像的子镜像。这允许 ECI 与构建包（例如 Paketo）配合使用，这些构建包创建使用 Docker 套接字挂载的临时本地镜像。
...
- 修复了当 Docker Desktop 作为 `User` 安装时，配置完整性检查功能会报告系统路径而不是用户路径的问题。
- 修复了尝试从绑定挂载卷读取扩展属性的应用程序可能会遇到失败的错误。修复 [docker/for-mac#7377](https://github.com/docker/for-mac/issues/7377)。

#### For Windows

- 修复了当用户的意图是保持 `credsStore` 为空时，Docker Desktop 会将 docker 的 `credsStore` 重置为 `desktop` 的错误。修复 [docker/for-win#9843](https://github.com/docker/for-win/issues/9843)。
...
- 即使 Kubernetes 正在运行并且 Compose Bridge 已启用，Compose 文件查看器中的 **转换和部署** 按钮也可能被禁用。解决方法是在 **实验性** 设置选项卡中禁用 Compose Bridge，使用 **应用并重启** 应用更改，然后重新启用并再次选择 **应用并重启**。
- 在 Docker CLI 中针对注册表进行身份验证时存在一个已知问题 (`docker login [registry address]`)，如果提供的注册表地址包含存储库/镜像名称（例如 `docker login index.docker.io/docker/welcome-to-docker`），则存储库部分 (`docker/welcome-to-docker`) 不会被规范化，导致凭据存储不正确，这会导致后续从注册表拉取 (`docker pull index.docker.io/docker/welcome-to-docker`) 时无法进行身份验证。为防止这种情况，在运行 `docker login` 时不要在注册表地址中包含任何额外的后缀。
  > [!NOTE]
  > 使用包含 URL 路径段的地址进行 `docker login` 不是文档化的用例，被视为不受支持。推荐的用法是仅指定注册表主机名，以及可选的端口，作为 `docker login` 的地址。
- 当运行 `docker compose up` 且 Docker Desktop 处于资源节省模式时，该命令无响应。作为解决方法，请手动退出资源节省模式，Docker Compose 将再次变得有响应。
- 当[增强容器隔离 (ECI)](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/_index.md) 启用时，Docker Desktop 可能无法进入资源节省模式。这将在未来的 Docker Desktop 版本中修复。
...
<em class="text-gray-400 italic dark:text-gray-500">2024-07-31</em>
...
- BuildKit 现在会评估 Dockerfile 规则以通知您潜在问题。
...
- 配置完整性检查功能现在提供有关您的 Docker Desktop 配置已更改内容的更多上下文。更多信息，请参阅 [FAQs](/manuals/desktop/troubleshoot-and-support/faqs/macfaqs.md)。
- 配置完整性检查功能在修复 Docker Desktop 失败时显示错误。
- 修复了 IPv6 TCP 被设置为 `host.docker.internal` 的错误。修复 [docker/for-mac#7332](https://github.com/docker/for-mac/issues/7332)。
- 修复了 `docker-compose` 符号链接指向空位置的问题。修复 [docker/for-mac#7345](https://github.com/docker/for-mac/issues/7345)。

#### For Linux

- 修复了卸载后某些 `wincred` 值仍保留的问题。由 Javier Yong [@Javiery3889](https://github.com/Javiery3889) 报告。
...
<em class="text-gray-400 italic dark:text-gray-500">2024-07-04</em>
...
- [设置管理](/manuals/enterprise/security/hardened-desktop/settings-management/_index.md) 现在让管理员可以将默认值设置为 `ProxyEnableKerberosNTLM`。
- 移除了用于旧版 Visual Studio Code 的临时兼容性修复。
- 构建视图：
  - 将导入的构建记录的图标更改为“文件”图标。
  - 改进了尝试连接到已连接的 Docker Build Cloud 构建器时的错误消息。
  - 修复了构建记录会意外消失的问题。
  - 修复了阻止用户重新打开[导入的构建](use-desktop/builds.md#import-builds)的问题。
  - 修复了当构建状态从运行更改为已完成时，构建详细信息未显示的问题。
  - 修复了构建详细信息中格式错误的构建源链接。
  - 修复了命名上下文的构建统计信息缺失的问题。
...
- 修复了在 WSL 2 引擎上的一个问题，如果用户手动移动了 `docker-desktop-data` 发行版，Docker Desktop 将无法检测到其存在。
...
- 修复了回归问题，即当 `/var/run/docker.socket` 指向意外路径时，**另一个应用程序更改了您的桌面配置** 警告消息会出现。
- 将配置检查菜单项和横幅更改为通知。
- 改进了绑定挂载上的读写操作性能。
...
- 修复了更新前创建的容器、镜像和卷对用户可能不可见的错误。修复 [docker/for-win#14118](https://github.com/docker/for-win/issues/14118)。
...
- Docker Compose 文件查看器显示您的 Compose YAML，带有语法高亮和指向相关文档的上下文链接（Beta，逐步推出）。
- 新的侧边栏用户体验。
...
- 修复了在 macOS 和带有 Hyper-V 的 Windows 上使用 gRPC FUSE 文件共享时，主机上修改的绑定挂载文件在容器重启后未更新的错误。修复 [docker/for-mac#7274](https://github.com/docker/for-mac/issues/7274), [docker/for-win#14060](https://github.com/docker/for-win/issues/14060)。
- 构建视图：
...
- 默认情况下，只有市场上列出的扩展才能在 Docker Desktop 中安装。这可以在 Docker Desktop 的设置中更改。扩展开发人员需要更改此选项才能测试他们的扩展。

### For Windows

- 修复了 [CVE-2024-5652](https://www.cve.org/cverecord?id=CVE-2024-5652)，其中 `docker-users` 组中的用户可以通过 Windows 容器模式下的 `exec-path` Docker 守护进程配置选项导致 Windows 拒绝服务。此漏洞由 Hashim Jawad ([@ihack4falafel](https://github.com/ihack4falafel)) 与 Trend Micro Zero Day Initiative 合作发现。

### 弃用

#### 所有平台

- 以前作为 `com.docker.cli` 提供的 CLI 二进制文件现在简称为 `docker`。此版本将 CLI 二进制文件保留为 `com.docker.cli`，但将在下一个版本中移除。
...
- 升级到 Docker Desktop 4.31.0 时，使用 Docker Desktop 4.8.0 或更早版本在仅 WSL 的 Windows 主机上创建的现有容器、镜像和卷对用户不可见。数据不会丢失，只是对 Docker Desktop 4.31.0 不可见。如果受到影响，请降级到 4.30 或更早版本。有关更多信息，请参阅：[docker/for-win#14118](https://github.com/docker/for-win/issues/14118)。

#### For Linux

- 尚不支持 Ubuntu 24.04 LTS，Docker Desktop 将无法启动。由于最新 Ubuntu 版本限制非特权命名空间的方式更改，需要至少运行一次 `sudo sysctl -w kernel.apparmor_restrict_unprivileged_userns=0`。请参阅 [Ubuntu 博客](https://ubuntu.com/blog/)。

## 4.30.0

<em class="text-gray-400 italic dark:text-gray-500">2024-05-06</em>
...
- 添加了对 Windows 上 [Kerberos 和 NTLM 代理身份验证](/manuals/desktop/settings-and-maintenance/settings.md#proxy-authentication) 的支持。需要商业订阅。
...
- 在无根容器中运行 `docker build` 命令时，改进了增强容器隔离 (ECI) 的安全性。
...
- 修复了自诊断 CLI 中的错误，该错误错误地显示 VM 尚未启动。修复 [docker/for-mac#7241](https://github.com/docker/for-mac/issues/7241)。
...
- 开发环境正在逐步淘汰，并已移至 **开发中功能** 选项卡中的 **Beta** 选项卡。
- 构建视图：
  - 更好的批量删除构建记录。
...
- 修复了在 2019 年之后的 Mac 上使用虚拟化框架时触发分段错误的错误。请参阅 [docker/for-mac#6824](https://github.com/docker/for-mac/issues/6824)。
...
- 如果未安装 Rosetta，修复了阻止 Docker Desktop 启动的错误。修复 [docker/for-mac#7243](https://github.com/docker/for-mac/issues/7243)。

#### For Windows

- 为 WSL2 添加了简化的配置模式，避免了对辅助 `docker-desktop-data` WSL 发行版的需求（实验性）。
- 修复了 WSL 环境中 Docker CLI 的 bash 补全。
- 修复了 Docker Desktop 4.28 中的回归问题，该问题导致在 WSL 上使用 Docker-in-Docker（通过挂载 `/var/run/docker.sock`）时，主机文件绑定挂载到容器中无法在容器内正确显示。
...
- 如果您启用了需要登录才能使用的 Docker Desktop 功能，例如 **主机网络**，您必须保持登录状态才能使用 Docker Desktop。要继续使用 Docker Desktop 或修改这些设置，请确保您已登录。
...
- 以非管理员用户身份启动 Docker Desktop 时，如果用户不是 **docker-users** 组的成员，可能会触发以下错误连接 `ENOENT \\.\pipe\errorReporter`。
  这可以通过将用户添加到 **docker-users** 组来解决。在启动 Docker Desktop 之前，请确保注销并重新登录，并使用 `wsl --unregister docker-desktop` 注销（如果已创建）`docker-desktop` 发行版。

#### For Linux

- 尚不支持 Ubuntu 24.04 LTS，Docker Desktop 将无法启动。由于最新 Ubuntu 版本限制非特权命名空间的方式更改，需要至少运行一次 `sudo sysctl -w kernel.apparmor_restrict_unprivileged_userns=0`。有关更多详细信息，请参阅 [Ubuntu 博客](https://ubuntu.com/blog/ubuntu-23-10-restricted-unprivileged-user-namespaces)。

## 4.29.0

<em class="text-gray-400 italic dark:text-gray-500">2024-04-08</em>
...
- 共享云现在可以从仪表板右侧的页脚访问。
- 添加了对 macOS、Windows 和 Docker Desktop for Linux 上主机网络的 Beta 支持 [docker#238](https://github.com/docker/roadmap/issues/238)。
- 向新未读通知添加了时间戳。
...
- 修复了浏览器无法将身份验证信息发送回 Docker Desktop 的错误。修复 [docker/for-mac/issues#7160](https://github.com/docker/for-mac/issues/7160)。
...
- 修复了 [CVE-2024-6222](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2024-6222)，该漏洞允许通过容器逃逸获得 Docker Desktop VM 访问权限的攻击者通过传递扩展和仪表板相关的 IPC 消息进一步逃逸到主机。由 Billy Jheng Bing-Jhong、Đỗ Minh Tuấn、Muhammad Alifa Ramdhan 与 Trend Micro Zero Day Initiative 合作报告。
...
<em class="text-gray-400 italic dark:text-gray-500">2024-02-26</em>
...
- 重新添加了 `CONFIG_BLK_DEV_DM` 内核配置用于 `kpartx`。修复 [docker/for-mac#7197](https://github.com/docker/for-mac/issues/7197)。
- 允许通过代理自动配置 `pac file` 设置 `SOCKS` 代理。
...
- 修复了 Rosetta 下的 `httpd` 问题。[docker/for-mac#7182](https://github.com/docker/for-mac/issues/7182)
- 修复了导致 `virtualization.framework` 崩溃的错误。修复 [docker/for-mac#7024](https://github.com/docker/for-mac/issues/7024)
...
<em class="text-gray-400 italic dark:text-gray-500">2024-02-08</em>
...
- 修复了即使禁用了 **在 Docker Desktop 启动时打开 Docker 仪表板** 选项，仪表板在启动时也会显示的错误。修复 [docker/for-win#13887](https://github.com/docker/for-win/issues/13887)。
...
<em class="text-gray-400 italic dark:text-gray-500">2024-02-01</em>
...
- [同步文件共享](/manuals/desktop/features/synchronized-file-sharing.md) 在 Docker Desktop 内提供快速灵活的主机到 VM 文件共享。利用 [Docker 收购 Mutagen](https://www.docker.com/blog/mutagen-acquisition/) 背后的技术，此功能提供了一种替代虚拟绑定挂载的方法，它使用同步文件系统缓存，为处理大型代码库的开发人员提高了性能。
...
- 修复了当选择 **始终下载更新** 复选框时，自动更新不会下载的错误。
...
- 容器使用图表已移至垂直的 **资源使用** 侧面板，以在容器列表中留出更多空间。通过 **显示图表** 按钮访问使用图表保持不变。
...
- 修复了在某些语言环境下无法将 UTF-16 字符串编码为 UTF-8 的错误。修复 [docker/for-win#13868](https://github.com/docker/for-win/issues/13868)。
- 修复了在应用程序重启时，使用 WSL 集成时凭据存储配置会重置的错误。修复 [docker/for-win#13529](https://github.com/docker/for-win/issues/13529)。
- 修复了阻止正确的 WSL 引擎错误传播给用户的问题。
- 修复了在退出 Windows 容器模式时导致 Docker Desktop 挂起的错误。

### 安全

#### For Windows

- 缓解了 Docker Desktop 安装程序在 Windows 上的多个 DLL 侧加载漏洞，由 Suman Kumar Chakraborty ([@Hijack-Everything](https://github.com/Hijack-Everything)) 报告。
...
<em class="text-gray-400 italic dark:text-gray-500">2023-12-14</em>
...
<em class="text-gray-400 italic dark:text-gray-500">2023-12-04</em>


### 新功能

- 管理员现在可以使用[设置管理](/manuals/enterprise/security/hardened-desktop/settings-management/_index.md)控制 **开发中功能** 选项卡中 Beta 和实验性功能的访问权限。
- 在页脚引入了四种新的版本更新状态。
...
- 修复了同时启用 containerd 镜像存储和 Wasm 时不会启用 Wasm 的错误。
- containerd 集成：
...
- 修复了增强容器隔离模式下的错误，其中 `docker run --init` 失败。
- 修复了当用户开始下载新版本的 Docker Desktop 后，提示用户下载新版本的通知仍然可见的错误。
- 添加了一个通知，指示 Docker Desktop 何时正在安装新版本。
- 修复了当用户将鼠标悬停在没有调用操作的通知上时，光标更改为指针的错误。

#### For Mac

- 修复了 Rosetta 无法与 PHP 配合使用的问题。修复 [docker/for-mac#6773](https://github.com/docker/for-mac/issues/6773) 和 [docker/for-mac#7037](https://github.com/docker/for-mac/issues/7037)。
- 修复了 Rosetta 不工作的几个问题。修复 [[docker/for-mac#6973](https://github.com/docker/for-mac/issues/6973), [[docker/for-mac#7009](https://github.com/docker/for-mac/issues/7009), [[docker/for-mac#7068](https://github.com/docker/for-mac/issues/7068) 和 [[docker/for-mac#7075](https://github.com/docker/for-mac/issues/7075)
- 改进了 NodeJS 在 Rosetta 下的性能。
- 修复了 **无法打开 /proc/self/exe** Rosetta 错误。
- 修复了 **在登录时启动 Docker Desktop** 设置不起作用的错误。修复 [docker/for-mac#7052](https://github.com/docker/for-mac/issues/7052)。
- 您现在可以通过 UI 启用对 UDP 使用内核网络路径。修复 [docker/for-mac#7008](https://github.com/docker/for-mac/issues/7008)。
- 修复了 `uninstall` CLI 工具缺失的回归问题。
- 解决了当通过设置管理禁用分析时导致 Docker Desktop 变得无响应的问题。
...
<em class="text-gray-400 italic dark:text-gray-500">2023-11-21</em>
...
- 修复了在 WSL 2 上的 Docker Desktop 在空闲时会意外关闭 dockerd 的错误。修复 [docker/for-win#13789](https://github.com/docker/for-win/issues/13789)

## 4.25.1

<em class="text-gray-400 italic dark:text-gray-500">2023-11-13</em>
...
<em class="text-gray-400 italic dark:text-gray-500">2023-10-26</em>
...
- Docker Desktop 现在检测 WSL 版本是否过时。如果检测到过时的 WSL 版本，您可以允许 Docker Desktop 自动更新安装，或者您可以在 Docker Desktop 外手动更新 WSL。
- Docker Desktop for Windows 的新安装现在需要 Windows 版本 19044 或更高版本。
...
- containerd 集成：
  ...
  - 修复了使用经典构建器构建的镜像的 `docker history` 不显示中间镜像 ID 的问题。
  - 修复了 `docker load` 无法从压缩的 tar 存档加载镜像的问题。
  - 修复了注册表镜像不起作用的问题。
  - 修复了在为同一容器多次并发调用 `docker diff` 时无法正常工作的问题。
  - 修复了 `docker push` 在将层推送到同一注册表上的不同存储库时未重用层的问题。
- Docker Init：
  - 修复了生成的文件中包含的过时 Docker 文档链接
  - 添加了对 ASP.NET Core 8（除了 6 和 7 之外）的支持
- 修复了安装 Wasm shim 时导致失败的错误。
- 修复了 Docker Desktop 每 15 分钟退出[资源节省模式](https://docs.docker.com/desktop/use-desktop/resource-saver/)，或者，如果计时器设置为超过 15 分钟，资源节省模式永远不会启动的错误。
- 将 **启用后台 SBOM 索引** 选项提升到 **常规设置**。

#### For Mac

- 在 macOS 上安装或更新 Docker Desktop 的最低操作系统版本现在是 macOS Monterey（版本 12）或更高版本。
- 如果用户与 `Docker.app` 的所有者不匹配，改进了无法完成更新时的错误消息。修复 [docker/for-mac#7000](https://github.com/docker/for-mac/issues/7000)。
...
- 升级到 MacOS 14 可能导致 Docker Desktop 也更新到最新版本，即使禁用了自动更新选项。
...
<em class="text-gray-400 italic dark:text-gray-500">2023-10-12</em>
...
<em class="text-gray-400 italic dark:text-gray-500">2023-10-04</em>
...
- 修复了 Docker Desktop for Windows 上的错误，即 Docker Desktop 仪表板无法正确显示容器日志。修复 [docker/for-win#13714](https://github.com/docker/for-win/issues/13714)。

## 4.24.0

<em class="text-gray-400 italic dark:text-gray-500">2023-09-28</em>


### 新功能

- 新的通知中心现已对所有用户开放，因此您可以收到有关新版本、安装进度更新等的通知。选择 Docker Desktop 仪表板右下角的铃铛图标以访问通知中心。
- Compose Watch 现已对所有用户可用。有关更多信息，请参阅[使用 Compose Watch](/manuals/compose/how-tos/file-watch.md)。
...
- containerd 集成：
  ...
  - 修复了 `docker images`（不带 `--all`）显示中间层（由旧的经典构建器创建）的问题。
  - 修复了 `docker diff` 包含额外差异的问题。
...
- 修复了在早于 12.5 的 macOS 版本的 Apple Silicon Mac 上的内核恐慌。修复 [docker/for-mac#6975](https://github.com/docker/for-mac/issues/6975)。
...
- 修复了安装程序创建 root 拥有目录的错误。修复 [docker/for-mac#6984](https://github.com/docker/for-mac/issues/6984)。
- 修复了当缺少 `/Library/LaunchDaemons` 时，安装程序在设置 docker 套接字时失败的错误。修复 [docker/for-mac#6967](https://github.com/docker/for-mac/issues/6967)。
- 修复了在 macOS 上将特权端口绑定到非 localhost IP 时出现权限被拒绝的错误。修复 [docker/for-mac#697](https://github.com/docker/for-mac/issues/6977)。
- 修复了在 4.23 中引入的资源泄漏。与 [docker/for-mac#6953](https://github.com/docker/for-mac/issues/6953) 相关。
...
- 修改了 Docker Desktop 资源节省功能，以在 WSL 中没有容器运行时跳过减少内核内存，因为在某些情况下这会导致超时。相反，鼓励用户通过 .wslconfig 文件直接在 WSL 上启用 "autoMemoryReclaim"（自 WSL 1.3.10 起可用）。

### 已知问题

#### For Mac

- 创建端口为 53 的容器会失败并显示错误 `address already in use`。作为解决方法，请通过在 `~/Library/Group Containers/group.com.docker/settings.json` 位置的 `settings.json` 文件中添加 `"kernelForUDP": false` 来停用网络加速。

## 4.23.0

<em class="text-gray-400 italic dark:text-gray-500">2023-09-11</em>
...
- 修复了 **容器列表** 操作按钮被裁剪的错误。
- containerd 镜像存储：
  - 修复了在与某些镜像交互时出现的 `failed to read config content` 错误。
...
- 改进了单流 TCP 吞吐量。
- 恢复了 macOS 的健康检查，如果您的系统发生更改可能会导致运行 Docker 二进制文件出现问题，该检查会通知您。
...
- 修复了当非管理员用户切换到 Windows 容器或在禁用 WSL 并切换到 Hyper-V 引擎后提示输入凭据的错误。
  此问题会在操作系统重新启动后或在 Docker Desktop 冷启动时发生。

### 安全

#### 所有平台

- 修复了 [CVE-2023-5165](https://www.cve.org/cverecord?id=CVE-2023-5165)，该漏洞允许通过调试 shell 绕过增强容器隔离。受影响的功能仅适用于 Docker Business 客户，并假设用户未被授予本地 root 或管理员权限的环境。
- 修复了 [CVE-2023-5166](https://www.cve.org/cverecord?id=CVE-2023-5166)，该漏洞允许通过精心制作的扩展图标 URL 窃取访问令牌。

### 已知问题

- 在 Docker Desktop 上绑定特权端口在 macOS 上不起作用。作为解决方法，您可以使用 `0.0.0.0` 将端口暴露在所有接口上，或使用 `127.0.0.1` 使用 localhost。

## 4.22.1

<em class="text-gray-400 italic dark:text-gray-500">2023-08-24</em>
...
- 缓解了影响 Docker Desktop 启动和资源节省模式的几个问题。[docker/for-mac#6933](https://github.com/docker/for-mac/issues/6933)
...
<em class="text-gray-400 italic dark:text-gray-500">2023-08-03</em>
...
- [资源使用](use-desktop/container.md) 已从实验性转为 GA。
...
- [设置管理](/manuals/enterprise/security/hardened-desktop/settings-management/_index.md) 现在让您可以为组织关闭 Docker 扩展。
...
- 修复了当 vpnkit 预期不运行时，自诊断工具显示误报失败的错误。修复 [docker/for-win#13479](https://github.com/docker/for-win/issues/13479)。
...
<em class="text-gray-400 italic dark:text-gray-500">2023-07-03</em>
...
- 移除了配置健康检查以进行进一步调查并解决特定设置问题。

## 4.21.0

<em class="text-gray-400 italic dark:text-gray-500">2023-06-29</em>
...
- 使用 OpenTelemetry 改进了产品使用报告（实验性）。
...
- 添加了 macOS 的健康检查，如果系统发生更改可能会导致运行 Docker 二进制文件出现问题，该检查会通知用户。

#### For Windows

- 修复了 WSL 2 上的错误，如果 Desktop 被暂停、终止然后重新启动，除非首先使用 `wsl --shutdown` 关闭 WSL，否则启动会挂起。
...
<em class="text-gray-400 italic dark:text-gray-500">2023-06-05</em>
...
- 修复了在非西方语言环境下难以解析主机上 WSL 版本的错误。修复 [docker/for-win#13518](https://github.com/docker/for-win/issues/13518) 和 [docker/for-win#13524](https://github.com/docker/for-win/issues/13524)。

## 4.20.0

<em class="text-gray-400 italic dark:text-gray-500">2023-05-30</em>
...
- [Docker Scout CLI](https://docs.docker.com/scout/#docker-scout-cli) 现在会在未作为参数提供时查找最近构建的镜像。
...
- 修复了在关闭时 Docker Desktop 未停止其 WSL 发行版（docker-desktop 和 docker-desktop-data）的错误，从而不必要地消耗主机内存。
...
<em class="text-gray-400 italic dark:text-gray-500">2023-04-27</em>
...
- 当 `docker-compose` 操作失败时，向仪表板错误添加了更多详细信息 ([docker/for-win#13378](https://github.com/docker/for-win/issues/13378))。
...
- Docker Desktop 现在停止在应用程序启动时覆盖 .docker/config.json `credsStore` 密钥。请注意，如果您使用自定义凭据助手，则 CLI `docker login` 和 `docker logout` 不会影响 UI 是否登录到 Docker。通常，最好通过 UI 登录到 Docker，因为 UI 支持多因素身份验证。
- 添加了关于即将从 Docker Desktop 中移除 Compose V1 的警告。可以使用 `COMPOSE_V1_EOL_SILENT=1` 抑制。
- 在 Compose 配置中，YAML 中的布尔字段应为 `true` 或 `false`。弃用的 YAML 1.1 值（如 “on” 或 “no”）现在会产生警告。
...
- 恢复了在 Windows 上完全修补 etc/hosts（再次包含 `host.docker.internal` 和 `gateway.docker.internal`）。对于 WSL，此行为由 **常规** 选项卡中的新设置控制。修复 [docker/for-win#13388](https://github.com/docker/for-win/issues/13388) 和 [docker/for-win#13398](https://github.com/docker/for-win/issues/13398)。
- 修复了在更新 Docker Desktop 时桌面上出现虚假 `courgette.log` 文件的错误。修复 [docker/for-win#12468](https://github.com/docker/for-win/issues/12468)。
- 修复了“放大”快捷键 (ctrl+=)。修复 [docker/for-win#13392](https://github.com/docker/for-win/issues/13392)。
- 修复了在第二次容器类型切换后托盘菜单未正确更新的错误。修复 [docker/for-win#13379](https://github.com/docker/for-win/issues/13379)。

#### For Mac

- 提高了在 macOS Ventura 及更高版本上使用虚拟化框架时的 VM 网络性能。Docker Desktop for Mac 现在使用 gVisor 代替 VPNKit。要继续使用 VPNKit，请在 `~/Library/Group Containers/group.com.docker/settings.json` 位置的 `settings.json` 文件中添加 `"networkType":"vpnkit"`。
- 修复了在卸载时显示错误窗口的错误。
...
<em class="text-gray-400 italic dark:text-gray-500">2023-04-03</em>
...
- [Docker Engine v20.10.24](https://docs.docker.com/engine/release-notes/20.10/#201024)，其中包含对 [CVE-2023-28841](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2023-28841)、
  [CVE-2023-28840](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2023-28840) 和
  [CVE-2023-28842](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2023-28842) 的修复。
...
- 带有 WSL 后端的 Docker Desktop 不再需要 `com.docker.service` 特权服务永久运行。有关更多信息，请参阅 [Windows 的权限要求](https://docs.docker.com/desktop/windows/permission-requirements/)。

### For Mac

- 修复了主机上存储的属性未为 VirtioFS 用户缓存的性能问题。
- 首次启动 Docker Desktop for Mac 时，用户会看到一个安装窗口，以确认或调整需要特权访问的配置。有关更多信息，请参阅 [Mac 的权限要求](https://docs.docker.com/desktop/mac/permission-requirements/)。
...
- 修复了 [CVE-2023-1802](https://www.cve.org/cverecord?id=CVE-2023-1802)，其中 Artifactory 集成中的一个安全问题会导致在 HTTPS 检查失败时回退到通过纯 HTTP 发送注册表凭据。仅启用了 **访问实验性功能** 的用户会受到影响。修复 [docker/for-win#13344](https://github.com/docker/for-win/issues/13344)。
...
- 从 **故障排除** 页面在 Mac 上卸载 Docker Desktop 可能会触发意外的致命错误弹出窗口。

## 4.17.1

<em class="text-gray-400 italic dark:text-gray-500">2023-03-20</em>
...
<em class="text-gray-400 italic dark:text-gray-500">2023-02-27</em>
...
- 您现在可以发现已在扩展市场中自主发布的扩展。有关自行发布的扩展的更多信息，请参阅[市场扩展](/manuals/extensions/marketplace.md)。
- **容器文件浏览器** 作为实验性功能提供。直接从 GUI 调试容器内的文件系统。
- 您现在可以在 **全局搜索** 中搜索卷。
...
- 将开发环境更新为 v0.2.2，包含初始设置可靠性和安全性修复。
- 为新用户添加了 whalecome 调查。
- 故障排除页面上的确认对话框现在与其他类似对话框的样式一致。
- 在 Kubernetes 集群启动之前重置它导致的致命错误。
- 为 containerd 集成实现了 `docker import`。
- 修复了使用现有标签进行镜像标记的问题（使用 containerd 集成）。
- 为 containerd 集成实现了镜像上的悬空过滤器。
- 修复了当镜像不再存在时 `docker ps` 失败的问题（使用 containerd 集成）。

#### For Mac

- 修复了在未安装特权助手工具 `com.docker.vmnetd` 的系统上下载注册表访问管理策略的问题。
- 修复了如果 `/Library/PrivilegedHelperTools` 不存在，`com.docker.vmnetd` 无法安装的错误。
...
- 修复了用户在每次启动时都被提示创建 docker 套接字符号链接的错误。修复 [docker/for-mac#6634](https://github.com/docker/for-mac/issues/6634)。
- 修复了 **在登录时启动 Docker Desktop** 设置不起作用的错误。修复 [docker/for-mac#6723](https://github.com/docker/for-mac/issues/6723)。
- 修复了 UDP 连接跟踪和 `host.docker.internal`。修复 [docker/for-mac#6699](https://github.com/docker/for-mac/issues/6699)。
...
- 添加了针对 `musl` 的 WSL 集成工具的静态链接，因此无需在用户发行版中安装 `alpine-pkg-glibc`。
- 添加了在 WSL 2 上运行 cgroupv2 的支持。这通过将 `kernelCommandLine = systemd.unified_cgroup_hierarchy=1 cgroup_no_v1=all` 添加到 `%USERPROFILE%\.wslconfig` 文件的 `[wsl2]` 部分来激活。
...
- 修复了在 Visual Studio Code 中打开失败的错误，如果目标路径有空格。
- 修复了导致 `~/.docker/context` 损坏和错误消息“unexpected end of JSON input”的错误。您也可以删除 `~/.docker/context` 来解决此问题。
- 确保在 WSL 2 中使用的凭据助手已正确签名。与 [docker/for-win#10247](https://github.com/docker/for-win/issues/10247) 相关。
...
- 修复了导致剩余文件留在 Debian 包根目录中的错误。修复 [docker/for-linux#123](https://github.com/docker/desktop-linux/issues/123)。

### 安全

#### 所有平台

- 修复了 [CVE-2023-0628](https://www.cve.org/cverecord?id=CVE-2023-0628)，该漏洞允许攻击者在开发环境容器初始化期间执行任意命令，通过诱骗用户打开精心制作的恶意 `docker-desktop://` URL。
- 修复了 [CVE-2023-0629](https://www.cve.org/cverecord?id=CVE-2023-0629)，该漏洞允许非特权用户通过将 Docker 主机设置为 `docker.raw.sock` 或 Windows 上的 `npipe:////.pipe/docker_engine_linux` 来绕过增强容器隔离 (ECI) 限制，通过 `-H` (`--host`) CLI 标志或 `DOCKER_HOST` 环境变量，并在没有 ECI 提供的额外强化功能的情况下启动容器。这不会影响已在运行的容器，也不会影响通过常规方法（不使用 Docker 的原始套接字）启动的容器。

## 4.16.3

<em class="text-gray-400 italic dark:text-gray-500">2023-01-30</em>
...
- 修复了在启动时 Docker Desktop 无法报告缺少或过时的 WSL 安装的问题。修复 [docker/for-win#13184](https://github.com/docker/for-win/issues/13184)。
...
- 修复了在启用 containerd 集成功能时，`docker build` 和 `docker tag` 命令产生 `image already exists` 错误的问题。
- 修复了在 4.16 中引入的回归问题，该问题导致在 amd64 系统上使用目标平台 linux/386 的容器网络中断。修复 [docker/for-mac/6689](https://github.com/docker/for-mac/issues/6689)。

#### For Mac

- 修复了 `Info.plist` 的大小写，这导致 `vmnetd` 在区分大小写的文件系统上中断。修复 [docker/for-mac/6677](https://github.com/docker/for-mac/issues/6677)。

#### For Windows

- 修复了在 4.16 中引入的回归问题，导致在 WSL2 模式下卡在“启动”阶段。修复 [docker/for-win/13165](https://github.com/docker/for-win/issues/13165)

## 4.16.1

<em class="text-gray-400 italic dark:text-gray-500">2023-01-13</em>
...
- 修复了容器内的 `sudo` 因某些镜像出现与安全相关的错误而失败的问题。修复 [docker/for-mac/6675](https://github.com/docker/for-mac/issues/6675) 和 [docker/for-win/13161](https://github.com/docker/for-win/issues/13161)。

## 4.16.0

<em class="text-gray-400 italic dark:text-gray-500">2023-01-12</em>
...
- 分析大型镜像的速度现在快 4 倍。
- 新的本地镜像视图已从实验性转为 GA。
- 为 macOS 13 添加了新的 Beta 功能 Rosetta for Linux，可在 Apple Silicon 上更快地仿真 Intel 镜像。
...
- 修复了 compose 堆栈上的操作无法正常工作的问题。修复 [docker/for-mac#6566](https://github.com/docker/for-mac/issues/6566)。
...
- 修复了工具提示阻止操作按钮的问题。修复 [docker/for-mac#6516](https://github.com/docker/for-mac/issues/6516)。
- 修复了 **容器** 视图中空白的“发生错误”容器列表。

#### For Mac

- 在 macOS 上安装或更新 Docker Desktop 的最低操作系统版本现在是 macOS Big Sur（版本 11）或更高版本。
- 修复了当启用增强容器隔离时，如果使用旧的 `osxfs` 实现进行文件共享，Docker 引擎无法启动的问题。
...
- 修复了在每次重新启动后提示用户安装 `/var/run/docker.sock` 符号链接的问题。
- 确保安装 `/var/run/docker.sock` 符号链接的登录项已签名。
- 修复了在恢复出厂设置时删除 `$HOME/.docker` 的错误。

### For Windows

- 修复了在打印“加载元数据”时 `docker build` 挂起的错误。修复 [docker/for-win#10247](https://github.com/docker/for-win/issues/10247)。
...
<em class="text-gray-400 italic dark:text-gray-500">2022-12-01</em>


### 新增

- 为 macOS 用户带来了显著的性能改进，提供了启用新 VirtioFS 文件共享技术的选项。适用于 macOS 12.5 及更高版本。
- Docker Desktop for Mac 不再需要在安装时或首次运行时安装特权辅助进程 `com.docker.vmnetd`。更多信息请参阅 [Mac 的权限要求](https://docs.docker.com/desktop/mac/permission-requirements/)。
- 添加了 [WebAssembly 功能](/manuals/desktop/features/wasm.md)。需与 [containerd 集成](/manuals/desktop/features/containerd.md) 结合使用。
- 改进了 Beta 和实验性设置的描述，以清楚解释其差异以及用户如何访问它们。
- 对于 Mac 和 Linux，虚拟机的可用磁盘空间现在显示在 Docker Desktop 仪表板的页脚中。
- 如果可用空间低于 3GB，页脚中会显示磁盘空间警告。
- 对 Docker Desktop 界面的更改，使其更加符合 ADA 无障碍标准且视觉上更加统一。
- 在 **扩展** 中添加了 **Build** 标签页，其中包含构建扩展所需的所有资源。
- 添加了更轻松共享扩展的能力，可以通过 `docker extension share` CLI 或扩展 **Manage** 标签页中的共享按钮实现。
- 市场中的扩展现在会显示安装数量。您也可以按安装数量对扩展进行排序。
- 开发环境允许将 Git 仓库克隆到本地绑定挂载，这样您就可以使用任何本地编辑器或 IDE。
...
- 容器在使用 containerd 集成时，重启后会自动恢复。
- 修复了使用 containerd 集成时列出多平台镜像的问题。
- 更好地处理了使用 containerd 集成时的悬空镜像。
- 在使用 containerd 集成时，实现了镜像的 "reference" 过滤器。
- 添加了对通过容器中的 `proxy.pac`、`docker pull` 等自动选择上游 HTTP/HTTPS 代理的支持。
- 修复了在拉取时解析镜像引用的回归问题。修复了 [docker/for-win#13053](https://github.com/docker/for-win/issues/13053)、[docker/for-mac#6560](https://github.com/docker/for-mac/issues/6560) 和 [docker/for-mac#6540](https://github.com/docker/for-mac/issues/6540)。
...
- 当 Docker Desktop 使用“系统”代理且 Windows 设置更改时，Docker Desktop 现在会使用新的 Windows 设置而无需重启。
...
<em class="text-gray-400 italic dark:text-gray-500">2022-11-17</em>
...
- 修复了阻止 **Images** 标签页上的 **Analyze Image** 按钮工作的问题。
- 修复了如果 `/usr/local/lib` 不存在则不会为用户创建符号链接的 bug。修复了 [docker/for-mac#6569](https://github.com/docker/for-mac/issues/6569)

## 4.14.0

<em class="text-gray-400 italic dark:text-gray-500">2022-11-10</em>


### 新增

- 将虚拟化框架设置为 macOS >= 12.5 的默认虚拟机管理程序。
- 将先前的安装迁移到 macOS >= 12.5 的虚拟化框架虚拟机管理程序。
- Docker Business 用户可用的增强容器隔离功能，现在可以从常规设置中启用。

### 更新

- [Docker Engine v20.10.21](/manuals/engine/release-notes/20.10.md#201021)，
  包含针对 Git 漏洞（跟踪于 [CVE-2022-39253](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-39253)）的缓解措施，
  并更新了 `image:tag@digest` 镜像引用的处理方式。
...
- Docker Desktop 现在需要一个大小为 /24 的内部网络子网。如果您之前使用的是 /28，它会自动扩展到 /24。如果您遇到网络问题，请检查 Docker 子网与您的基础架构之间是否存在冲突。修复了 [docker/for-win#13025](https://github.com/docker/for-win/issues/13025)。
- 修复了当 Git URL 包含大写字符时，阻止用户创建开发环境的问题。
- 修复了诊断中报告的 `vpnkit.exe is not running` 错误。
- 将 qemu 回滚到 6.2.0，以修复在运行模拟 amd64 代码时出现 `PR_SET_CHILD_SUBREAPER is unavailable` 等错误。
- 在扩展中启用了 [contextIsolation](https://www.electronjs.org/docs/latest/tutorial/context-isolation) 和 [sandbox](https://www.electronjs.org/docs/latest/tutorial/sandbox) 模式。现在扩展在独立的上下文中运行，这通过限制对大多数系统资源的访问来限制恶意代码可能造成的危害。
- 包含了 `unpigz` 以允许并行解压缩拉取的镜像。
- 修复了在所选容器上执行操作的相关问题。[修复 https://github.com/docker/for-win/issues/13005](https://github.com/docker/for-win/issues/13005)
- 添加了允许您为容器或项目视图显示时间戳的功能。
- 修复了在使用 Control+C 中断 `docker pull` 时可能出现的段错误。
- 增加了默认的 DHCP 租约时间，以避免虚拟机网络故障和每两小时断开连接。
- 移除了容器列表上的无限旋转器。[修复 https://github.com/docker/for-mac/issues/6486](https://github.com/docker/for-mac/issues/6486)
- 修复了在 **设置** 中显示已用空间值不正确的 bug。
- 修复了导致 Kubernetes 在使用 containerd 集成时无法启动的 bug。
- 修复了导致 `kind` 在使用 containerd 集成时无法启动的 bug。
- 修复了导致开发环境在使用 containerd 集成时无法工作的 bug。
- 在 containerd 集成中实现了 `docker diff`。
- 在 containerd 集成中实现了 `docker run —-platform`。
- 修复了导致不安全注册表在使用 containerd 集成时无法工作的 bug。

#### Mac

- 修复了虚拟化框架用户的启动失败问题。
- 重新添加了 Mac 上的 `/var/run/docker.sock` 默认符号链接，以提高与 `tilt` 和 `docker-py` 等工具的兼容性。
- 修复了在新 Mac 安装上无法创建开发环境的问题（错误 "Cannot connect to the Docker daemon at unix:///var/run/docker.sock. Is the docker daemon running?"）。
...
- 修复了在 Windows 上 Compose v2 并非总是默认启用的问题。
- Docker Desktop 现在会在卸载时删除 `C:\Program Files\Docker` 文件夹。

### 已知问题

- 对于某些 Mac OS 用户，安装程序存在一个已知问题，会阻止安装 Docker Desktop 中实验性漏洞和软件包发现功能所需的新辅助工具。要解决此问题，需要使用以下命令创建一个符号链接：`sudo ln -s /Applications/Docker.app/Contents/Resources/bin/docker-index /usr/local/bin/docker-index`

## 4.13.1

<em class="text-gray-400 italic dark:text-gray-500">2022-10-31</em>
...
- 修复了在使用 `Control+C` 或 `CMD+C` 中断 `docker pull` 时可能出现的段错误。
- 增加了默认的 DHCP 租约时间，以避免虚拟机网络故障和每两小时断开连接。
- 将 `Qemu` 回滚到 `6.2.0`，以修复在运行模拟 amd64 代码时出现 `PR_SET_CHILD_SUBREAPER is unavailable` 等错误。

#### Mac

- 重新添加了 Mac 上的 `/var/run/docker.sock` 符号链接默认值，以提高与 `tilt` 和 `docker-py` 等工具的兼容性。修复了 [docker/for-mac#6529](https://github.com/docker/for-mac/issues/6529)。
- 修复了阻止在新 Mac 安装上创建开发环境并导致 `错误 "Cannot connect to the Docker daemon at unix:///var/run/docker.sock. Is the docker daemon running?")` 的问题。
...
<em class="text-gray-400 italic dark:text-gray-500">2022-10-19</em>


### 新增

- 为 Docker Business 用户引入了两项新安全功能：设置管理和增强容器隔离。阅读更多关于 Docker Desktop 新的 [强化 Docker Desktop 安全模型](/manuals/enterprise/security/hardened-desktop/_index.md)。
...
- 将 Docker Engine 和 Docker CLI 更新到 [v20.10.20](/manuals/engine/release-notes/20.10.md#201020)，
  其中包含针对 Git 漏洞（跟踪于 [CVE-2022-39253](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-39253)）的缓解措施，
  并更新了 `image:tag@digest` 镜像引用的处理方式，以及针对 [CVE-2022-36109](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-36109) 的修复。
...
- [Qemu 7.0.0](https://wiki.qemu.org/ChangeLog/7.0) 用于 Docker Desktop VM 内部的 CPU 模拟。
...
- **为这些主机和域名绕过代理设置** 现在可以正确处理 HTTPS 的域名。
- **远程仓库** 视图和每日提示现在可以与需要身份验证的 HTTP 和 HTTPS 代理一起使用。
- 我们为处于产品开发生命周期早期阶段的功能引入了暗启动。选择加入的用户可以随时在“测试版功能”部分的设置中选择退出。
- 为扩展市场添加了类别。
- 在鲸鱼菜单和 **扩展** 标签页上添加了指示器，用于指示扩展更新何时可用。
- 修复了扩展卸载失败的问题，这些扩展的镜像名称没有命名空间，例如 'my-extension'。
- 在 **容器** 标签页中显式显示端口映射。
...
- 添加了一个小改进，当点击侧边栏中的 **扩展** 时，可以导航到扩展市场。
- 添加了一个徽章来标识市场中的新扩展。
- 修复了 Kubernetes 在使用 containerd 集成时无法启动的问题。
- 修复了 `kind` 在使用 containerd 集成时无法启动的问题。
- 修复了开发环境在使用 containerd 集成时无法工作的问题。
- 在 containerd 集成中实现了 `docker diff`。
- 在 containerd 集成中实现了 `docker run —-platform`。
- 修复了不安全注册表在使用 containerd 集成时无法工作的问题。
- 修复了在 **设置** 中显示已用空间值不正确的 bug。
- Docker Desktop 现在从 Github releases 安装凭据帮助程序。参见 [docker/for-win#10247](https://github.com/docker/for-win/issues/10247)、[docker/for-win#12995](https://github.com/docker/for-win/issues/12995)。
...
<em class="text-gray-400 italic dark:text-gray-500">2022-09-01</em>


### 新增

- 添加了使用 containerd 拉取和存储镜像的能力。这是一个实验性功能。
- Docker Desktop 现在可以运行未标记的镜像。修复了 [docker/for-mac#6425](https://github.com/docker/for-mac/issues/6425)。
- 为 Docker 扩展市场添加了搜索功能。修复了 [docker/roadmap#346](https://github.com/docker/roadmap/issues/346)。
- 添加了放大、缩小或将 Docker Desktop 设置为实际大小的能力。这可以通过在 Mac 和 Windows 上分别使用键盘快捷键 ⌘ + / CTRL +、⌘ - / CTRL -、⌘ 0 / CTRL 0，或通过 Mac 上的视图菜单来完成。
- 如果任何相关容器可停止，则添加了 compose 停止按钮。
- 单个 compose 容器现在可以从 **容器** 视图中删除。
- 移除了 Fedora 35 上 virtiofsd <-> qemu 协议不匹配的解决方法，因为它不再需要。Fedora 35 用户应将 qemu 软件包升级到最新版本（截至撰写本文时为 qemu-6.1.0-15.fc35）。
- 为容器实现了集成终端。
- 默认情况下添加了工具提示以显示所有外部链接的链接地址。
...
- 修复了 [CVE-2023-0625](https://www.cve.org/cverecord?id=CVE-2023-0625)，该漏洞允许通过扩展描述/变更日志进行 RCE，可能被恶意扩展滥用。

#### Windows

- 修复了 [CVE-2023-0627](https://www.cve.org/cverecord?id=CVE-2023-0627)，该漏洞允许绕过在 4.11 版本中引入的 `--no-windows-containers` 安装标志。此标志允许管理员禁用 Windows 容器的使用。
- 修复了 [CVE-2023-0633](https://www.cve.org/cverecord?id=CVE-2023-0633)，该漏洞涉及对 Docker Desktop 安装程序的参数注入，可能导致本地权限提升。

### Bug 修复和次要增强

#### 所有平台

- Compose V2 在恢复出厂设置后现已启用。
- Compose V2 在 Docker Desktop 的新安装上默认启用。
- Compose 中环境变量的优先级顺序更加一致，并已明确 [记录](/manuals/compose/how-tos/environment-variables/envvars-precedence.md)。
- 将内核升级到 5.10.124。
- 改进了由计算磁盘大小引起的整体性能问题。与 [docker/for-win#9401](https://github.com/docker/for-win/issues/9401) 相关。
- Docker Desktop 现在会阻止未安装 Rosetta 的 ARM mac 用户切换回仅包含 intel 二进制文件的 Compose V1。
- 将卷大小和 **创建** 列以及容器的 **启动** 列的默认排序顺序更改为降序。
- 重新组织了容器行操作，仅始终显示启动/停止和删除操作，而允许通过行菜单项访问其余操作。
- 快速入门指南现在会立即运行每个命令。
- 定义了容器/compose **状态** 列的排序顺序：运行中 > 部分运行中 > 已暂停 > 部分已暂停 > 已退出 > 部分已退出 > 已创建。
- 修复了即使存在镜像，Docker Desktop 中镜像列表显示为空的问题。与 [docker/for-win#12693](https://github.com/docker/for-win/issues/12693) 和 [docker/for-mac#6347](https://github.com/docker/for-mac/issues/6347) 相关。
- 根据是否显示系统容器来定义哪些镜像是“使用中”。如果与 Kubernetes 和扩展相关的系统容器未显示，则相关镜像不被定义为“使用中”。
- 修复了在某些语言的 Docker 客户端上 `docker exec` 会挂起的 bug。修复了 [https://github.com/apocas/dockerode/issues/534](https://github.com/apocas/dockerode/issues/534)。
- 构建扩展时失败的生成命令不再导致 Docker Desktop 意外退出。
- 修复了扩展在未禁用时在左侧菜单中显示为禁用的 bug。
- 修复了在启用注册表访问管理且访问 Docker Hub 被阻止时，`docker login` 到私有注册表的问题。
- 修复了如果当前集群元数据未存储在 `.kube/config` 文件中，Docker Desktop 无法启动 Kubernetes 集群的 bug。
- 更新了 Docker Desktop 和 MUI 主题包中的工具提示，以与整体系统设计保持一致。
- 复制的终端内容不再包含不间断空格。

#### Mac

- 在 macOS 上安装或更新 Docker Desktop 的最低版本现在为 10.15。修复了 [docker/for-mac#6007](https://github.com/docker/for-mac/issues/6007)。
- 修复了在下载更新后托盘菜单错误地显示“下载即将开始...”的 bug。修复了 [for-mac/issues#5677](https://github.com/docker/for-mac/issues/5677) 中报告的一些问题。
- 修复了应用更新后未重启 Docker Desktop 的 bug。
- 修复了当计算机睡眠时，如果用户使用 virtualization.framework 和限制性防火墙软件，会导致与 Docker 的连接丢失的 bug。
- 修复了即使用户退出应用程序后，Docker Desktop 仍在后台运行的 bug。修复了 [docker/for-mac##6440](https://github.com/docker/for-mac/issues/6440)
- 对于运行 macOS < 12.5 的用户，禁用了虚拟化框架和 VirtioFS。

#### Windows

- 修复了在更新期间显示的版本可能不正确的 bug。修复了 [for-win/issues#12822](https://github.com/docker/for-win/issues/12822)。

## 4.11.1

<em class="text-gray-400 italic dark:text-gray-500">2022-08-05</em>
...
- 修复了阻止 VM 系统位置（例如 /var/lib/docker）被绑定挂载的回归问题 [for-mac/issues#6433](https://github.com/docker/for-mac/issues/6433)

#### Windows

- 修复了从 WSL2 发行版 `docker login` 到私有注册表的问题 [docker/for-win#12871](https://github.com/docker/for-win/issues/12871)

## 4.11.0

<em class="text-gray-400 italic dark:text-gray-500">2022-07-28</em>


### 新增

- Docker Desktop 现在在 VMware ESXi 和 Azure VM 中为 Docker Business 客户提供全面支持。更多信息，请参阅 [在 VM 或 VDI 环境中运行 Docker Desktop](/manuals/desktop/setup/vm-vdi.md)
- 向扩展市场添加了两个新扩展（[vcluster](https://hub.docker.com/extensions/loftsh/vcluster-dd-extension) 和 [PGAdmin4](https://hub.docker.com/extensions/mochoa/pgadmin4-docker-extension)）。
- 扩展市场中添加了对扩展进行排序的功能。
- 修复了导致某些用户被过于频繁地要求提供反馈的 bug。您现在每年只会被要求提供两次反馈。
- 为 Docker Desktop 添加了自定义主题设置。这允许您为 Docker Desktop 指定暗色或亮色模式，独立于您的设备设置。修复了 [docker/for-win#12747](https://github.com/docker/for-win/issues/12747)
...
- 改进了 `docker login` 的错误消息，如果注册表访问管理阻止了 Docker 引擎访问 Docker Hub。
- 提高了主机和 Docker 之间的吞吐量。例如提高了 `docker cp` 的性能。
- 收集诊断所需时间更短。
- 在容器概览上选择或取消选择一个 compose 应用现在会选择/取消选择其所有容器。
- 容器概览镜像列上的标签名称可见。
- 在终端滚动条中添加了搜索装饰，以便可以看到视口之外的匹配项。
- 修复了容器页面上搜索无法正常工作的问题 [docker/for-win#12828](https://github.com/docker/for-win/issues/12828)。
- 修复了导致 **卷** 屏幕无限加载的问题 [docker/for-win#12789](https://github.com/docker/for-win/issues/12789)。
- 修复了容器 UI 中调整大小或隐藏列不起作用的问题。修复了 [docker/for-mac#6391](https://github.com/docker/for-mac/issues/6391)。
- 修复了在离开市场屏幕时，同时安装、更新或卸载多个扩展的状态丢失的 bug。
- 修复了关于页面中的 compose 版本仅在重启 Docker Desktop 后才从 v2 更新到 v1 的问题。
- 修复了用户无法看到日志视图的问题，因为其底层硬件不支持 WebGL2 渲染。修复了 [docker/for-win#12825](https://github.com/docker/for-win/issues/12825)。
...
- 如果在 UI 中配置了 HTTP/HTTPS 代理，则它会自动将来自镜像构建和运行容器的流量发送到代理。这避免了在每个容器或构建中单独配置环境变量的需要。
- 添加了 `--backend=windows` 安装程序选项，以将 Windows 容器设置为默认后端。

#### Linux

- 修复了在路径中包含空格时设置文件共享的 bug。

## 4.10.1

<em class="text-gray-400 italic dark:text-gray-500">2022-07-05</em>
...
- 修复了在 UI 中对从 WSL 创建的 Compose 应用执行操作失败的 bug。修复了 [docker/for-win#12806](https://github.com/docker/for-win/issues/12806)。

#### Mac

- 修复了由于路径未初始化而导致安装命令失败的 bug。修复了 [docker/for-mac#6384](https://github.com/docker/for-mac/issues/6384)。

## 4.10.0

<em class="text-gray-400 italic dark:text-gray-500">2022-06-30</em>


### 新增

- 您现在可以在 Docker Desktop 中运行镜像之前添加环境变量。
- 添加了功能以更轻松地处理容器的日志，例如正则表达式搜索以及在容器仍在运行时清除容器日志的能力。
- 在容器表上实现了反馈。添加了端口并分离了容器和镜像名称。
- 向扩展市场添加了两个新扩展，Ddosify 和 Lacework。

### 移除

- 在设计新主页时移除了主页。您可以在此处提供[反馈](https://docs.google.com/forms/d/e/1FAIpQLSfYueBkJHdgxqsWcQn4VzBn2swu4u_rMQRIMa8LExYb_72mmQ/viewform?entry.1237514594=4.10)。
...
- `docker extension` CLI 命令默认随 Docker Desktop 提供。
- 增大了扩展市场中显示的截图大小。
- 修复了如果其后端容器停止，Docker 扩展无法加载的 bug。修复了 [docker/extensions-sdk#16](https://github.com/docker/extensions-sdk/issues/162)。
- 修复了镜像搜索字段无故被清空的 bug。修复了 [docker/for-win#12738](https://github.com/docker/for-win/issues/12738)。
- 修复了许可证协议不显示并静默阻止 Docker Desktop 启动的 bug。
- 修复了未发布扩展的显示镜像和标签，以实际显示来自已安装未发布扩展的镜像和标签。
- 修复了支持屏幕上重复的页脚。
- 开发环境可以从 GitHub 仓库的子目录创建。
- 移除了在使用 Docker Desktop 离线时无法加载每日提示的错误消息。修复了 [docker/for-mac#6366](https://github.com/docker/for-mac/issues/6366)。

#### Mac

- 修复了 macOS 上 bash 补全文件位置的 bug。修复了 [docker/for-mac#6343](https://github.com/docker/for-mac/issues/6343)。
- 修复了如果用户名超过 25 个字符，Docker Desktop 无法启动的 bug。修复了 [docker/for-mac#6122](https://github.com/docker/for-mac/issues/6122)。
- 修复了由于无效的系统代理配置导致 Docker Desktop 无法启动的 bug。修复了 [docker/for-mac#6289](https://github.com/docker/for-mac/issues/6289) 中报告的一些问题。
- 修复了启用实验性虚拟化框架时 Docker Desktop 无法启动的 bug。
- 修复了卸载 Docker Desktop 后托盘图标仍然显示的 bug。

#### Windows

- 修复了导致 Hyper-V 上 CPU 使用率过高的 bug。修复了 [docker/for-win#12780](https://github.com/docker/for-win/issues/12780)。
- 修复了 Docker Desktop for Windows 无法启动的 bug。修复了 [docker/for-win#12784](https://github.com/docker/for-win/issues/12784)。
- 修复了 `--backend=wsl-2` 安装程序标志未将后端设置为 WSL 2 的问题。修复了 [docker/for-win#12746](https://github.com/docker/for-win/issues/12746)。
...
- 偶尔 Docker 引擎会在 `docker system prune` 期间重启。这是当前引擎中使用的 buildkit 版本的一个[已知问题](https://github.com/moby/buildkit/pull/2177)，将在未来的版本中修复。

## 4.9.1

<em class="text-gray-400 italic dark:text-gray-500">2022-06-16</em>
...
<p>
  <a rel="noopener"
     href="https://desktop.docker.com/win/main/amd64/81317/Docker%20Desktop%20Installer.exe">Windows</a>
  (<a rel="noopener"
      href="https://desktop.docker.com/win/main/amd64/81317/checksums.txt">checksum</a>) |
  
  <a rel="noopener" href="https://desktop.docker.com/mac/main/arm64/81317/Docker.dmg">Mac
    with Apple chip</a>
  (<a rel="noopener"
      href="https://desktop.docker.com/mac/main/arm64/81317/checksums.txt">checksum</a>) |
  <a rel="noopener" href="https://desktop.docker.com/mac/main/amd64/81317/Docker.dmg">Mac
    with Intel chip</a>
  (<a rel="noopener"
      href="https://desktop.docker.com/mac/main/amd64/81317/checksums.txt">checksum</a>)
  
  |
  <a rel="noopener"
     href="https://desktop.docker.com/linux/main/amd64/81317/docker-desktop-4.9.1-amd64.deb">Debian</a>
  -
  <a rel="noopener"
     href="https://desktop.docker.com/linux/main/amd64/81317/docker-desktop-4.9.1-x86_64.rpm">RPM</a>
  -
  <a rel="noopener"
     href="https://desktop.docker.com/linux/main/amd64/81317/docker-desktop-4.9.1-x86_64.pkg.tar.zst">Arch</a>
  (<a rel="noopener"
      href="https://desktop.docker.com/linux/main/amd64/81317/checksums.txt">checksum</a>)</p>
</blockquote>
...
- 修复了空白仪表板屏幕的问题。修复了 [docker/for-win#12759](https://github.com/docker/for-win/issues/12759)。

## 4.9.0

<em class="text-gray-400 italic dark:text-gray-500">2022-06-02</em>


### 新增

- 在主页上添加了关于 Elasticsearch、MariaDB、Memcached、MySQL、RabbitMQ 和 Ubuntu 的附加指南。
- 在 Docker Desktop 仪表板中添加了页脚，包含有关 Docker Desktop 更新状态和 Docker 引擎统计信息的一般信息。
- 重新设计了容器表，添加了：
  - 将容器 ID 复制到剪贴板的按钮
  - 每个容器的暂停按钮
  - 容器表的列调整大小
...
- 修复了在 Docker Desktop 暂停时退出应用程序导致 Docker Desktop 挂起的问题。
- 修复了 PKI 过期后 Kubernetes 集群无法正确重置的问题。
- 修复了扩展市场未使用已定义的 http 代理的问题。
- 改进了 Docker Desktop 仪表板中的日志搜索功能，以允许空格。
- 在仪表板按钮上使用鼠标中键点击现在表现为左键点击，而不是打开空白窗口。
...
- 修复了 WSL 2 集成中的一个 bug，如果文件或目录被绑定挂载到容器，并且容器退出，则该文件或目录将被同名的其他类型对象替换。例如，如果文件被目录替换或目录被文件替换，则任何尝试绑定挂载新对象都会失败。
- 修复了托盘图标和仪表板 UI 不显示且 Docker Desktop 未完全启动的 bug。修复了 [docker/for-win#12622](https://github.com/docker/for-win/issues/12622)。

### 已知问题

#### Linux

- 更改绑定挂载中文件的所有权权限会失败。这是由于我们在主机和运行 Docker 引擎的 VM 之间实现文件共享的方式所致。我们旨在在下一个版本中解决此问题。

## 4.8.2

<em class="text-gray-400 italic dark:text-gray-500">2022-05-18</em>
...
- 修复了手动代理设置导致拉取镜像时出现问题的问题。修复了 [docker/for-win#12714](https://github.com/docker/for-win/issues/12714) 和 [docker/for-mac#6315](https://github.com/docker/for-mac/issues/6315)。
- 修复了禁用扩展时 CPU 使用率过高的问题。修复了 [docker/for-mac#6310](https://github.com/docker/for-mac/issues/6310)。
- Docker Desktop 现在会在日志文件和诊断中编辑 HTTP 代理密码。
...
<em class="text-gray-400 italic dark:text-gray-500">2022-05-09</em>


### 新增

- 发布了 [Docker Desktop for Linux](/manuals/desktop/setup/install/linux/_index.md)。
- [Docker 扩展](/manuals/extensions/_index.md) 和扩展 SDK 的测试版发布。
- 创建了一个 Docker 主页，您可以在其中运行流行镜像并了解如何使用它们。
- [Compose V2 现已正式发布](https://www.docker.com/blog/announcing-compose-v2-general-availability/)

### Bug 修复和增强

- 修复了在更新 Docker Desktop 时导致 Kubernetes 集群被删除的 bug。
...
<em class="text-gray-400 italic dark:text-gray-500">2022-05-06</em>
...
- 引入了读取系统代理。除非代理与您的操作系统级代理不同，否则您不再需要手动配置代理。
- 修复了在代理后运行时在仪表板中显示远程仓库的 bug。
- 修复了 vpnkit 建立并阻塞客户端连接的问题，即使服务器已消失。参见 [docker/for-mac#6235](https://github.com/docker/for-mac/issues/6235)
- 对 Docker Desktop 中的卷标签页进行了改进：
  - 显示卷大小。
  - 可以调整列大小、隐藏和重新排序。
  - 列的排序顺序和隐藏状态会持久化，即使在 Docker Desktop 重启后也是如此。
  - 在标签页之间切换时，行选择会持久化，即使在 Docker Desktop 重启后也是如此。
- 修复了开发环境标签页中当屏幕添加更多项目时未添加滚动条的 bug。
- 标准化了仪表板中的标题标题和操作。
- 添加了通过 HTTP 代理下载注册表访问管理策略的支持。
- 修复了与计算机长时间处于睡眠模式时远程仓库为空相关的问题。
- 修复了在清理过程中，如果镜像名称未标记为 "&lt;none>" 但其标签是，则悬空镜像未被选中的 bug。
...
- 修复了没有命名空间的镜像（例如 `docker pull <private registry>/image`）会被注册表访问管理错误地阻止的问题，除非在设置中启用了对 Docker Hub 的访问。

#### Mac

- Docker Desktop 的图标现在符合 Big Sur 风格指南。参见 [docker/for-mac#5536](https://github.com/docker/for-mac/issues/5536)
- 修复了 Dock 图标重复和 Dock 图标无法按预期工作的问题。修复了 [docker/for-mac#6189](https://github.com/docker/for-mac/issues/6189)。
- 改进了对 `Cmd+Q` 快捷键的支持。
...
- 目前，如果您正在运行 Kubernetes 集群，升级到 Docker Desktop 4.8.0 时它将被删除。我们旨在在下一个版本中修复此问题。
...
<em class="text-gray-400 italic dark:text-gray-500">2022-04-19</em>
...
- 修复了更新因符号链接错误而失败的 bug。修复了 [docker/for-win#12650](https://github.com/docker/for-win/issues/12650)。
- 修复了阻止使用 Windows 容器模式的 bug。修复了 [docker/for-win#12652](https://github.com/docker/for-win/issues/12652)。

## 4.7.0

<em class="text-gray-400 italic dark:text-gray-500">2022-04-07</em>


### 新增

- IT 管理员现在可以使用命令行远程安装 Docker Desktop。
- 添加了 Docker 软件物料清单 (SBOM) CLI 插件。新的 CLI 插件使用户能够为 Docker 镜像生成 SBOM。
- 对于新的 Kubernetes 集群，使用 [cri-dockerd](https://github.com/Mirantis/cri-dockerd) 代替 `dockershim`。此更改对用户来说是透明的，Kubernetes 容器像以前一样在 Docker 引擎上运行。`cri-dockerd` 允许 Kubernetes 使用标准的 [容器运行时接口](https://github.com/kubernetes/cri-api#readme) 管理 Docker 容器，该接口与用于控制其他容器运行时的接口相同。更多信息，请参阅 [Dockershim 的未来是 cri-dockerd](https://www.mirantis.com/blog/the-future-of-dockershim-is-cri-dockerd/)。
...
- 将 Docker Engine 更新至 v20.10.14，以解决 [CVE-2022-24769](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-24769)
- 将 containerd 更新至 v1.5.11，以解决 [CVE-2022-24769](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-24769)

### Bug 修复和增强

#### 所有平台

- 修复了注册表访问管理策略在失败后从未刷新的 bug。
- UI 中的日志和终端现在尊重您的操作系统主题的亮色和暗色模式。
- 通过多选复选框轻松一次性清理多个卷。
- 改进了登录反馈。
...
- 修复了使用 Hyperkit 时，从睡眠唤醒后 gettimeofday() 性能下降的问题。修复了 [docker/for-mac#3455](https://github.com/docker/for-mac/issues/3455)。
...
- 修复了 WSL 2 集成中的一个 bug，该 bug 导致在重启 Docker Desktop 或切换到 Windows 容器后 Docker 命令停止工作。

## 4.6.1

<em class="text-gray-400 italic dark:text-gray-500">2022-03-22</em>
...
- 防止 vpnkit-forwarder 陷入循环，用错误消息填满日志。
- 修复了未设置 HTTP 代理时的诊断上传问题。修复了 [docker/for-mac#6234](https://github.com/docker/for-mac/issues/6234)。
- 从自我诊断中移除了一个误报的 "vm is not running" 错误。修复了 [docker/for-mac#6233](https://github.com/docker/for-mac/issues/6233)。

## 4.6.0

<em class="text-gray-400 italic dark:text-gray-500">2022-03-14</em>
...
- Docker Desktop 4.6.0 为 macOS 用户提供了启用名为 VirtioFS 的新实验性文件共享技术的选项。在测试中，VirtioFS 已被证明可以显著减少主机和虚拟机之间同步更改所需的时间，从而带来显著的性能改进。更多信息，请参阅 [VirtioFS](/manuals/desktop/settings-and-maintenance/settings.md#beta-features)。
...
- 修复了 [CVE-2022-0847](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-0847)，即“Dirty Pipe”，这是一个可能允许攻击者从容器内部修改主机上容器镜像中文件的问题。
  如果使用 WSL 2 后端，您必须通过运行 `wsl --update` 来更新 WSL 2。

#### Windows

- 修复了 [CVE-2022-26659](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-26659)，该漏洞可能允许攻击者在安装或更新 Docker Desktop 期间覆盖系统上任何管理员可写文件。
...
- 修复了 Docker Desktop 不再在后台启动的回归问题。修复了 [docker/for-mac#6167](https://github.com/docker/for-mac/issues/6167)。
- 修复了缺少 Docker Desktop Dock 图标的问题。修复了 [docker/for-mac#6173](https://github.com/docker/for-mac/issues/6173)。
- 在使用实验性 `virtualization.framework` 时，加快了块设备访问速度。参见 [benchmarks](https://github.com/docker/roadmap/issues/7#issuecomment-1050626886)。
- 将默认 VM 内存分配增加到物理内存的一半（最小 2 GB，最大 8 GB），以获得更好的开箱即用性能。

#### Windows

- 修复了 UI 卡在 `启动中` 状态的问题，尽管 Docker Desktop 在命令行中工作正常。
- 修复了缺少 Docker Desktop 系统托盘图标的问题 [docker/for-win#12573](https://github.com/docker/for-win/issues/12573)
- 修复了在最新 5.10.60.1 内核下 WSL 2 中的注册表访问管理。
- 修复了在从 WSL 2 环境启动的 Compose 应用程序中选择容器时 UI 崩溃的问题。修复了 [docker/for-win#12567](https://github.com/docker/for-win/issues/12567)。
...
- 启用 VirtioFS 后，以不同 Unix 用户 ID 运行进程的容器可能会遇到缓存问题。例如，如果以 `root` 身份运行的进程查询文件，而另一个以用户 `nginx` 身份运行的进程立即尝试访问同一文件，`nginx` 进程将收到“权限被拒绝”错误。

## 4.5.1

<em class="text-gray-400 italic dark:text-gray-500">2022-02-15</em>
...
- 修复了 Docker Desktop 仪表板中的崩溃，该崩溃会导致系统托盘菜单消失。

如果您在 Windows Home 上运行 Docker Desktop，安装 4.5.1 将自动将其切换回 WSL 2。如果您运行的是其他版本的 Windows，并且希望 Docker Desktop 使用 WSL 2 后端，则必须通过在 **设置 > 常规** 部分启用 **使用基于 WSL 2 的引擎** 选项手动切换。
或者，您可以编辑位于 `%APPDATA%\Docker\settings.json` 的 Docker Desktop 设置文件，并将 `wslEngineEnabled` 字段的值手动切换为 `true`。

## 4.5.0

<em class="text-gray-400 italic dark:text-gray-500">2022-02-10</em>


### 新增

- Docker Desktop 4.5.0 引入了新版本的 Docker 菜单，为所有操作系统创建了一致的用户体验。更多信息，请参阅博客文章 [New Docker Menu & Improved Release Highlights with Docker Desktop 4.5](https://www.docker.com/blog/new-docker-menu-improved-release-highlights-with-docker-desktop-4-5/)
...
- 修复了 [CVE-2021-44719](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2021-44719)，该漏洞允许 Docker Desktop 用于从容器访问主机上的任何用户文件，绕过允许的共享文件夹列表。

#### Windows

- 修复了 [CVE-2022-23774](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-23774)，该漏洞允许 Docker Desktop 允许攻击者移动任意文件。

### Bug 修复和增强

#### 所有平台

- 修复了在用户退出 Docker Desktop 并启动应用程序后，Docker Desktop 错误地提示用户登录的问题。
- 通过在 Linux 中设置 `fs.inotify.max_user_watches=1048576` 和 `fs.inotify.max_user_instances=8192` 来增加文件系统监视 (inotify) 限制。修复了 [docker/for-mac#6071](https://github.com/docker/for-mac/issues/6071)。

#### Mac

- 修复了在使用 `osxfs` 且没有主机目录与 VM 共享时，导致 VM 在启动期间无响应的问题。
- 修复了如果应用程序是在不同版本的 Docker Compose 中启动的，则不允许用户使用 Docker Desktop Dashboard 停止 Docker Compose 应用程序的问题。例如，如果用户在 V1 中启动了 Docker Compose 应用程序，然后切换到 Docker Compose V2，则尝试停止 Docker Compose 应用程序将失败。
- 修复了在用户退出 Docker Desktop 并启动应用程序后，Docker Desktop 错误地提示用户登录的问题。
...
- 修复了与使用版本 2 启动的 compose 应用相关的问题，但仪表板仅处理版本 1。

### 已知问题

#### Windows

从头安装 Docker Desktop 4.5.0 有一个 bug，会默认 Docker Desktop 使用 Hyper-V 后端而不是 WSL 2。这意味着，Windows Home 用户将无法启动 Docker Desktop，因为 WSL 2 是唯一受支持的后端。要解决此问题，您必须从计算机上卸载 4.5.0，然后下载并安装 Docker Desktop 4.5.1 或更高版本。或者，您可以编辑位于 `%APPDATA%\Docker\settings.json` 的 Docker Desktop settings.json 文件，并将 `wslEngineEnabled` 字段的值手动切换为 `true`。

## 4.4.4

<em class="text-gray-400 italic dark:text-gray-500">2022-01-24</em>
...
- 在浏览器中登录后点击 **Proceed to Desktop**，有时不会将仪表板置于前台。
- 登录后，当仪表板获得焦点时，即使点击后台窗口，它有时也会保持在前台。作为解决方法，您需要在点击另一个应用程序窗口之前点击仪表板。
- 当通过 `registry.json` 文件启用组织限制时，每周提示会显示在强制登录对话框之上。

## 4.4.3

<em class="text-gray-400 italic dark:text-gray-500">2022-01-14</em>
...
- 禁用了仪表板快捷键，以防止在最小化或未聚焦时捕获它们。修复了 [docker/for-win#12495](https://github.com/docker/for-win/issues/12495)。
...
<em class="text-gray-400 italic dark:text-gray-500">22-01-13</em>


### 新增

- 轻松、安全地使用 Auth0 和单点登录登录
  - 单点登录：具有 Docker Business 订阅的用户现在可以配置 SSO，以使用其身份提供商 (IdP) 进行身份验证来访问 Docker。更多信息，请参阅 [单点登录](/manuals/enterprise/security/single-sign-on/_index.md)。
  - 登录 Docker Desktop 现在会引导您通过浏览器进行，因此您可以获得密码管理器自动填充的所有好处。
...
- 修复了 [CVE-2021-45449](../security/_index.md#cve-2021-45449)，该漏洞影响当前使用 Docker Desktop 版本 4.3.0 或 4.3.1 的用户。

Docker Desktop 版本 4.3.0 和 4.3.1 有一个 bug，可能会在登录期间将敏感信息（访问令牌或密码）记录在用户的机器上。
这仅影响使用 Docker Desktop 4.3.0、4.3.1 并且用户在 4.3.0、4.3.1 上登录过的用户。访问此数据需要有权访问用户的本地文件。

### Bug 修复和增强

#### 所有平台

- 如果 `registry.json` 在 `allowedOrgs` 字段中包含多个组织，Docker Desktop 会显示错误。如果您为不同的开发者组使用多个组织，则必须为每个组提供一个单独的 `registry.json` 文件。
- 修复了 Compose 中的一个回归问题，该问题将容器名称分隔符从 `-` 改回 `_`。修复了 [docker/compose-switch](https://github.com/docker/compose-switch/issues/24)。

#### Mac

- 修复了仪表板中容器的内存统计信息。修复了 [docker/for-mac/#4774](https://github.com/docker/for-mac/issues/6076)。
- 在 `settings.json` 中添加了一个已弃用的选项：`"deprecatedCgroupv1": true`，它将 Linux 环境切换回 cgroups v1。如果您的软件需要 cgroups v1，您应该更新它以与 cgroups v2 兼容。尽管 cgroups v1 应该继续工作，但某些未来功能可能会依赖于 cgroups v2。此外，某些 Linux 内核错误可能仅在 cgroups v2 中修复。
- 修复了在暂停 Docker Desktop 后将计算机置于睡眠模式，导致计算机从睡眠模式唤醒后 Docker Desktop 无法从暂停状态恢复的问题。修复了 [for-mac#6058](https://github.com/docker/for-mac/issues/6058)。

#### Windows

- 执行 `恢复出厂设置` 不再关闭 Docker Desktop。
...
- 在浏览器中登录后点击 **Proceed to Desktop**，有时不会将仪表板置于前台。
- 登录后，当仪表板获得焦点时，即使点击后台窗口，它有时也会保持在前台。作为解决方法，您需要在点击另一个应用程序窗口之前点击仪表板。
- 当仪表板打开时，即使它没有焦点或已最小化，它仍然会捕获键盘快捷键（例如 ctrl-r 用于重启）。
...
这个新版本的 `docker scan` 能够检测 [Log4j 2 CVE-2021-44228](https://nvd.nist.gov/vuln/detail/CVE-2021-44228) 和 [Log4j 2 CVE-2021-45046](https://nvd.nist.gov/vuln/detail/CVE-2021-45046)。

更多信息，请阅读博客文章 [Apache Log4j 2 CVE-2021-44228](https://www.docker.com/blog/apache-log4j-2-cve-2021-44228/)。

## 4.3.1

<em class="text-gray-400 italic dark:text-gray-500">2021-12-11</em>
...
<em class="text-gray-400 italic dark:text-gray-500">2021-12-02</em>
...
- 如果主机缺少互联网连接，添加了自我诊断警告。
- 修复了用户无法使用卷 UI 中的“另存为”选项从卷保存文件的问题。修复了 [docker/for-win#12407](https://github.com/docker/for-win/issues/12407)。
- Docker Desktop 现在使用 cgroupv2。如果您需要在容器中运行 `systemd`，那么：
  - 确保您的 `systemd` 版本支持 cgroupv2。[它必须至少是 `systemd` 247](https://github.com/systemd/systemd/issues/19760#issuecomment-851565075)。考虑将任何 `centos:7` 镜像升级到 `centos:8`。
...
- Apple 硅芯片上的 Docker Desktop 不再需要 Rosetta 2，除了[三个可选的命令行工具](/manuals/desktop/troubleshoot-and-support/troubleshoot/known-issues.md)。

#### Windows

- 修复了如果主目录路径包含正则表达式中使用的字符，导致 Docker Desktop 在启动期间失败的问题。修复了 [docker/for-win#12374](https://github.com/docker/for-win/issues/12374)。

### 已知问题

Docker Desktop 仪表板在基于 Hyper-V 的机器上错误地将容器内存使用显示为零。
您可以使用命令行中的 [`docker stats`](/reference/cli/docker/container/stats.md) 命令作为解决方法来查看实际内存使用情况。参见 [docker/for-mac#6076](https://github.com/docker/for-mac/issues/6076)。

### 弃用

- 以下内部 DNS 名称已被弃用，并将在未来版本中移除：`docker-for-desktop`、`docker-desktop`、`docker.for.mac.host.internal`、`docker.for.mac.localhost`、`docker.for.mac.gateway.internal`。您现在必须使用 `host.docker.internal`、`vm.docker.internal` 和 `gateway.docker.internal`。
- 已移除：自定义 RBAC 规则已从 Docker Desktop 中移除，因为它将 `cluster-admin` 权限授予所有服务账户。修复了 [docker/for-mac/#4774](https://github.com/docker/for-mac/issues/4774)。

## 4.2.0

<em class="text-gray-400 italic dark:text-gray-500">2021-11-09</em>


### 新增

**暂停/恢复**：您现在可以在不积极使用 Docker Desktop 时暂停您的会话，以节省计算机上的 CPU 资源。

- 实现了 [Docker Public Roadmap#226](https://github.com/docker/roadmap/issues/226)

**软件更新**：关闭自动检查更新的选项现在对所有 Docker 订阅的用户可用，包括 Docker Personal 和 Docker Pro。所有与更新相关的设置都已移至 **软件更新** 部分。

- 实现了 [Docker Public Roadmap#228](https://github.com/docker/roadmap/issues/228)

**窗口管理**：当您关闭并重新打开 Docker Desktop 时，Docker Desktop 仪表板的窗口大小和位置会保留。
...
- 修复了在 Docker Desktop 仪表板上显示更新可用性的指示器位置。

#### Mac

- 修复了在致命错误对话框上点击 **退出** 导致 Docker Desktop 停止响应的问题。
- 修复了影响在主机目录之上绑定挂载 `docker volume` 的用户的罕见启动失败问题。如果存在，此修复还将删除相应主机目录上手动添加的 `DENY DELETE` ACL 条目。
- 修复了一个 bug，该 bug 会导致在升级时忽略 `Docker.qcow2` 文件，而使用新的 `Docker.raw` 文件，从而导致容器和镜像消失。请注意，如果系统同时存在这两个文件（由于之前的 bug），则将使用最近修改的文件，以避免最近的容器和镜像再次消失。要强制使用旧的 `Docker.qcow2`，请删除较新的 `Docker.raw` 文件。修复了 [docker/for-mac#5998](https://github.com/docker/for-mac/issues/5998)。
- 修复了一个 bug，该 bug 会导致子进程在关闭期间意外失败，从而触发意外的致命错误弹出窗口。修复了 [docker/for-mac#5834](https://github.com/docker/for-mac/issues/5834)。

#### Windows

- 修复了在致命错误对话框中点击退出时 Docker Desktop 有时会挂起的问题。
- 修复了当更新已下载但尚未应用时，频繁显示 **下载更新** 弹出窗口的问题 [docker/for-win#12188](https://github.com/docker/for-win/issues/12188)。
- 修复了安装新更新时在应用程序有时间关闭之前就终止应用程序的问题。
- 修复：即使组策略阻止用户启动先决条件服务（例如 LanmanServer），Docker Desktop 的安装现在也能正常工作 [docker/for-win#12291](https://github.com/docker/for-win/issues/12291)。

## 4.1.1

<em class="text-gray-400 italic dark:text-gray-500">2021-10-12</em>
...
> 从 4.1.0 升级时，Docker 菜单不会更改为 **更新并重启**，因此您可以等待下载完成（图标更改），然后选择 **重启**。此 bug 在 4.1.1 中已修复，适用于未来的升级。

- 修复了一个 bug，该 bug 会导致在升级时忽略 `Docker.qcow2` 文件，而使用新的 `Docker.raw` 文件，从而导致容器和镜像消失。如果系统同时存在这两个文件（由于之前的 bug），则将使用最近修改的文件，以避免最近的容器和镜像再次消失。要强制使用旧的 `Docker.qcow2`，请删除较新的 `Docker.raw` 文件。修复了 [docker/for-mac#5998](https://github.com/docker/for-mac/issues/5998)。
- 修复了更新通知覆盖层在 **设置** 按钮和 Docker Desktop 仪表板中的 **软件更新** 按钮之间有时不同步的问题。
...
<em class="text-gray-400 italic dark:text-gray-500">2021-09-30</em>


### 新增

- **软件更新**：设置标签页现在包含一个新部分，以帮助您管理 Docker Desktop 更新。**软件更新** 部分会在有新更新时通知您，并允许您下载更新或查看新版本中包含的信息。
- **Compose V2** 您现在可以在常规设置中指定是否使用 Docker Compose V2。
- **卷管理**：卷管理现在对任何订阅的用户可用，包括 Docker Personal。实现了 [Docker Public Roadmap#215](https://github.com/docker/roadmap/issues/215)
...
- 修复了与反恶意软件软件触发相关的 bug，自我诊断避免调用 `net.exe` 实用程序。
...
- 修复了 CLI 上下文切换与 UI 不同步的问题。参见 [docker/for-win#11721](https://github.com/docker/for-win/issues/11721)。
- 在 `settings.json` 中添加了键 `vpnKitMaxPortIdleTime`，以允许禁用或延长空闲网络连接超时。
- 修复了退出时的崩溃。参见 [docker/for-win#12128](https://github.com/docker/for-win/issues/12128)。
- 修复了 CLI 工具在 WSL 2 发行版中不可用的 bug。
- 修复了从 Linux 切换到 Windows 容器时因 `panic.log` 的访问权限而卡住的问题。参见 [for-win#11899](https://github.com/docker/for-win/issues/11899)。

### 已知问题

#### Windows

在某些基于 WSL 的发行版（例如 ArchWSL）上升级到 4.1.0 时，Docker Desktop 可能无法启动。参见 [docker/for-win#12229](https://github.com/docker/for-win/issues/12229)

## 4.0.1

<em class="text-gray-400 italic dark:text-gray-500">2021-09-13</em>
...
- [Compose V2 RC3](https://github.com/docker/compose/releases/tag/v2.0.0-rc.3)
  ...
  - 修复了使用 `compose up --scale` 进行降级时的 go panic。
  - 修复了 `compose run --rm` 在捕获退出代码时的竞态条件。

### Bug 修复和次要更改

#### 所有平台

- 修复了 Docker Desktop 仪表板中无法复制粘贴的 bug。

#### Windows

- 修复了 Docker Desktop 在使用 Hyper-V 引擎时无法正确启动的 bug。参见 [docker/for-win#11963](https://github.com/docker/for-win/issues/11963)

## 4.0.0

<em class="text-gray-400 italic dark:text-gray-500">2021-08-31</em>


### 新增

Docker 已[宣布](https://www.docker.com/blog/updating-product-subscriptions/)更新和扩展产品订阅，以提高开发者和企业的生产力、协作性和安全性。

更新后的 [Docker 订阅服务协议](https://www.docker.com/legal/docker-subscription-service-agreement) 包含对 **Docker Desktop** 条款的更改。

- Docker Desktop **对小型企业（少于 250 名员工且年收入低于 1000 万美元）、个人使用、教育和非商业开源项目保持免费**。
- 对于大型企业的专业使用，需要付费订阅（**Pro、Team 或 Business**），每月仅需 5 美元。
- 这些条款的生效日期为 2021 年 8 月 31 日。对于需要付费订阅才能使用 Docker Desktop 的用户，有一个宽限期，直到 2022 年 1 月 31 日。
...
要了解这些变化如何影响您，请阅读 [常见问题解答](https://www.docker.com/pricing/faq)。
更多信息，请参阅 [Docker 订阅概述](../subscription/_index.md)。

### 升级

- [Compose V2 RC2](https://github.com/docker/compose-cli/releases/tag/v2.0.0-rc.2)
  - 修复了 `compose down` 的项目名称不区分大小写的问题。参见 [docker/compose-cli#2023](https://github.com/docker/compose-cli/issues/2023)
  - 修复了非规范化项目名称。
  - 修复了部分引用上的端口合并。
...
- 修复了 CLI 工具在 WSL 2 发行版中不可用的 bug。
- 修复了由于 `panic.log` 的访问权限而无法从 Linux 切换到 Windows 容器的 bug。[for-win#11899](https://github.com/docker/for-win/issues/11899)
