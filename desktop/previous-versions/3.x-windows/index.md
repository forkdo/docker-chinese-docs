...
此页面包含 Docker Desktop for Windows 3.x 的发行说明。
...
- 修复了 DNS 服务器在收到意外大的数据报文后会失败的错误。
- 修复了 iptables 更新时出现的虚假跟踪。
- 修复了添加多个端口转发选项时的延迟问题。
- 修复了当 WSL 2 主目录与 Windows 主目录相同时，WSL 2 同步代码会创建悬空符号链接的错误。修复了 [docker/for-win#11668](https://github.com/docker/for-win/issues/11668)。
...
- 修复了当 WSL 2 下的 Linux 主目录被设置为 Windows 主目录（例如 `/mnt/c/Users/...`）的错误。
- 修复了如果无法解析 CLI 上下文，Desktop 将无法启动的错误。修复了 [docker/for-win#11601](https://github.com/docker/for-win/issues/11601)。
...
- [Compose V2 beta 6](https://github.com/docker/compose-cli/releases/tag/v2.0.0-beta.6)
  - `compose run` 和 `compose exec` 命令对 stdout 和 stderr 使用单独的流。参见 [docker/compose-cli#1873](https://github.com/docker/compose-cli/issues/1873)。
...
- 开发环境：修复了在某些创建和删除场景中出现空白屏幕的问题。修复了 [dev-environments#4](https://github.com/docker/dev-environments/issues/4)。
- 开发环境：修复了删除环境时的错误处理。修复了 [dev-environments#8](https://github.com/docker/dev-environments/issues/8)。
- 开发环境：在创建或删除环境时，**开始**、**停止**和**共享**按钮将被禁用。
...
**Compose V2 beta**：Docker Desktop 现在包含 Compose V2 的测试版，该版本支持作为 Docker CLI 一部分的 `docker compose` 命令。虽然 `docker-compose` 仍然受支持和维护，但 Compose V2 的实现直接依赖于作为规范一部分维护的 compose-go 绑定。Docker CLI 中的 compose 命令支持大多数 `docker-compose` 命令和标志。它有望成为 `docker-compose` 的直接替代品。还有一些标志尚未实现，请参阅 [docker-compose 兼容性列表](/manuals/compose/releases/migrate.md) 以获取有关新 compose 命令中支持的标志的更多信息。如果在使用 Compose V2 时遇到任何问题，可以通过在 Docker Desktop **实验性**设置中进行更改，或运行命令 `docker-compose disable-v2` 轻松切换回 Compose v1。请通过在 [Compose-CLI](https://github.com/docker/compose-cli/issues) GitHub 仓库中创建问题，让我们了解您对新 'compose' 命令的反馈。

### Bug 修复和次要更改

- 修复了当临时文件夹路径包含点时用户无法安装 Docker Desktop 的错误。修复了 [docker/for-win#11514](https://github.com/docker/for-win/issues/11514)。
...
- 修复了由于 Hyper-V VM 过早重启而导致断电后 Docker Desktop 无法重新启动的错误。
...
- 修复了在 VM IP 更改后 `*.docker.internal` 的 DNS 条目和 Kubernetes 集群重置的问题。
...
- 移除了“默认将 Docker Stacks 部署到 Kubernetes”的 Kubernetes 设置。该组件已在 2.4.0.0 中移除，但我们忘记了移除该设置。修复了 [docker/for-mac#4966](https://github.com/docker/for-mac/issues/4966)。
...
出于专业开发目的使用 Docker Desktop 的开发人员有时可能需要跳过特定的更新。因此，拥有付费 Docker 订阅的用户可以在出现提醒时跳过特定更新的通知。

对于在 IT 管理环境中工作且没有管理权限来安装 Docker Desktop 更新的开发人员，如果您的 Docker ID 是团队订阅的一部分，现在可以在设置菜单中选择完全退出 Docker Desktop 更新的通知。
...
- 修复了当在主机上使用随机端口运行容器时，导致 Docker Desktop 仪表板错误地使用端口 0 打开浏览器，而不是使用已分配端口的问题。
...
- Docker 仪表板在您启动 Docker Desktop 时自动打开。
- Docker 仪表板每周显示一条提示。
- BuildKit 现在是所有用户的默认构建器，而不仅仅是新安装的用户。要关闭此设置，请转到**设置** > **Docker 引擎**，并将以下块添加到 Docker 守护进程配置文件：
```json
"features": {
    "buildkit": false
}
...
- 移除了已弃用的 DNS 名称 `docker.for.win.localhost`。在容器中使用 DNS 名称 `host.docker.internal` 来访问在主机上运行的服务。[docker/for-win#10619](https://github.com/docker/for-win/issues/10619)

### Bug 修复和次要更改

- 修复了在容器详细信息屏幕上滚动日志时按钮会消失的问题。修复了 [docker/for-win#10160](https://github.com/docker/for-win/issues/10160)。
...
- 修复了具有长容器/镜像名称的容器实例视图。修复了 [docker/for-win#10160](https://github.com/docker/for-win/issues/10160)。
...
- 修复了在升级到 3.0.0 后可能导致 Docker Desktop 无法启动的问题。修复了 [docker/for-win#9755](https://github.com/docker/for-win/issues/9755)。
...
- `docker compose` 的第一个版本（作为现有 `docker-compose` 的替代品）。支持一些基本命令，但尚未具备 `docker-compose` 的完整功能。
  ...
  - 支持基本卷、绑定挂载、网络和环境变量
    ...
    请通过在 [compose-cli](https://github.com/docker/compose-cli/issues) GitHub 仓库中创建问题，让我们了解您的反馈。
...
- 修复了当尝试使用 `-v /var/run/docker.sock:` 启动不存在的容器时出现意外 EOF 错误的问题。参见 [docker/for-mac#5025](https://github.com/docker/for-mac/issues/5025)。
