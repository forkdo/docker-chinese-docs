...
- **卷管理 (Volume Management)**：现在可以按名称、创建日期和卷的大小对卷进行排序。还可以使用**搜索**字段搜索特定卷。更多信息，请参阅[探索卷](../use-desktop/volumes.md)。
...
- 修复了在收到意外大的数据报文后 DNS 服务器会失败的错误。
- 修复了与硬件加速相关的问题 [docker/for-mac#5121](https://github.com/docker/for-mac/issues/5121)
- 修复了与 mac 上的“跳过此更新”相关的问题 [docker/for-mac#5842](https://github.com/docker/for-mac/issues/5842)
...
**开发环境预览 (Dev Environments Preview)**：开发环境使您能够与团队成员无缝协作，无需在 Git 分支之间切换即可将代码放到团队成员的机器上。使用开发环境时，只需单击一下即可与团队成员共享进行中的工作，而无需处理任何合并冲突。
...
- [Compose V2 beta](https://github.com/docker/compose-cli/releases/tag/v2.0.0-beta.4)
  - 修复了当文件绑定挂载到嵌套挂载点时容器无法启动的错误。修复了 [docker/compose-cli#1795](https://github.com/docker/compose-cli/issues/1795)。
  - 添加了对容器链接和外部链接的支持。
...
- 修复了由于 `/usr/bin` 不在 `PATH` 上而导致 Docker 启动失败的错误。修复了 [docker/for-mac#5770](https://github.com/docker/for-mac/issues/5770)
- Docker Desktop 现在允许修改主机目录中的文件，该目录是容器中的嵌套挂载点。修复了 [docker/for-mac#5748](https://github.com/docker/for-mac/issues/5748)。
- 修复了设置迁移错误，该错误导致 Docker Desktop 在升级到 3.4.0 后找不到镜像和容器数据。修复了 [docker/for-mac#5754](https://github.com/docker/for-mac/issues/5754)。
...
**跳过 Docker Desktop 更新**：所有用户现在都可以在提示安装单个 Docker Desktop 版本时跳过更新。
...
- 文件名不是 `docker-compose.yml` 的 Docker Compose 应用程序现在可以从 Docker Desktop 中删除。修复了 [docker/for-win#11046](https://github.com/docker/for-win/issues/11046)
...
- 当启用 gRPC FUSE 时，避免运行 `com.docker.osxfs`。修复了 [docker/for-mac#5725](https://github.com/docker/for-mac/issues/5725)。

### 已知问题

- 在 Apple Silicon 的原生 `arm64` 容器中，`debian:buster`、`ubuntu:20.04` 和 `centos:8` 中的旧版本 `libssl` 在连接到某些 TLS 服务器时会出现段错误，例如 `curl https://dl.yarnpkg.com`。该错误已在 `debian:bullseye`、`ubuntu:21.04` 和 `fedora:35` 的新版本 `libssl` 中修复。
...
- 修复了在 Apple 芯片上网络传输中偶尔丢失最后一个字节的错误。
- 修复了引擎 `daemon.json` 中定义的 `metrics-port` 阻止应用程序重启的错误。
- 修复了临时端口泄露的问题。修复了 [docker/for-mac#5611](https://github.com/docker/for-mac/issues/5611)。
- 在 Apple 芯片上使用 `qemu` 模拟更现代的 Intel CPU，以获得更好的镜像兼容性。请参阅 [docker/for-mac#5561](https://github.com/docker/for-mac/issues/5561)。
- 默认启用 buildkit 垃圾回收。
- 修复了阻止绑定到端口 123 的错误。修复了 [docker/for-mac#5589](https://github.com/docker/for-mac/issues/5589)。
...
Docker Desktop 现在可用于 Apple silicon 以及 Intel 芯片。这使开发人员可以选择本地开发环境，并扩展了基于 ARM 的应用程序的开发管道。更多信息，请参阅 [适用于 Apple silicon 的 Docker Desktop](/manuals/desktop/setup/install/mac-install.md)。
...
- 修复了当无法在后端与 Docker Hub 建立连接时导致 Docker Desktop 在启动期间失败的问题。修复了 [docker/for-win#10896](https://github.com/docker/for-win/issues/10896)

- **Apple silicon 的 Mac**：Docker Desktop 现在减少了空闲 CPU 消耗。

### 已知问题

**Apple silicon**

- 从容器内部 ping 互联网不能按预期工作。为了测试网络，我们建议使用 `curl` 或 `wget`。请参阅 [docker/for-mac#5322](https://github.com/docker/for-mac/issues/5322#issuecomment-809392861)。
- 当 TCP 流半关闭时，用户可能会偶尔遇到数据丢失。
...
出于专业开发目的使用 Docker Desktop 的开发人员有时可能需要跳过特定更新。因此，拥有付费 Docker 订阅的用户可以在出现提醒时跳过特定更新的通知。

对于在 IT 管理环境中工作的开发人员，他们没有管理权限来安装 Docker Desktop 更新，现在设置菜单中有一个选项，如果您的 Docker ID 是团队订阅的一部分，可以选择完全退出 Docker Desktop 更新的通知。
...
- 修复了在查看使用显式项目名称启动的 compose 应用程序时的问题。修复了 [docker/for-win#10564](https://github.com/docker/for-win/issues/10564)。
- 修复了 `--add-host host.docker.internal:host-gateway` 导致 `host.docker.internal` 解析到错误 IP 地址的错误。请参阅 [docker/for-linux#264](https://github.com/docker/for-linux/issues/264#issuecomment-785137844)。
- 修复了导致容器间 HTTP 流量被错误路由到外部 HTTP 代理的错误。修复了 [docker/for-mac#5476](https://github.com/docker/for-mac/issues/5476)。
- 修复了在调整磁盘大小时可能导致与 VM 磁盘在同一文件夹中的其他文件被删除的错误。修复了 [docker/for-mac#5486](https://github.com/docker/for-mac/issues/5486)。
- 修复了增量下载导致 `Illegal instruction exception` 的问题。修复了 [docker/for-mac#5459](https://github.com/docker/for-mac/issues/5459)。
- 对加密连接应用基于域的 HTTPS 代理 `no_proxy` 规则。修复了 [docker/for-mac#2732](https://github.com/docker/for-mac/issues/2732)。
- 修复了恢复出厂默认设置对话框中缺少文本的问题。修复了 [docker/for-mac#5457](https://github.com/docker/for-mac/issues/5457)。
- 修复了在主机上使用随机端口运行容器时导致 Docker Desktop 仪表板错误地使用端口 0 打开浏览器，而不是使用分配的端口的问题。
...
- 修复了在 iTerm2 没有其他窗口打开时在其中打开终端的问题。修复了 [docker/roadmap#98](https://github.com/docker/roadmap/issues/98#issuecomment-791927788)。
...
- 修复了在更新到版本 3.2.0 后有时导致 Docker Desktop 启动失败的问题。修复了 [docker/for-mac#5406](https://github.com/docker/for-mac/issues/5406)。如果您在尝试从 3.2.0 更新到 3.2.1 时仍然遇到此问题，我们建议您卸载 3.2.0 并手动安装 Docker Desktop 3.2.1。
...
- 添加了使用新 Apple 虚拟化框架的实验性支持（需要 macOS Big Sur 11.1 或更高版本）
...
- 修复了在容器详细信息屏幕上滚动日志时按钮会消失的问题。修复了 [docker/for-mac#5290](https://github.com/docker/for-mac/issues/5290)
- 修复了在 IPv6 容器网络中转发多个端口时的问题。修复了 [docker/for-mac#5247](https://github.com/docker/for-mac/issues/5247)
- 修复了 `docker load` 无法再使用 xz 存档的回归问题。修复了 [docker/for-mac#5271](https://github.com/docker/for-mac/issues/5271)
- 修复了**容器/应用**视图中的导航问题。修复了 [docker/for-win#10160](https://github.com/docker/for-win/issues/10160#issuecomment-764660660)
- 修复了容器/镜像名称较长时的容器实例视图。修复了 [docker/for-mac#5290](https://github.com/docker/for-mac/issues/5290)
...
Docker Desktop 有时在更新到版本 3.2.0 后无法启动。如果您遇到此问题，我们建议您卸载 3.2.0 并手动安装 [Docker Desktop 3.2.1](#docker-desktop-321)。请参阅 [docker/for-mac#5406](https://github.com/docker/for-mac/issues/5406)。
...
- 通过使缓存失效更快来避免在 `docker-compose up` 期间超时。修复了 [docker/for-mac#4957](https://github.com/docker/for-mac/issues/4957)。
- 在使缓存失效时避免生成虚假的文件系统 DELETE 事件。修复了 [docker/for-mac#5124](https://github.com/docker/for-mac/issues/5124)。

### 已知问题

- 某些 DNS 地址在基于 Alpine Linux 3.13 的容器内无法解析。请参阅 [docker/for-mac#5020](https://github.com/docker/for-mac/issues/5020)。
...
- 修复了导致重叠卷挂载失败的问题。修复了 [docker/for-mac#5157](https://github.com/docker/for-mac/issues/5157)。但是，[docker/for-mac#4957](https://github.com/docker/for-mac/issues/4957) 和 [docker/for-mac#5124](https://github.com/docker/for-mac/issues/5124) 的修复因此更改而被撤销，因此这些问题现在再次出现。

### 已知问题

- 某些 DNS 地址在基于 Alpine Linux 3.13 的容器内无法解析。请参阅 [docker/for-mac#5020](https://github.com/docker/for-mac/issues/5020)。
- 如果有多个服务启动，在 docker-compose up 期间可能会超时。请参阅 [docker/for-mac#4957](https://github.com/docker/for-mac/issues/4957) 和 [docker/for-mac#5124](https://github.com/docker/for-mac/issues/5124)。
...
- 通过使缓存失效更快来避免在 `docker-compose up` 期间超时。修复了 [docker/for-mac#4957](https://github.com/docker/for-mac/issues/4957)。
- 在使缓存失效时避免生成虚假的文件系统 DELETE 事件。修复了 [docker/for-mac#5124](https://github.com/docker/for-mac/issues/5124)。
- 现在可以与容器共享 `~/Library` 中的目录（Docker Desktop 数据目录除外）。修复了 [docker/for-mac#5115](https://github.com/docker/for-mac/issues/5115)。
...
- 修复了一个可能的文件共享错误，即当文件在主机上修改时，它在容器中可能显示为错误的大小。这是对 [docker/for-mac#4999](https://github.com/docker/for-mac/issues/4999) 的部分修复。
- 删除了不必要的日志消息，这些消息会减慢文件系统事件注入的速度。
- 重新启用了实验性 SOCKS 代理。修复了 [docker/for-mac#5048](https://github.com/docker/for-mac/issues/5048)。
- 修复了在尝试使用 `-v /var/run/docker.sock:` 启动不存在的容器时出现意外 EOF 错误的问题。请参阅 [docker/for-mac#5025](https://github.com/docker/for-mac/issues/5025)。
- 当应用程序需要对特定目录的写访问权限时，显示错误消息而不是崩溃。请参阅 [docker/for-mac#5068](https://github.com/docker/for-mac/issues/5068)
