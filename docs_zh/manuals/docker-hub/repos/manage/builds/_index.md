...
根据您的[订阅计划](https://www.docker.com/pricing?ref=Docs&refAction=DocsHubRepoBuilds)，您可能会获得并发构建能力，这意味着可以同时运行 `N` 个自动构建。`N` 的值根据您的订阅计划进行配置。一旦有 `N+1` 个构建正在运行，任何额外的构建将进入队列，稍后运行。
