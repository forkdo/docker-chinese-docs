...
4. 查看 **Docker Hub Builder** OAuth 应用的设置。
    ...
    > [!NOTE]
    >
    > 如果您是任何 GitHub 组织的所有者，您可能会在此屏幕上看到选项，允许从此处授予 Docker Hub 访问这些组织的权限。您也可以单独编辑组织的第三方访问设置，以授予或撤销 Docker Hub 的访问权限。请参阅
...
如果组织之前未明确授予或撤销访问权限，您通常可以在链接用户账户的同时授予访问权限。在这种情况下，链接账户屏幕中组织名称旁边会出现一个 **Grant access** 按钮，如下所示。如果此按钮未出现，您必须手动授予应用程序的访问权限。
...
要永久撤销 Docker Hub 对您的 GitHub 账户的访问权限，您必须同时从 Docker Hub 和您的 GitHub 账户中取消链接。
...
要永久撤销 Docker Hub 对您的 Bitbucket 账户的访问权限，您必须同时从 Docker Hub 取消链接，并在您的 Bitbucket 账户中撤销授权。
