---
title: 在 Docker Home 中配置设置管理
linkTitle: 使用 Docker Home
description: 在 Docker Home 中为整个组织配置和强制执行 Docker Desktop 设置
...
使用 Docker Home 为整个组织的 Docker Desktop 创建和管理设置策略。设置策略可让您标准化配置、强制执行安全要求并保持一致的 Docker Desktop 环境。

## 先决条件

在开始之前，请确保您已具备：

- 已安装 [Docker Desktop](/manuals/desktop/release-notes.md)
- [已验证的域名](/enterprise/security/single-sign-on/connect/#step-1-add-a-domain)
- 为您的组织[强制执行登录](/manuals/enterprise/security/enforce-sign-in/_index.md)
- Docker Business 订阅

> [!IMPORTANT]
>
> 您可以随时创建设置管理策略，但您的组织需要先验证域名，策略才会生效。
...
1. 选择 **Docker Desktop**，然后选择 **Settings Management**。
...
1. 提供名称和可选描述。
   ...
   > Docker Home 策略会覆盖本地的 `admin-settings.json` 文件。
...
1. 使用以下状态配置每个设置：
     ...
     | Docker Home 状态 | 描述                               | `admin-settings.json` 等效       |
     | :------------------ | :--------------------------------- |:---------------------------------- |
     | **User-defined**    | 用户可以更改该设置       | 省略该设置                   |
     | **Always enabled**  | 设置开启并锁定           | `"value": true`, `"locked": true`  |
     | **Enabled**         | 设置开启但可以更改   | `"value": true`, `"locked": false` |
     | **Always disabled** | 设置关闭并锁定          | `"value": false`, `"locked": true` |
     | **Disabled**        | 设置关闭但可以更改  | `"value": false`, `"locked": false`|
...
在 Docker Home 的 **Desktop Settings Management** 页面中，使用 **Actions** 菜单可以：
