...
title: 增强的容器隔离
linkTitle: 增强的容器隔离
description: 增强的容器隔离（ECI）通过防止恶意容器破坏 Docker Desktop 或主机系统，为 Docker Desktop 提供额外的安全保护
keywords: 增强的容器隔离, 容器安全, sysbox 运行时, linux 用户命名空间, 硬化桌面
aliases:
...
增强的容器隔离（ECI）通过防止基于容器的攻击并减少开发者环境中的安全漏洞，保护 Docker Desktop 和主机系统免受破坏。它自动应用高级安全技术，同时保持完整的开发者生产力和工作流兼容性。

ECI 加强了容器隔离，并锁定管理员创建的安全配置，例如 [注册表访问管理策略](/manuals/enterprise/security/hardened-desktop/registry-access-management.md) 和 [设置管理](../settings-management/_index.md) 控件。
...
- 需要在不影响开发者工作流的情况下实现更强容器隔离的安全团队
- 需要额外保护以运行不受信任或第三方容器镜像的企业

## 增强的容器隔离如何工作

Docker 使用 [Sysbox 容器运行时](https://github.com/nestybox/sysbox) 实现 ECI，这是一个
安全增强版的标准 OCI runc 运行时的分支。启用 ECI 后，通过 `docker run` 或 `docker create` 创建的容器自动使用 Sysbox 而非 runc，无需更改开发者工作流。

即使使用 `--privileged` 标志的容器也能通过增强的容器隔离安全运行，防止它们破坏 Docker Desktop 虚拟机或其他容器。

> [!NOTE]
>
> 启用 ECI 后，Docker CLI 的 `--runtime` 标志将被忽略。
...
启用增强的容器隔离后，所有容器利用 Linux 用户命名空间实现更强的隔离。容器中的 root 用户映射到 Docker Desktop VM 中的非特权用户：
...
通过使用 Linux 用户命名空间，ECI 确保容器进程在 Linux VM 中从不以有效用户 ID 运行，将其能力限制在容器内的资源。
...
增强的容器隔离通过确保特权容器只能访问容器边界内的资源来转换它们。例如，特权容器无法访问 Docker Desktop 的网络配置：
...
增强的容器隔离防止容器与 Docker Desktop VM 共享 Linux 命名空间，维持隔离边界：
...
增强的容器隔离在防止访问敏感 VM 目录的同时，保持对标准文件共享的支持：
...
ECI 通过仅拦截控制路径系统调用（很少使用）而让数据路径系统调用不受影响，高效执行系统调用过滤，保持容器性能。

### 自动文件系统用户 ID 映射

增强的容器隔离通过自动文件系统映射解决具有不同用户 ID 范围的容器之间的文件共享挑战。

每个容器获得独占的用户 ID 映射，但 Sysbox 使用 Linux 内核 ID 映射挂载（2021 年添加）或替代的 shiftsfs 模块进行文件系统用户 ID 重映射。这将容器的实用户 ID 映射到标准范围的文件系统访问，实现：
...
ECI 在容器内模拟 `/proc` 和 `/sys` 文件系统的部分，以隐藏敏感主机信息并为内核资源提供每个容器的视图：
...
几个 Linux 内核未命名空间化的 `/proc/sys` 资源被每个容器模拟，Sysbox 在编程内核设置时协调值。这使得通常需要特权访问的容器工作负载能够安全运行。

## 性能和兼容性

增强的容器隔离保持优化的性能和完全的兼容性：

- 无性能影响：系统调用过滤仅针对控制路径调用，不影响数据路径操作
- 完全的工作流兼容性：现有的开发流程、工具和容器镜像无需更改即可工作
...
> [!IMPORTANT]
>
> ECI 保护因 Docker Desktop 版本而异，目前不保护扩展容器。Docker 构建和 Docker Desktop 中的 Kubernetes 具有因版本而异的保护级别。详细信息请参阅 [增强的容器隔离限制](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/limitations.md)。