...
keywords: 增强型容器隔离, 限制, wsl, hyper-v, kubernetes, docker build
toc_max: 3
weight: 30
aliases:
...
增强型容器隔离（Enhanced Container Isolation，简称 ECI）存在一些平台特定的限制和功能约束。了解这些限制有助于您规划安全策略并设定合理的预期。
...
增强型容器隔离的安全级别取决于您的 Windows 后端配置。

下表比较了 WSL 2 上的 ECI 和 Hyper-V 上的 ECI：

| 安全功能                                   | WSL 上的 ECI   | Hyper-V 上的 ECI   | 说明               |
| ---------------------------------------- | ------------ | ---------------- | ----------------- |
| 强化安全的容器                             | 是           | 是               | 使恶意容器工作负载更难攻破 Docker Desktop Linux VM 和主机。 |
| Docker Desktop Linux VM 受保护，用户无法直接访问 | 否           | 是               | 在 WSL 上，用户可以直接访问 Docker Engine 或绕过 Docker Desktop 安全设置。 |
| Docker Desktop Linux VM 拥有独立内核     | 否           | 是               | 在 WSL 上，Docker Desktop 无法保证内核级配置的完整性。 |

WSL 2 的安全漏洞包括：

- 直接 VM 访问：用户可以通过直接访问 VM 来绕过 Docker Desktop 安全设置：`wsl -d docker-desktop`。这使用户获得 root 权限，可修改 Docker Engine 设置并绕过设置管理配置。
...
Docker Build 保护取决于驱动和 Docker Desktop 版本：

| 构建驱动 | 保护 | 版本要求 |
|:------------|:-----------|:---------------------|
| `docker`（默认） | 受保护 | Docker Desktop 4.30 及以后版本（WSL 2 除外） |
| `docker`（旧版） | 未受保护 | Docker Desktop 4.30 之前的版本 |
| `docker-container` | 始终受保护 | 所有 Docker Desktop 版本 |

以下 Docker Build 功能在 ECI 中不工作：
...
集成的 Kubernetes 功能无法受益于 ECI 保护。恶意或特权 Pod 可能攻破 Docker Desktop VM 并绕过安全控制。
...
启用 ECI 后，每个 Kubernetes 节点都在 ECI 保护的容器中运行，从而提供与 Docker Desktop VM 的更强隔离。

## 未受保护的容器类型

以下容器类型目前无法受益于 ECI 保护：

- Docker Extensions：扩展容器在 ECI 保护之外运行
- Docker Debug：Docker Debug 容器绕过 ECI 限制
- Kubernetes Pod：使用 Docker Desktop 集成 Kubernetes 时

### 建议

仅使用来自可信来源的扩展，并在安全敏感环境中避免使用 Docker Debug。
...
除非满足以下条件，否则无法允许任意本地镜像（不在注册表中的镜像）挂载 Docker 套接字：
...
当注册表中的允许镜像更新时，本地容器可能在您刷新本地镜像之前被意外阻止：
...
- Docker Desktop 4.36 及以后版本：支持通配符允许列表（`"*"`）和改进的派生镜像处理
- Docker Desktop 4.34 及以后版本：支持派生镜像（`allowDerivedImages`）
- Docker Desktop 4.30 及以后版本：默认驱动的 Docker Build 保护（WSL 2 除外）
...
大多数容器在有无 ECI 的情况下运行相同。然而，一些高级工作负载的行为可能不同：
...
使用 Sysbox 运行时（配合 ECI）的容器与生产环境中的标准 OCI runc 运行时可能存在细微差异。这些差异通常只影响特权或系统级操作。