...
keywords: 镜像访问管理, docker 官方镜像, 已验证发布者, 供应链安全, docker 企业版, 允许列表, 镜像限制, 拉取限制
tags: [admin]
aliases:
...
您还可以使用仓库允许列表（allowlist）来批准绕过所有其他访问控制的具体仓库。
...
- [强制登录](/manuals/enterprise/security/enforce-sign-in/_index.md)。镜像访问管理仅在用户使用组织凭据登录 Docker Desktop 时生效。
...
> [!NOTE]
>
> 镜像访问管理默认对组织成员关闭。无论策略设置如何，组织所有者始终可以访问所有镜像。

要配置镜像访问管理：

1. 登录 [Docker Home](https://app.docker.com) 并从左上角账户下拉菜单中选择您的组织。
2. 选择 **Docker Desktop**，然后 **Image Access**。
3. 使用 **切换** 启用镜像访问。
4. 选择要允许的镜像类型：
    ...
    - **仓库允许列表（allowlist）**：应允许的具体仓库列表。在下一步中配置。
5. 如果在上一步中启用了 **仓库允许列表（allowlist）**，您可以在允许列表中添加或删除具体仓库：
    - 要添加仓库，在 **仓库允许列表（allowlist）** 部分，选择 **添加仓库到允许列表（Add repositories to allowlist）** 并按照屏幕上的说明操作。
    - 要删除仓库，在 **仓库允许列表（allowlist）** 部分，选择其旁边的垃圾桶图标。
...
- 从最严格的策略开始，并根据合法的业务需求逐步扩展：
    1. 从 Docker 官方镜像和组织镜像开始
    2. 如果需要，添加 Docker 已验证发布者镜像用于商业工具
    3. 仔细评估社区镜像，仅用于特定、经过审查的用例
    4. 谨慎使用仓库允许列表（allowlist）。仅添加已通过组织安全审查流程彻底审查和批准的仓库
- 监控使用模式：审查开发人员尝试拉取的镜像，识别对额外镜像类型的合法请求，定期审计批准的镜像类别以保持相关性，并使用 Docker Desktop 分析来监控使用模式。
- 定期审查仓库允许列表（allowlist）：定期审计允许列表中的仓库，确保它们仍然是必要和可信的，并删除任何不再需要或维护的仓库。
...
- 分层安全控制：镜像访问管理与 [注册表访问管理](registry-access-management.md) 结合使用效果最佳，以控制开发人员可以访问哪些注册表，与 [增强容器隔离](enhanced-container-isolation/_index.md) 结合以在运行时保护容器，以及与 [设置管理](settings-management/_index.md) 结合以控制 Docker Desktop 配置。
