...
keywords: 自定义角色, 权限, 访问控制, 组织管理, docker hub, docker home, 安全
---
...
4. 通过展开权限类别并选中权限的复选框来设置角色的权限。有关可用权限的完整列表，请参阅[自定义角色权限参考](#custom-roles-permissions-reference)。
5. 选择 **Review**（审查）以审查您的自定义角色配置并查看所选权限的摘要。
6. 选择 **Create**（创建）。
...
1. 登录 [Docker Home](https://app.docker.com)。
2. 选择 **Roles**（角色）。
3. 从列表中找到您的自定义角色，然后选择 **Actions menu**（操作菜单）。
4. 选择 **Edit**（编辑）。
5. 您可以编辑以下自定义角色设置：
   - Label（标签）
   - Description（描述）
...
1. 登录 [Docker Home](https://app.docker.com)。
2. 选择 **Roles**（角色）。
3. 在角色列表中，查看 **Users**（用户）和 **Teams**（团队）列以查看分配数量。
4. 选择特定角色以详细查看其权限和分配情况。
...
5. 一旦没有用户或团队被分配，请返回 **Roles**（角色）。
6. 找到您的自定义角色并选择 **Actions menu**（操作菜单）。
7. 选择 **Delete**（删除）。
8. 在确认窗口中，选择 **Delete**（删除）以确认。
...
| 权限 (Permission)      | 描述 (Description)                                          |
| :-------------- | :--------------------------------------------------- |
| View policies   | 查看现有的 AI 治理策略和规则 |
| Manage policies | 完全访问 AI 治理策略管理       |
