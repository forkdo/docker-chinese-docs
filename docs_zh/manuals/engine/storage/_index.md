...
镜像挂载可在你选择的路径下，将另一个镜像的内容提供给容器内部使用。被挂载的镜像是只读的，且不属于容器自身的镜像，因此你可以从一个镜像引入工具或资源，而无需重新构建另一个镜像。

当你需要使用以镜像形式打包的文件时，请使用镜像挂载，例如挂载一个工具丰富的镜像来调试极简容器，或者在运行不同镜像的容器之间共享只读资源。镜像挂载需要 [containerd 镜像存储](containerd.md)。
