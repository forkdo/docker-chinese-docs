...
任务是一种单向机制。它通过一系列状态单调地演进：`ASSIGNED`、`PREPARING`、`RUNNING` 等。如果任务失败，编排器会移除该任务及其容器，然后根据服务指定的期望状态创建一个新任务来替换它。

Docker Swarm 模式的底层逻辑是一个通用调度器和编排器。服务和任务抽象本身并不知道它们所实现的容器。理论上，你可以实现其他类型的任务，例如虚拟机任务或非容器化进程任务。调度器和编排器对任务的类型是不可知的。然而，Docker 仅支持容器任务。
