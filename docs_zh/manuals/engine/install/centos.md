...
1. 安装 Docker 包。
   ...
   如果提示接受 GPG 密钥，请验证指纹是否匹配 `060A 61C5 1B55 8A7F 742B 77AA C52F EB6B 621E 9F35`，如果匹配，请接受它。
   ...
   通过其完全限定的包名安装特定版本，即包名 (`docker-ce`) 加上版本字符串（第 2 列），用连字符 (`-`) 分隔。例如，`docker-ce-3:{{% param "docker_ce_version" %}}-1.el9`。
...
1. 转到 [{{% param "download-url-base" %}}/]({{% param "download-url-base" %}}/) 并选择您的 CentOS 版本。然后浏览到 `x86_64/stable/Packages/` 并下载您要安装的 Docker 版本的 `.rpm` 文件。
