...
- 默认情况下，套接字路径设置为 `$XDG_RUNTIME_DIR/docker.sock`。
  `$XDG_RUNTIME_DIR` 通常设置为 `/run/user/$UID`。
- 默认情况下，数据目录设置为 `~/.local/share/docker`。
  数据目录不应位于 NFS 上。
- 默认情况下，守护进程配置目录设置为 `~/.config/docker`。
...
在 Docker Engine v23.0 之前，用户必须显式指定套接字路径或 CLI 上下文。
...
要在"有根" Docker 中运行 Rootless Docker，请使用 `docker:<version>-dind-rootless`
...
`docker:<version>-dind-rootless` 镜像以非 root 用户（UID 1000）身份运行。
...
要通过 SSH 暴露 Docker API 套接字，您需要确保 `$DOCKER_HOST`
在远程主机上已设置。
...
仅在使用 cgroup v2 和 systemd 运行时才支持使用 cgroup 相关的 `docker run` 标志（如 `--cpus`、`--memory`、`--pids-limit`）限制资源。
请参阅 [更改 cgroup 版本](/manuals/engine/containers/runmetrics.md) 以启用 cgroup v2。

如果 `docker info` 显示 `Cgroup Driver` 为 `none`，则不满足条件。
当不满足这些条件时，rootless 模式会忽略 cgroup 相关的 `docker run` 标志。
有关解决方法，请参阅 [无 cgroup 时限制资源](#limiting-resources-without-cgroup)。
...
尽管它们以进程粒度而不是容器粒度工作，并且可以被容器进程随意禁用。

例如：

- 将 CPU 使用率限制为 0.5 个核心（类似于 `docker run --cpus 0.5`）：
...
- 将每个命名空间 UID 2000 的最大进程数限制为 100（类似于 `docker run --pids-limit=100`）：
