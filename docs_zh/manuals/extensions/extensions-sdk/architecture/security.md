...
请注意，扩展并不局限于在扩展元数据的 [host 部分](../architecture/metadata.md#host-section) 中列出的二进制文件：由于这些二进制文件可以包含以用户身份运行的任何代码，只要用户有权限，它们就可以进一步执行任何其他命令。

Extensions SDK 提供了一组 JavaScript API，用于从扩展 UI 代码中调用命令或执行这些二进制文件。扩展还可以提供一个后端部分，在后台启动一个长期运行的容器。
