...
linkTitle: 构建流程
description: 了解 Docker 如何通过自动化、以安全为中心的流水线来构建、测试和维护 Docker 硬化镜像。
...
本文档解释了基础 DHI 镜像以及 DHI Select 和 DHI Enterprise 订阅所提供的定制镜像的构建流程。

对于 DHI Select 或 DHI Enterprise 订阅用户，基础镜像和定制镜像的自动化安全更新流水线均受 [SLA 承诺](https://docs.docker.com/go/dhi-sla/)保障，包括针对关键和高危漏洞的 7 天 SLA。DHI Community 提供安全基线，但不保证修复时间。

## 构建透明度

Docker 硬化镜像通过公开可用的定义和可验证的证明，为镜像的构建方式提供透明度。

### 镜像定义

所有镜像定义均在[目录仓库](https://github.com/docker-hardened-images/catalog)中公开提供。

每个镜像定义都是一份声明式 YAML 规范，包含元数据、内容、构建流水线步骤、安全配置和运行时设置。

### SLSA 证明

每个 Docker 硬化镜像都包含 SLSA 构建级别 3 证明，提供可验证的构建来源。有关 SLSA 证明及其验证方式的详细信息，请参阅 [SLSA](security-concepts/slsa.md)。
...
当您使用 DHI Select 或 DHI Enterprise 定制 DHI 镜像时，您的变更会被打包为叠加在基础镜像之上的 OCI 制品。Docker 会监控您的制品仓库，并在您推送更新时自动重建您的定制镜像。
...
5.  **测试与扫描**：镜像会经过全面的兼容性和功能性[测试](test.md)，并会进行[恶意软件扫描](malware-scanning.md)以及密钥和漏洞扫描。
...
Docker 能快速响应关键漏洞。通过从源代码构建关键组件，而不是等待打包更新，Docker 可以在上游修复后的几天内修补关键和高危 CVE，并发布带有新证明的更新镜像。对于 DHI Enterprise 订阅用户，这种快速响应受[关键和高危漏洞的 7 天 SLA](https://docs.docker.com/go/dhi-sla/) 保障。
...
当您使用 DHI Select 或 DHI Enterprise 定制 DHI 镜像时，构建过程得以简化：

1.  **监控**：Docker 监控您的 OCI 制品仓库的变更。
2.  **重建触发**：当您向 OCI 制品推送更新，或基础 DHI 镜像更新时，自动重建开始。
3.  **获取基础镜像**：获取最新的基础 DHI 镜像。
4.  **应用定制**：将您的 OCI 制品应用到基础镜像。
5.  **扫描**：对定制镜像进行[恶意软件扫描](malware-scanning.md)以及密钥和漏洞扫描。
6.  **签名和证明**：Docker 对定制镜像进行签名，并生成证明（SBOM、VEX 文档、构建来源）。
7.  **发布**：签名的定制镜像发布到 Docker Hub，证明发布到 Docker Scout 注册表。
