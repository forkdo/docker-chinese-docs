...
description: 遵循快速开始指南，探索并运行 Docker Hardened Image。
weight: 2
keywords: docker hardened images quickstart, run secure image
---

本指南通过一个真实示例，展示如何从零开始运行 Docker Hardened Image（DHI）。在最后，你将对比 DHI 与标准 Docker 镜像，以更好地理解它们之间的差异。虽然步骤使用了特定镜像作为示例，但这些步骤适用于任何 DHI。

> [!NOTE]
>
> Docker Hardened Images 对所有人免费提供，无需订阅，无使用限制，无厂商锁定。当你需要 FIPS 或 STIG 合规变体、自定义能力或 SLA 支持等企业功能时，可升级至 DHI Enterprise 订阅。

## 步骤 1：找到要使用的镜像

1. 前往 [Docker Hub](https://hub.docker.com/hardened-images/catalog) 中的 Hardened Images 目录并登录。
2. 在左侧边栏中，选择 **Hardened Images**。如果你有 DHI Enterprise，则选择 **Hardened Images** > **Catalog**。
3. 使用搜索栏或筛选器查找镜像（例如 `python`、`node`、`golang`）。本指南以 Python 镜像为例。
...
你可以像拉取和运行任何其他 Docker 镜像一样拉取和运行 DHI。注意，Docker Hardened Images 设计为最小化且安全，因此它们可能不包含你期望的典型镜像中的所有工具或库。你可以在 [采用 DHIs 时的注意事项](./how-to/use.md#considerations-when-adopting-dhis) 中查看典型差异。
...
> [!NOTE]
>
> 这是示例输出。你的结果可能因新发现的 CVE 和镜像更新而有所不同。
>
> Docker 在 Docker Hardened Images 中维持接近零的 CVE。对于 DHI Enterprise 订阅，当发现新 CVE 时，将在行业领先的 SLA 时间范围内修复。了解有关 [SLA 支持的安全功能](./features.md#sla-backed-security) 的更多信息。
...
你已拉取并运行了第一个 Docker Hardened Image。以下是几种继续探索的方式：

- [将现有应用迁移到 DHIs](./migration/migrate-with-ai.md)：使用 Docker 的 AI 助手更新你的 Dockerfile，以使用 Docker Hardened Images 作为基础。

- [开始试用](https://hub.docker.com/hardened-images/start-free-trial)：探索 DHI Enterprise 订阅的优势，例如访问 FIPS 和 STIG 变体、自定义镜像和 SLA 支持的更新。
...
- [扫描 DHIs](./how-to/scan.md)：使用 Docker Scout 或其他扫描器分析镜像，识别已知 CVE。