...
title: 配置文件参考
linkTitle: 配置文件
description: cagent YAML 配置文件格式完整参考
keywords: [ai, agent, cagent, configuration, yaml]
weight: 10
---

本文档参考了 cagent 代理的 YAML 配置文件格式，涵盖文件结构、代理参数、模型配置、工具集设置和 RAG 源。

有关每个工具集能力的详细文档和特定选项，请参阅 [工具集参考](./toolsets.md)。
...
| `num_history_items`    | integer | 对话历史限制                                   | 否   |
...
使用 `sub_agents` 将工作分解为任务。根代理将工作分配给子代理并获取结果，同时保持控制权。

使用 `handoffs` 将整个对话转移到不同的代理。新代理完全接管。

### 命令

用户通过 `/command_name` 调用的命名提示。支持使用 `${env.VARIABLE}` 的 JavaScript 模板字面量：
...
将响应约束到 JSON 模式（仅 OpenAI 和 Gemini）：
...
| `top_p`               | float   | 核采样（0.0-1.0）                              | 否   |
| `frequency_penalty`   | float   | 重复惩罚（-2.0 到 2.0，仅 OpenAI）             | 否   |
| `presence_penalty`    | float   | 话题惩罚（-2.0 到 2.0，仅 OpenAI）             | 否   |
| `base_url`            | string  | 自定义 API 端点                                | 否   |
| `parallel_tool_calls` | boolean | 启用并行工具执行（默认：true）                 | 否   |
| `token_key`           | string  | 认证令牌键                                     | 否   |
| `track_usage`         | boolean | 跟踪令牌使用情况                               | 否   |
| `thinking_budget`     | mixed   | 推理努力（提供者特定）                         | 否   |
...
- **Gemini**：整数令牌预算（0 为禁用，-1 为动态，最大 24576）
...
> [!NOTE] 本节涵盖工具集配置语法。有关每个工具集能力、可用工具和特定配置选项的详细文档，请参阅 [工具集参考](./toolsets.md)。

所有工具集都支持公共属性，如 `tools`（白名单）、`defer`（延迟加载）、`toon`（输出压缩）、`env`（环境变量）和 `instruction`（使用指导）。详情请参阅 [工具集参考](./toolsets.md)。
...
- type: mcp
  ...
  tools: ["read_file", "write_file"] # 可选：限制到特定工具
  env:
...
所有策略都支持分块配置。分块大小和重叠以字符（Unicode 代码点）衡量，而非令牌。

#### 分块嵌入

使用向量嵌入的直接语义搜索。最适合理解意图、同义词和释义。
...
LLM 增强的语义搜索。使用语言模型在嵌入前为每个分块生成丰富的语义摘要，捕获更深层含义。
...
使用 BM25 算法的关键词搜索。最适合精确术语、专业术语和代码标识符。
...
对于源代码，使用基于 AST 的分块。使用语义嵌入，您可以将 AST 元数据包含在 LLM 提示中：
...
| 字段        | 类型     | 说明                                                     |
| ----------- | -------- | -------------------------------------------------------- |
| `docs`      | []string | 文档路径（支持 glob 模式，遵循 `.gitignore`）            |
| `tool`      | object   | 自定义 RAG 工具名称/描述/指导                            |
| `strategies`| []object | 检索策略（策略特定字段见上文）                           |
