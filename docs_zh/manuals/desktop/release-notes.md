...
发布日期超过 6 个月的旧版 Docker Desktop 无法下载。以前的发行说明可在我们的[文档仓库](https://github.com/docker/docs/tree/main/content/manuals/desktop/previous-versions)中找到。

更多常见问题解答，请参见 [FAQs](/manuals/desktop/troubleshoot-and-support/faqs/releases.md)。
...
- 修复了在长时间的 Ask Gordon 会话中每次击键时出现的性能问题。
- 修复了当组织配置了注册表访问管理以阻止 Docker Hub 时，kubeadm 模式下的 Kubernetes 无法启动的问题。
...
- 修复了 `/dev/shm` 权限不足导致容器无法写入的错误。修复了 [docker/for-mac#7804](https://github.com/docker/for-mac/issues/7804)。
...
- 添加了安全补丁以解决 [CVE-2025-13743](https://www.cve.org/cverecord?id=CVE-2025-13743)，该问题导致 Docker Desktop 诊断包在日志输出中包含过期的 Hub PAT，原因是错误对象序列化。
...
- 为 Docker Desktop 添加了新的端口绑定设置。管理员也可以通过使用 `admin-settings.json` 文件的设置管理来控制此设置。
...
- Docker Model Runner 改进：
   - 现在可以为 [Docker Model Runner 的 OpenAI API 端点](/manuals/ai/model-runner/api-reference.md#rest-api-examples) 跳过 `/engines` 前缀：`curl http://localhost:12434/v1/models`。
   - 现在可以为[在 Docker Hub 上发布的模型](https://hub.docker.com/u/ai) 跳过 `ai/` 前缀：`docker model pull`。
   - 下载中断后现在可以恢复。
...
- 您现在可以从 **Kubernetes** 视图设置 Kubernetes 资源。此新视图还提供 Pod、服务和部署的实时显示。
...
- [Dynamic MCP](/manuals/ai/mcp-catalog-and-toolkit/dynamic-mcp.md)（实验性）现已在 Docker Desktop 中可用。
...
- Docker Desktop 不再将 Stargz Snapshotter 故障视为致命错误。如果发生故障，Docker Desktop 将在没有 Stargz Snapshotter 的情况下继续运行。
...
- 修复了 [CVE-2025-9164](https://www.cve.org/cverecord?id=CVE-2025-9164)，该漏洞导致 Windows 版 Docker Desktop 安装程序由于不安全的 DLL 搜索顺序而容易受到 DLL 劫持。安装程序在检查系统目录之前，会在用户的下载文件夹中搜索所需的 DLL，从而允许通过恶意 DLL 放置进行本地权限提升。

### 新增

- [cagent](/manuals/ai/cagent/_index.md) 现在通过 Docker Desktop 提供。
...
- 修复了在等待用户输入新密码时 Docker Desktop 使用过期代理密码的问题。
...
- 您现在可以为 [macOS](/manuals/desktop/setup/install/mac-install.md#proxy-configuration) 和 [Windows](/manuals/desktop/setup/install/windows-install.md#proxy-configuration) 使用安装程序标志指定 PAC 文件和嵌入式 PAC 脚本。
...
- 修复了当 Desktop 重启时 kind 集群状态有时会重置的问题。修复了 [docker/for-mac#77445](https://github.com/docker/for-mac/issues/7745)。
- 移除了过时的 `mcp` 密钥，以符合最新的 VS Code MCP 服务器更改。
- 更新凭证助手到 [v0.9.4](https://github.com/docker/docker-credential-helpers/releases/tag/v0.9.4)。
- 修复了在等待用户输入新密码时 Docker Desktop 使用过期代理密码的问题。
- 修复了在某些条件下导致 Docker Desktop 定期使用 Docker CLI 工具创建新进程的错误。修复了 [docker/for-win#14944](https://github.com/docker/for-win/issues/14944)。
- 修复了通过 Compose 使用 Docker Model Runner 时模型未配置为嵌入的错误。要指定模型应配置为嵌入，您必须显式添加 `--embeddings` 运行时标志，如 [Docker Compose 中的 AI 模型](https://docs.docker.com/ai/compose/models-and-compose/#model-configuration-options) 中所述。修复了 [docker/model-runner#166](https://github.com/docker/model-runner/issues/166)。

#### Windows

- 移除了 `HKLM\SOFTWARE\Docker Inc.\Docker\1.0` 注册表项。改为在路径中查找 `docker.exe` 以找出 Docker Desktop 的安装位置。
...
- 修复了 [CVE-2025-10657](https://www.cve.org/CVERecord?id=CVE-2025-10657)，该漏洞导致增强型容器隔离的 [Docker Socket 命令限制](../enterprise/security/hardened-desktop/enhanced-container-isolation/config.md#command-restrictions) 功能仅在 Docker Desktop 4.46.0 中无法正常工作（其配置被忽略）。

### 新增

- 为 Docker 的 MCP 目录添加了动态 MCP 服务器发现和支持。
- 使用增强型容器隔离时，管理员现在可以阻止在挂载了 Docker 套接字的容器中使用 `docker plugin` 和 `docker login` 命令。
...
- 您现在可以使用过滤器、排序和改进的搜索功能更轻松地搜索 MCP 服务器。
- Docker Debug 在调试环境变量设置为空值的容器时不再挂起。
- 增强了 Docker Model Runner，具有 CLI 中的丰富响应渲染、Docker Desktop 仪表板中的对话上下文以及可恢复的下载。

#### Mac

- 移除了 `com.apple.security.cs.allow-dyld-environment-variables` 权限，该权限允许通过 `DYLD_INSERT_LIBRARIES` 环境变量将签名的任意动态库加载到 Docker Desktop。
- 修复了配置描述文件登录强制执行在某些客户环境中失效的回归问题。
- 修复了在写入本地内容存储（不带 `--push` 标志）时 `docker model package` 命令有时会挂起的错误。
- 修复了使用 `unless-stopped` 重启策略启动的容器永远不会重启的错误。修复了 [docker/for-mac#7744](https://github.com/docker/for-mac/issues/7744)。
...
- 更新体验经过重新设计，使理解和管理 Docker Desktop 及其组件的更新变得更加容易。
...
- 使用 Docker CLI 时，当 Docker 上下文元数据中存在键值对（`"GODEBUG":"..."`）时，您现在可以设置 `GODEBUG` 环境变量。这意味着 CLI 二进制文件中具有负序列号的证书默认受支持。
- 更新了 Docker 订阅服务协议链接以指向最新版本。

#### Mac

- 通过启用 `llama.cpp` 推理进程的沙箱化，提高了 Docker Model Runner 的安全性。
- 修复了导致 Docker Desktop 启动缓慢且看起来冻结的错误。修复了 [docker/for-mac#7671](https://github.com/docker/for-mac/issues/7671)。
...
- 修复了笔记本电脑从睡眠状态唤醒后导致 Docker Desktop 崩溃的错误。修复了 [docker/for-mac#7741](https://github.com/docker/for-mac/issues/7741)。
- 修复了 VM 有时会因 **虚拟机意外停止** 错误而失败的问题。
- 修复了在容器启动后连接到网络或从网络断开连接时会破坏端口映射的错误。修复了 [docker/for-mac#7693](https://github.com/docker/for-mac/issues/7693)。

#### Windows

- 修复了当用户缺少正确权限时，默认情况下无法将 CLI 插件部署到 `~/.docker/cli-plugins` 的错误。
- 修复了如果 `docker-desktop` 发行版不存在，重新定位 WSL 数据发行版将失败的错误。
...
- 修复了 [CVE-2025-9074](https://www.cve.org/CVERecord?id=CVE-2025-9074)，该漏洞允许在 Docker Desktop 上运行的恶意容器访问 Docker Engine 并启动其他容器，而无需挂载 Docker 套接字。这可能允许未经授权访问主机系统上的用户文件。增强型容器隔离 (ECI) 无法缓解此漏洞。
...
- 修复了在 4.44.0 版本中发现的问题，该问题导致当 `vpnkit` CIDR 被锁定且未在桌面设置管理中指定值时启动失败。

#### Windows

- 修复了在从使用旧版 `version-pack-data` 目录结构的发行版升级后，卷和容器不可见的问题。
- 解决了 WSL 2 中一个罕见的问题，即 Docker CLI 因 **需要代理身份验证** 错误而失败。
- 修复了当用户对该目录缺少执行权限时，CLI 插件未部署到 `~/.docker/cli-plugins` 的错误。
...
- 添加了运行多个模型的能力，并在资源不足时收到警告。这避免了在使用大型模型时 Docker Desktop 冻结。
...
- Apple 虚拟化现在是默认的 VMM，以获得更好的性能，并且移除了 QEMU 虚拟化。请参阅[博客文章](https://www.docker.com/blog/docker-desktop-for-mac-qemu-virtualization-option-to-be-deprecated-in-90-days/)。
- DockerVMM 的性能和稳定性得到改进。
...
我们注意到 [CVE-2025-23266](https://nvd.nist.gov/vuln/detail/CVE-2025-23266)，这是一个影响 CDI 模式下 NVIDIA Container Toolkit（最高版本 1.17.7）的关键漏洞。Docker Desktop 包含版本 1.17.8，不受影响。但是，捆绑了较早工具包版本的旧版 Docker Desktop 如果手动启用了 CDI 模式，则可能受到影响。请升级到 Docker Desktop 4.44 或更高版本，以确保您使用的是已修补的版本。
...
- 修复了使用 `--restart` 标志启动的容器在使用增强型容器隔离时无法正确重启的错误。
- 改进了 [Kubernetes 自定义注册表镜像](/manuals/desktop/use-desktop/kubernetes.md#configuring-a-custom-image-registry-for-kubernetes-control-plane-images) 和增强型容器隔离 (ECI) 之间的交互，因此在使用自定义注册表作为 Kubernetes 控制平面镜像时，不再需要手动更新 [ECI Docker Socket 镜像列表](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/config.md)。
- 修复了当用户需要登录但当前已注销时，kind 模式下的 Docker Desktop Kubernetes 集群在重启 Docker Desktop 后无法启动的错误。
- 修复了在启用[增强型容器隔离](/enterprise/security/hardened-desktop/enhanced-container-isolation/) 时阻止将 MCP 秘密挂载到容器中的错误。
- 修复了在已指定 `--publish` 时阻止使用 `--publish-all` 的错误。
- 修复了导致 **镜像** 视图无限滚动的错误。修复了 [docker/for-mac#7725](https://github.com/docker/for-mac/issues/7725)。
- 修复了在资源节省器模式下 **卷** 选项卡为空白的错误。
- 更新了首次启动时的服务条款文本。
...
- 通过重新引入常规使用上的性能提升，修复了自 4.42.0 以来的回归问题。
...
- 修复了阻止 Traefik 自动检测容器端口的错误。修复了 [docker/for-mac#7693](https://github.com/docker/for-mac/issues/7693)。
- 修复了在容器启动后连接到网络或从网络断开连接时会破坏端口映射的错误。修复了 [docker/for-mac#7693](https://github.com/docker/for-mac/issues/7693#issuecomment-3131427879)。
...
- 修复了 Docker Desktop 可能泄漏 `com.docker.build` 进程并无法启动的错误。修复了 [docker/for-win#14840](https://github.com/docker/for-win/issues/14840)。
- 修复了在使用带有 `cgroups v1` 的 WSL 且启用了增强型容器隔离 (ECI) 时，阻止 Docker Desktop Kubernetes 在 kind 模式下启动的错误。
...
- 修复了 [CVE-2025-6587](https://www.cve.org/CVERecord?id=CVE-2025-6587)，该漏洞导致敏感的系统环境变量包含在 Docker Desktop 诊断日志中，可能导致潜在的密钥泄露。

### 错误修复和增强功能

#### 所有平台

- 修复了导致 `docker start` 为已在运行的容器丢弃端口映射的错误。
- 修复了当容器重新启动时，容器端口无法在 GUI 上显示的错误。
- 修复了导致 Docker API 出现 `500 Internal Server Error for API route and version` 错误应用程序启动的错误。
- 设置中的 **应用并重启** 按钮现在标记为 **应用**。应用更改后的设置时不再重启 VM。
- 修复了在 Docker 关闭期间进行 `fsck` 时磁盘会损坏的错误。
...
- `docker buildx bake` 将不会在具有顶层 models 属性的 Compose 文件中构建镜像。请改用 `docker compose build`。
- 包含 HTML 的 Gordon 响应可能导致桌面 UI 永久损坏。作为解决方法，您可以删除 `persisted-state.json` 文件以重置 UI。该文件位于以下目录中：
...
- Docker Desktop 的“主机网络”功能与最新的 WSL 2 Linux 内核可能存在兼容性问题。如果您遇到此类问题，请将 WSL 2 降级到 2.5.7。
...
- 修复了在创建后立即检查时（例如使用脚本时）容器端口列表显示为空的错误。[docker/for-mac#7693](https://github.com/docker/for-mac/issues/7693)

#### Windows

- 禁用了 WSL 2 中的资源节省器模式，以防止 `docker` CLI 命令在 WSL 2 发行版中挂起。[docker/for-win#14656](https://github.com/docker/for-win/issues/14656#issuecomment-2960285463)
...
- 默认重新为容器启用 `seccomp`。使用 `docker run --security-opt seccomp=unconfined` 为容器禁用 seccomp。
...
- 修复了团队订阅用户的[强制登录](/manuals/enterprise/security/enforce-sign-in/_index.md)问题。
...
- 在 macOS 上安装或更新 Docker Desktop 的最低版本现在是 macOS Ventura 13.3。

#### Windows

- 修复了 Windows WSL 上增强型容器隔离中的一个错误，即容器内具有硬链接的文件具有 `nobody:nogroup` 所有权。
- 修复了导致 Docker Desktop 崩溃的错误。与 [docker/for-win#14782](https://github.com/docker/for-win/issues/14782) 相关。
- 修复了在 WSL 2 启动时导致 `找不到网络名称` 错误的错误。修复了 [docker/for-win#14714](https://github.com/docker/for-win/issues/14714)。
- 修复了在卸载时 Docker Desktop 不会删除 hosts 文件中的条目的问题。
- 修复了在某些系统语言下读取自动启动注册表项时的问题。修复了 [docker/for-win#14731](https://github.com/docker/for-win/issues/14731)。
- 修复了 Docker Desktop 添加了无法识别的 /etc/wsl.conf `crossDistro` 选项的错误，该错误导致 WSL 2 记录错误。请参阅 [microsoft/WSL#4577](https://github.com/microsoft/WSL/issues/4577)
- 修复了如果另一个 WSL 发行版仍在使用 Linux cgroups v1，Docker Desktop 在 WSL 2.5.7 上无法启动的错误。修复了 [docker/for-win#14801](https://github.com/docker/for-win/issues/14801)
...
- 此版本包含一个回归问题，导致 `docker port` 在使用 testcontainers-node 时出现“未找到主机 IP 的主机端口”错误。请参阅 [testcontainers/testcontainers-node#818](https://github.com/testcontainers/testcontainers-node/issues/818#issuecomment-2941575369)

#### Windows

- 运行 Wasm 容器会间歇性挂起。请参阅 [docker/for-mac#7666](https://github.com/docker/for-mac/issues/7666)。
- 在某些机器上，资源节省器会导致其他 WSL 2 发行版冻结。解决方法是禁用资源节省器。请参阅 [docker/for-win#14656](https://github.com/docker/for-win/issues/14656)。
...
- 修复了当 Docker Model Runner 不受支持或未启用时，GUI 中仍显示 `模型` 菜单的问题。
...
- Docker Model Runner 现在可用于配备 NVIDIA GPU 的 x86 Windows 机器。
- 您现在可以使用 Docker Model Runner 将[模型推送到 Docker Hub](/manuals/ai/model-runner.md#push-a-model-to-docker-hub)。
- 在支持 Docker Model Runner 的硬件上，为 Mac 和 Windows 的 Docker Desktop 添加了对 Docker Model Runner 的模型管理和聊天界面的支持。用户现在可以通过新的专用界面查看、交互和管理本地 AI 模型。
- [Docker Compose](/manuals/ai/compose/models-and-compose.md) 和 Testcontainers [Java](https://java.testcontainers.org/modules/docker_model_runner/) 以及 [Go](https://golang.testcontainers.org/modules/dockermodelrunner/) 现在支持 Docker Model Runner。
- 在 [Microsoft 应用商店](https://apps.microsoft.com/detail/xp8cbj40xlbwkx?hl=en-GB&gl=GB) 中推出 Docker Desktop。
...
- 修复了 [CVE-2025-3224](https://www.cve.org/CVERecord?id=CVE-2025-3224)，该漏洞允许获得用户机器访问权限的攻击者在 Docker Desktop 更新时执行权限提升。
- 修复了 [CVE-2025-4095](https://www.cve.org/CVERecord?id=CVE-2025-4095)，该漏洞导致在使用 MacOS 配置描述文件时未强制执行注册表访问管理 (RAM) 策略，允许用户从未批准的注册表拉取镜像。
- 修复了 [CVE-2025-3911](https://www.cve.org/CVERecord?id=CVE-2025-3911)，该漏洞允许具有用户机器读取权限的攻击者从 Docker Desktop 日志文件中获取敏感信息，包括为正在运行的容器配置的环境变量。
...
- 通过高级搜索功能和容器级过滤增强了日志读取体验，实现更快的调试和故障排除。
- 改进了下载注册表访问管理配置时的错误消息。
- 如果 Docker 无法绑定 ICMPv4 套接字，它现在会记录错误并继续运行，而不是退出。
- 在 Docker Desktop Linux VM 中启用了内存保护密钥机制，允许像 Oracle 数据库镜像这样的容器正确运行。
- 修复了在 Mac、Windows Hyper-V 或 Linux 上启用[增强型容器隔离](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/_index.md) 时容器访问 `/proc/sys/kernel/shm*` sysctls 的问题。
- 添加了内核模块 `nft_fib_inet`，这是在 Linux 容器中运行 firewalld 所必需的。
- MacOS QEMU 虚拟化选项将于 2025 年 7 月 14 日弃用。

#### Mac

- 修复了导致高 CPU 使用率的错误。修复了 [docker/for-mac#7643](https://github.com/docker/for-mac/issues/7643)。
- 修复了在 M3 Mac 上使用 Rosetta 的多架构构建问题。
- 修复了当 `/Library/Application Support/com.docker.docker/` 目录不存在时导致无法应用 RAM 策略限制的问题。

#### Windows

- Windows `.exe` 安装程序现在包含改进的锁定文件处理。修复了 [docker/for-win#14299](https://github.com/docker/for-win/issues/14299) 和 [docker/for-win#14316](https://github.com/docker/for-win/issues/14316)。
- 修复了安装后 `Docker Desktop.exe` 不显示版本信息的问题。修复了 [docker/for-win#14703](https://github.com/docker/for-win/issues/14703)。

### 已知问题

#### 所有平台

- 如果您使用 `desktop.plist`（在 macOS 上）或注册表项（在 Windows 上）强制登录，并且还有 `registry.json`，如果用户属于 `desktop.plist`/注册表项中列出的组织，但不属于 `registry.json` 中指定的任何组织，则登录将失败。要解决此问题，请删除 `registry.json` 文件。

#### Windows

- 如果在 Windows 注册表项 `allowedOrgs` 中使用空格分隔格式指定了多个组织，登录将失败，用户将被注销。作为解决方法，请在注册表项值中将每个组织单独放在一行上。
...
- 您现在可以直接在 Docker Desktop 中使用 [Docker Model Runner (Beta)](/manuals/ai/model-runner.md) 从 Docker Hub 拉取、运行和管理 AI 模型。目前适用于配备 Apple Silicon 的 Docker Desktop for Mac。
...
- 当 Time Machine 实用程序受到限制时，移除了继续/重启弹出窗口。
...
- 修复了当为指定端口的服务器存储令牌时，`docker-credential-osxkeychain` 和 `docker-credential-desktop` 返回格式错误的 URI 的错误。
...
- 在显示特权助手错误消息时切换到 Windows 容器可能导致状态不一致。作为解决方法，请退出 Docker Desktop，在 `settings-store.json` 中将 `UseWindowsContainers` 更改为 `false`，然后重新启动 Docker Desktop。
...
- 修复了 [CVE-2025-1696](https://www.cve.org/CVERecord?id=CVE-2025-1696)，该漏洞可能以明文形式在日志文件中泄露代理身份验证凭据。
...
- 通过支持用户在 `docker history` 中选择特定平台来支持多平台镜像。
...
- 修复了 ECI Docker 套接字权限中的错误，该错误有时会阻止在允许的镜像或派生自允许镜像的镜像的容器上挂载 Docker 套接字。
- 修复了在引擎重启后 Docker Desktop 无法立即再次进入资源节省器模式的错误。
...
- 修复了导致在 M4 Macbook Pro 上运行的所有 Java 程序发出 SIGILL 错误的错误。请参阅 [docker/for-mac#7583](https://github.com/docker/for-mac/issues/7583)。
...
- 当 Docker Desktop 使用 WSL 时，修复了 Paketo buildpack 在增强型容器隔离下失败的错误。
- 修复了如果安装了 WSL 版本 1 发行版，WSL 2 集成将失败的错误。
...
- [Ask Gordon](/manuals/ai/gordon/_index.md) 更加广泛可用。它仍处于 Beta 阶段。
...
- 修复了在启用[增强型容器隔离](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/_index.md) 时，使用 `curl` 通过 Docker API 创建容器失败的错误。
- 修复了在刷新期过后 RAM 策略未刷新的错误。
- 修复了增强型容器隔离中的一个错误，该错误发生在将 Docker 套接字挂载到容器中，然后从该容器内部使用绑定挂载创建 Docker 容器时。
- 修复了导致 GUI 和 CLI 之间出现差异的问题，前者强制在端口映射中使用 `0.0.0.0` HostIP。这导致通过 Engine 的 `ip` 标志或通过桥接选项 `com.docker.network.bridge.host_binding_ipv4` 配置的默认绑定 IP 未被使用。
...
- 修复了 Docker VMM 中来自非根卷的绑定挂载无法按预期工作的错误。
- 修复了在没有 IPv6 的系统上导致启动失败的问题。修复了 [docker/for-mac#14298](https://github.com/docker/for-win/issues/14298)。
...
- 当 WSL 集成设置更改时，Docker Desktop 现在会重新启动。这确保了在使用增强型容器隔离时正确设置 WSL 集成。

#### Linux

- 添加了对 gvisor 网络的支持。qemu 版本不兼容（8.x）的用户将保持在 qemu 网络上，其他用户将自动迁移。
...
- 防止了一个错误，该错误导致 Docker Desktop 无法将 `com.docker.vmnetd` 或 `com.docker.socket` 更新到较新版本。

### 已知问题

#### Mac

- 如果您看到关于 `com.docker.vmnetd` 或 `com.docker.socket` 存在恶意软件的安全弹出窗口，请按照 [docker/for-mac#7527](https://github.com/docker/for-mac/issues/7527) 中记录的步骤操作。
...
- 修复了在使用[增强型容器隔离](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/_index.md) 时，Docker Desktop 因 `index out of range [0] with length 0` 而发生 panic 的问题。
...
- 修复了在资源节省器模式下（仅限带有 WSL2 后端的 Windows）或在切换引擎后（macOS），重启引擎后 Docker Desktop 仪表板与 Docker 守护进程不同步的错误。
- 修复了在资源节省器模式下重启引擎后，资源节省器模式无法重新启动的错误。
- 构建 UI：
  - 修复了某些构建找不到源文件的错误。
  - 修复了错误日志未显示在 **源** 选项卡中的错误。
  - 修复了用户必须滚动到底部才能在 **源** 选项卡中查看错误日志的错误。
  - 修复了 **日志** 选项卡中时间戳损坏的错误。
...
- Apple Silicon 上的 QEMU（旧版）作为 VMM 将在未来的版本中被移除。建议您切换到 Apple 虚拟化框架以提高性能和稳定性。如果遇到问题，请[联系 Docker 支持](https://www.docker.com/support/) 或[提交 GitHub 问题](https://github.com/docker/for-mac/issues)。
- osxfs（旧版）将在未来的版本中被移除。建议您切换到 VirtioFS 以提高性能。如果遇到问题，请[联系 Docker 支持](https://www.docker.com/support/) 或[提交 GitHub 问题](https://github.com/docker/for-mac/issues)。
...
- 现有在 Windows 上使用 WSL2 引擎的 Docker Desktop 安装现在会自动迁移到统一的单发行版架构，以增强一致性和性能。
- 管理员现在可以：
  ...
  - 使用桌面设置管理通过 admin.docker.com 管理和强制执行默认值（早期访问）。
- 增强型容器隔离 (ECI) 已得到改进：
...
- 修复了如果 Docker Desktop 配置为使用代理，由于在获取身份验证令牌时内部超时而无法启动的问题。
...
- 修复了非根容器的绑定挂载所有权。修复了 [docker/for-mac#6243](https://github.com/docker/for-mac/issues/6243)。
- Docker Desktop 在手动暂停后不会自动取消暂停。系统将保持暂停状态，直到您手动恢复 Docker 引擎。这修复了一个错误，即其他软件可能会通过在后台运行 CLI 命令意外触发恢复。修复了 [for-mac/#6908](https://github.com/docker/for-mac/issues/6908)
- 构建 UI：
  - **源** 选项卡现在支持多个源文件。
...
- 修复了 Docker VMM 中的一个错误，该错误阻止了 MySQL 和其他数据库容器启动。修复了 [docker/for-mac#7464](https://github.com/docker/for-mac/issues/7464) 的报告。
...
- 修复了高级选项 **允许特权端口映射** 无法按预期工作的错误。修复了 [docker/for-mac#7460](https://github.com/docker/for-mac/issues/7460)。
- Docker Desktop 现在可以在安装向导和设置屏幕中自动为 zsh、bash 和 fish 配置 shell 补全脚本。
- 修复了如果 Docker Desktop 由非管理员用户安装或当前用户以前是管理员，应用内更新会失败的错误。修复了 [for-mac/#7403](https://github.com/docker/for-mac/issues/7403) 和 [for-mac/#6920](https://github.com/docker/for-mac/issues/6920)

#### Windows

- 修复了阻止绑定 UDP 端口 53 的错误。
- 修复了在启动时 Windows 守护进程选项被覆盖的错误。
...
- 修复了 Docker Desktop 会错误地绑定到端口 `8888` 的错误。修复了 [docker/for-win#14389](https://github.com/docker/for-win/issues/14389) 和 [docker/for-mac#7468](https://github.com/docker/for-mac/issues/7468)
...
- 对[在 Red Hat Enterprise Linux 上运行 Docker Desktop](/manuals/desktop/setup/install/linux/rhel.md) 的支持现已正式可用。
- 卷备份和共享现已正式可用，可在 **卷** 视图中找到。
- 在 Docker Desktop 中使用系统 shell 的终端支持现已正式可用。
...
- 移除了 [docker-index](https://github.com/docker/index-cli-plugin)，因为它现在已弃用，您可以改用 `docker scout cves fs://<path to binary>`。
- 修复了镜像无法按标签排序或过滤的错误。修复了 [docker/for-win#14297](https://github.com/docker/for-win/issues/14297)。
- 修复了当 `registry.json` 文件格式错误时 `docker` CLI 无法按预期工作的错误。
- 修复了在 **镜像** 视图中选择 **推送到 Docker Hub** 操作会导致 `invalid tag format` 错误的错误。修复了 [docker/for-win#14258](https://github.com/docker/for-win/issues/14258)。
- 修复了当 ICMPv6 设置不成功时 Docker Desktop 启动失败的问题。
- 添加了允许 USB/IP 工作的驱动程序。
- 修复了增强型容器隔离 (ECI) [派生镜像的 Docker 套接字挂载权限](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/config.md) 中的错误，该错误在 Docker Desktop 使用 containerd 镜像存储时错误地拒绝了某些镜像的 Docker 套接字挂载。
- 启用 `NFT_NUMGEN`、`NFT_FIB_IPV4` 和 `NFT_FIB_IPV6` 内核模块。
- 构建 UI：
  - 在 **已完成的构建** 列表中突出显示构建检查警告。
  - 改进了构建时间图表的可视化。
  - 镜像标签添加到 **信息** 选项卡下的 **构建结果** 部分。
- 改进了 Mac 和 Linux 上全新安装的主机端磁盘利用率效率。
- 修复了在令牌过期时无法触发登录强制执行弹出窗口的错误。
- 修复了在使用[强制登录](/manuals/enterprise/security/enforce-sign-in/_index.md) 时，登录后容器不会立即显示在 GUI 中的错误。
...
- 修复了在启动时 `~/.docker/cli-plugins` 可能未填充的错误。
- 修复了阻止 php composer 或 postgres 以非 root 用户身份启动的错误。修复了 [docker/for-mac#7415](https://github.com/docker/for-mac/issues/7415)。
- 修复了可能导致主机上更改的文件显示为截断的错误。修复了 [docker/for-mac#7438](https://github.com/docker/for-mac/issues/7438)。

#### Windows

- Windows 版 Docker Desktop 的新安装现在需要 Windows 版本 19045 或更高版本。
- 修复了如果在 WSL 中通过内核配置或内核命令行禁用了 IPv6 会导致启动失败的问题。修复了 [docker/for-win#14240](https://github.com/docker/for-win/issues/14240)
...
- 自 4.34.0 版本以来，高级设置中的“允许特权端口映射”切换不起作用。有关更多信息，请参阅 [docker/for-mac#7460](https://github.com/docker/for-mac/issues/7460)。

#### Windows

- 版本 4.14.0 及更早的用户在使用应用内更新时可能会遇到问题。要更新到最新版本，请从此页面下载并安装最新的 Docker Desktop。
...
- 修复了 [CVE-2024-9348](https://www.cve.org/cverecord?id=CVE-2024-9348)，该漏洞允许通过精心制作的扩展描述/变更日志进行 RCE，可能被恶意扩展滥用。
...
- 修复了在资源节省器模式下 `docker compose up` 会变得无响应的错误。

### 安全

- 修复了 [CVE-2024-8695](https://www.cve.org/cverecord?id=CVE-2024-8695)，该漏洞允许通过精心制作的扩展描述/变更日志进行 RCE，可能被恶意扩展滥用。
- 修复了 [CVE-2024-8696](https://www.cve.org/cverecord?id=CVE-2024-8696)，该漏洞允许通过精心制作的扩展发布者 URL/附加 URL 进行 RCE，可能被恶意扩展滥用。
...
- 修复了 Docker Desktop 无法启动（通常在首次启动时）并错误地认为另一个应用程序实例正在运行的错误。([docker/for-win#14294](https://github.com/docker/for-win/issues/14294) 和 [docker/for-win#14034](https://github.com/docker/for-win/issues/14034))。
...
- 如果您通过 CLI 进行身份验证，您现在可以通过基于浏览器的流程进行身份验证，无需手动生成 PAT。
...
- [Compose Bridge](/manuals/compose/bridge/_index.md)（实验性）现在可从 Compose 文件查看器中获得。轻松将您的 Compose 项目转换并部署到 Kubernetes 集群。
...
- 修复了当容器以自动移除 (`--rm`) 启动但其端口绑定被 Docker Desktop 在启动时拒绝时，CLI 变得空闲的错误。
- 修复了在 **支持** 屏幕上诊断收集会随机失败的错误。
- 修复了容器的 **文件** 选项卡中文件夹无法展开的错误。修复了 [docker/for-win#14204](https://github.com/docker/for-win/issues/14204)。
- 应用内更新现在尊重代理设置。
- 扩展了 ECI Docker 套接字挂载权限功能，可选择性地允许源自允许镜像的子镜像。这允许 ECI 与创建使用 Docker 套接字挂载的临时本地镜像的 buildpack（例如 Paketo）一起工作。
- 修复了在使用某些代理设置时导致 **容器** 视图闪烁的错误。修复了 [docker/for-win#13972](https://github.com/docker/for-win/issues/13972)。
- 改进了 `docker image list` 的输出，以显示与多平台相关的镜像信息。

#### Mac

- 修复了在触发配置完整性检查功能时偶尔出现 `部分修复错误` 的问题。
- 配置完整性检查功能现在显示有关 Docker 套接字配置错误原因的信息。
- 修复了当 Docker Desktop 作为 `用户` 安装时，配置完整性检查功能会报告系统路径而不是用户路径的问题。
- 修复了尝试从绑定挂载卷读取扩展属性的应用程序可能失败的错误。修复了 [docker/for-mac#7377](https://github.com/docker/for-mac/issues/7377)。

#### Windows

- 修复了当用户打算保持 `credsStore` 为空时，Docker Desktop 会将 docker 的 `credsStore` 重置为 `desktop` 的错误。修复了 [docker/for-win#9843](https://github.com/docker/for-win/issues/9843)。
- 修复了在 WSL2 引擎中 Docker Desktop 无法启动的错误 [docker/for-win#14034](https://github.com/docker/for-win/issues/14034)。
- 修复了导致 WSL 发行版突然终止的错误。修复了 [docker/for-win/14230](https://github.com/docker/for-win/issues/14230)。
- 修复了导致 WSL 在每次启动时更新的问题。修复了 [docker/for-win/13868](https://github.com/docker/for-win/issues/13868), [docker/for-win/13806](https://github.com/docker/for-win/issues/13806)。

### 已知问题

- Compose Bridge 在 **实验性** 设置选项卡中启用时不会自动工作。需要几分钟时间才会通知您必须“修复” Docker Desktop，然后安装 `compose-bridge` 二进制文件。
- 即使 Kubernetes 正在运行且 Compose Bridge 已启用，Compose 文件查看器中的 **转换和部署** 按钮也可能被禁用。解决方法是禁用 **实验性** 设置选项卡中的 Compose Bridge，使用 **应用并重启** 应用更改，然后重新启用并再次选择 **应用并重启**。
- 在 Docker CLI 中针对注册表进行身份验证时存在一个已知问题 (`docker login [registry address]`)，如果提供的注册表地址包含存储库/镜像名称（例如 `docker login index.docker.io/docker/welcome-to-docker`），则存储库部分 (`docker/welcome-to-docker`) 不会被规范化，导致凭据存储不正确，从而导致后续从注册表拉取 (`docker pull index.docker.io/docker/welcome-to-docker`) 时无法通过身份验证。为防止这种情况，在运行 `docker login` 时，请不要在注册表地址中包含任何额外的后缀。
  > [!NOTE]
  > 使用包含 URL 路径段的地址进行 `docker login` 不是文档化的用例，被视为不受支持。推荐的用法是仅将注册表主机名，以及可选的端口，指定为 `docker docker login` 的地址。
- 当运行 `docker compose up` 且 Docker Desktop 处于资源节省器模式时，该命令无响应。作为解决方法，请手动退出资源节省模式，Docker Compose 将再次变得有响应。
- 当[增强型容器隔离 (ECI)](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/_index.md) 启用时，Docker Desktop 可能无法进入资源节省器模式。这将在未来的 Docker Desktop 版本中修复。
- 新的 [ECI 派生镜像的 Docker 套接字挂载权限](/manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/config.md#docker-socket-mount-permissions-for-derived-images) 功能在 Docker Desktop 配置为 **使用 containerd 拉取和存储镜像** 时尚未工作。这将在下一个 Docker Desktop 版本中修复。
...
- 添加了对 WSL2 2.3.11 及更高版本的支持，其中包括可加载内核模块。修复了 [docker/for-win#14222](https://github.com/docker/for-win/issues/14222)
...
- BuildKit 现在评估 Dockerfile 规则以通知您潜在问题。
- **资源分配** 设置现在可以直接从仪表板页脚显示的资源使用数据访问。
...
- 修复了使用 `--net=host` 启动并侦听 IPv6 地址的容器无法从主机访问的问题。
...
- Docker Desktop 现在在主机网络容器中存在端口冲突时发出通知。
...
- 配置完整性检查功能现在提供有关您的 Docker Desktop 配置已更改内容的更多上下文。有关更多信息，请参阅 [FAQs](/manuals/desktop/troubleshoot-and-support/faqs/macfaqs.md)。
- 配置完整性检查功能在修复 Docker Desktop 失败时显示错误。
- 修复了 IPv6 TCP 被设置为 `host.docker.internal` 的错误。修复了 [docker/for-mac#7332](https://github.com/docker/for-mac/issues/7332)。
- 修复了 `docker-compose` 符号链接指向空位置的问题。修复了 [docker/for-mac#7345](https://github.com/docker/for-mac/issues/7345)。

#### Linux

- 修复了某些 `wincred` 值在卸载后仍保留的问题。由 Javier Yong [@Javiery3889](https://github.com/Javiery3889) 报告。
...
- 包含了对 Docker Engine 中 AuthZ 插件绕过回归的修复。有关更多信息，请参阅 [CVE-2024-41110](https://www.cve.org/cverecord?id=CVE-2024-41110)。
...
- Docker Desktop 在 WSL 预发布版本 `v2.3.11.0` 和 `v2.3.12.0`（包含在 Windows 11 Insider 中）上无法启动。要解决此问题，请确保安装了 WSL `v2.2.4.0`。
  有关更多信息，请参阅 [microsoft/WSL#11794](https://github.com/microsoft/WSL/issues/11794)。这会影响 Docker Desktop 4.33.0 及更早版本。
...
- Docker Desktop 现在支持在 macOS 和带有 WSL2 后端的 Windows 上将数据移动到不同的驱动器。请参阅 [docker/for-win#13384](https://github.com/docker/for-win/issues/13384)。
- 您现在可以在 **卷** 选项卡中[为卷导出安排备份](use-desktop/volumes.md)（Beta）。
...
- 为没有依赖项的 Golang 项目添加了 Docker Init 支持。解决了 [docker/roadmap#611](https://github.com/docker/roadmap/issues/611)
...
- 构建视图：
  ...
  - 修复了构建记录会意外消失的问题。
  - 修复了阻止用户重新打开[导入的构建](use-desktop/builds.md#import-builds)的问题。
...
- 修复了 WSL 2 引擎上的一个问题，即如果用户手动移动了 `docker-desktop-data` 发行版，Docker Desktop 将无法检测到其存在。
- Windows on ARM 安装程序和[特权服务](/manuals/desktop/setup/install/windows-permission-requirements.md#privileged-helper) 现在为 ARM64 构建。
...
- 修复了回归问题，即每当 `/var/run/docker.socket` 指向意外路径时，就会出现 **另一个应用程序更改了您的桌面配置** 警告消息。
- 将配置检查菜单项和横幅更改为通知。
- 改进了绑定挂载上的读写操作性能。
- 修复了某些 `AMD64` Java 镜像的致命错误。修复了 [docker/for-mac/7286](https://github.com/docker/for-mac/issues/7286) 和 [docker/for-mac/7006](https://github.com/docker/for-mac/issues/7006)。
- 修复了在从 `/Applications` 安装时导致 Docker Desktop 删除 `Docker.app` 的问题。
- 修复了导致绑定挂载失败的错误。修复了 [docker/for-mac#7274](https://github.com/docker/for-mac/issues/7274)。

### 已知问题

#### 所有平台

- **使用 Compose 管理同步文件共享** 设置会自动为所有选择 **访问实验性功能** 的用户启用。这会将所有绑定挂载转换为同步文件共享。要禁用此行为，请取消选择 **访问实验性功能**。然后，手动删除任何文件共享，方法是转到 **资源** 中的 **文件共享** 选项卡，导航到 **同步文件共享** 部分，选择要删除的文件共享，然后选择 **删除**。
...
- 修复了更新前创建的容器、镜像和卷对用户可能不可见的错误。修复了 [docker/for-win#14118](https://github.com/docker/for-win/issues/14118)。
...
- 向 `settings.json` 添加了 `proxyEnableKerberosNTLM` 配置，以便在 Kerberos/NTLM 环境未正确设置时回退到基本代理身份验证。
- 修复了在启用增强型容器隔离时 Docker Debug 无法正常工作的错误。
- 修复了 UDP 响应未正确截断的错误。
- 修复了在使用[设置管理](/manuals/enterprise/security/hardened-desktop/settings-management/_index.md) 时 **更新** 屏幕被隐藏的错误。
- 修复了在启动时未正确应用 `admin-settings.json` 中定义的代理设置的错误。
- 修复了 **使用 Compose 管理同步文件共享** 切换开关未正确反映该功能值的错误。
- 修复了在 macOS 和带有 Hyper-V 的 Windows 上使用 gRPC FUSE 文件共享时，主机上修改的绑定挂载文件在容器重启后未更新的错误。修复了 [docker/for-mac#7274](https://github.com/docker/for-mac/issues/7274), [docker/for-win#14060](https://github.com/docker/for-win/issues/14060)。
- 构建视图：
  - 新的[导入构建](use-desktop/builds.md#import-builds) 功能，允许您导入其他人或 CI 环境中的构建的构建记录。
  - 修复了失败的构建结果中缺少 OpenTelemetry 迹线的问题。
...
- 向 VM 添加了 Linux 头文件，以简化自定义内核模块的编译。

### 安全

#### 所有平台

- 修复了增强型容器隔离 (ECI) 模式中的一个安全漏洞，该漏洞允许用户创建源自 Docker Desktop VM 内受限目录的 Docker 卷，并将其挂载到容器中，从而让容器访问此类受限的 VM 目录。
- 默认情况下，只有列出在市场中的扩展才能在 Docker Desktop 中安装。这可以在 Docker Desktop 的设置中更改。扩展开发者需要更改此选项以测试他们的扩展。

### For Windows

- 修复了 [CVE-2024-5652](https://www.cve.org/CVERecord?id=CVE-2024-5652)，该漏洞允许 `docker-users` 组中的用户通过 Windows 容器模式下的 `exec-path` Docker 守护进程配置选项导致 Windows 拒绝服务。此漏洞由 Hashim Jawad ([@ihack4falafel](https://github.com/ihack4falafel)) 与 Trend Micro Zero Day Initiative 合作发现。

### 弃用

#### 所有平台

- 以前作为 `com.docker.cli` 提供的 CLI 二进制文件现在简单地作为 `docker` 提供。此版本将 CLI 二进制文件保留为 `com.docker.cli`，但将在下一个版本中移除。
...
- 升级到 Docker Desktop 4.31.0 时，使用 Docker Desktop 4.8.0 或更早版本在仅 WSL 的 Windows 主机上创建的现有容器、镜像和卷对用户不可见。数据不会丢失，只是对 Docker Desktop 4.31.0 不可见。如果受到影响，请降级到 4.30 或更早版本。有关更多信息，请参见：[docker/for-win#14118](https://github.com/docker/for-win/issues/14118)。
...
- 在[设置管理](/manuals/enterprise/security/hardened-desktop/settings-management/_index.md) 中添加了管理入门调查的新设置。

#### Windows

- 在 Windows 上添加了对 [Kerberos 和 NTLM 代理身份验证](/manuals/desktop/settings-and-maintenance/settings.md#proxy-authentication) 的支持。需要商业订阅。
...
- 在无根容器中运行 `docker build` 命令时，改进了增强型容器隔离 (ECI) 的安全性。
- 修复了当 Docker Desktop 进入/退出资源节省器模式时，`docker events` 以 `Unexpected EOF` 退出的错误。
- 修复了当 Docker Desktop 处于资源节省器模式时，`docker stats --no-stream` 挂起的错误。
- 修复了自诊断 CLI 中的错误，该错误错误地显示 VM 尚未启动。修复了 [docker/for-mac#7241](https://github.com/docker/for-mac/issues/7241)。
- 修复了高吞吐量端口转发传输可能停滞的错误。修复了 [docker/for-mac#7207](https://github.com/docker/for-mac/issues/7207)。
- 修复了在移除 CLI 应用程序时 CLI 插件符号链接未被移除的错误。
- 修复了共享端口抽屉中的错误，为本地引擎显示正确的消息。
- 开发环境正在逐步淘汰，并已移至 **功能开发中** 选项卡中的 **Beta** 选项卡。
- 构建视图：
  - 更好的批量删除构建记录。
  - 为容器镜像和 Git 源的构建依赖项添加了打开相关网页的操作。
...
- 如果未安装 Rosetta，修复了阻止 Docker Desktop 启动的错误。修复了 [docker/for-mac#7243](https://github.com/docker/for-mac/issues/7243)。
...
- 修复了导致以下错误的错误：`合并设置时 integratedWslDistros 类型不匹配`。
...
- 要启用或禁用 **使用 Compose 管理同步文件共享**，必须同时选中或取消选中 **访问实验性功能** 和 **使用 Compose 管理同步文件共享**。
- 当使用自动移除选项 (`--rm`) 运行容器时，如果容器启动失败（例如：`docker run --rm alpine invalidcommand`），Docker CLI 有时会挂起。在这种情况下，可能需要手动终止 CLI 进程。
...
- 全新的改进版 Docker Desktop 错误屏幕：快速故障排除、轻松的诊断上传和可操作的补救措施。
...
- 以下功能的 Beta 版发布：
  ...
  - **卷** 选项卡中提供的[卷备份和共享扩展](use-desktop/volumes.md) 功能。
...
- Docker Init：
  ...
  - 改进了基于 Spring Boot 版本启动 Java 应用程序的方式。修复了 [docker/for-mac#7171](https://github.com/docker/for-mac/issues/7171)。
...
- 修复了在使用 QEMU 启动 Docker Desktop 时发生的竞争条件。
...
- 修复了当浏览器无法将身份验证信息发送回 Docker Desktop 时的错误。修复了 [docker/for-mac/issues#7160](https://github.com/docker/for-mac/issues/7160)。

#### Windows

- 修复了在 WSL 2 和 Hyper-V 之间切换后 `docker run -v` 失败的错误。
- 修复了当 Docker Desktop 关闭时，其 WSL 发行版（`docker-desktop` 和 `docker-desktop-data`）未停止的错误。修复了 [docker/for-win/issues/13443](https://github.com/docker/for-win/issues/13443) 和 [docker/for-win/issues/13938](https://github.com/docker/for-win/issues/13938)。
...
- 禁用了 Electron `runAsNode` fuse 以提高安全性。有关更多信息，请参阅 [Electron 的文档。](https://www.electronjs.org/blog/statement-run-as-node-cves)。
- 修复了 [CVE-2024-6222](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2024-6222)，该漏洞允许通过容器突破获得 Docker Desktop VM 访问权限的攻击者通过传递扩展和仪表板相关的 IPC 消息进一步逃逸到主机。由 Billy Jheng Bing-Jhong、Đỗ Minh Tuấn、Muhammad Alifa Ramdhan 与 Trend Micro Zero Day Initiative 合作报告。

### 已知问题

#### Mac

- 如果未安装 Rosetta，配备 Apple Silicon 的 Docker Desktop 无法启动。这将在未来的版本中修复。请参阅 [docker/for-mac#7243](https://github.com/docker/for-mac/issues/7243)。
...
- [设置管理](/manuals/enterprise/security/hardened-desktop/settings-management/_index.md) 现在允许管理员设置默认文件共享实现，并指定开发人员可以向其添加文件共享的路径。
...
- 构建视图：
  ...
  - 修复了上下文构建器事件并不总是触发的问题。
  - 重新设计了空状态视图，使仪表板更清晰。

#### Mac

- 修复了 Rosetta 的 `httpd` 问题。[docker/for-mac#7182](https://github.com/docker/for-mac/issues/7182)
- 修复了在 `virtualization.framework` 上导致崩溃的错误。修复了 [docker/for-mac#7024](https://github.com/docker/for-mac/issues/7024)

#### Windows

- 修复了 Windows 上 DNS 超时的问题。
- 在 WSL 用户发行版中添加了对增强型容器隔离 Docker 套接字挂载权限的支持。
- 修复了从 CLI 重定向输出时导致 `failed to get console mode` 错误的问题。
- 修复了在容器内挂载时引擎套接字权限的问题。修复了 [docker/for-win#13898](https://github.com/docker/for-win/issues/13898)

### 已知问题

#### Windows

- 在深色模式下，**资源**>**高级** 设置中的 **磁盘映像位置** 不可见。作为解决方法，请切换到浅色模式。
...
- 修复了使用设置管理时，默认设置值在启动时重置为默认值的错误。
- 修复了即使禁用了 **Docker Desktop 启动时打开 Docker 仪表板** 选项，仪表板在启动时也会显示的错误。修复了 [docker/for-win#13887](https://github.com/docker/for-win/issues/13887)。
- 修复了构建后端服务中的错误，该错误导致 Docker Desktop 崩溃。修复了 [docker/for-win#13885](https://github.com/docker/for-win/issues/13885), [docker/for-win#13896](https://github.com/docker/for-win/issues/13896), [docker/for-win#13899](https://github.com/docker/for-win/issues/13899), [docker/for-mac#7164](https://github.com/docker/for-mac/issues/7164), [docker/for-mac#7169](https://github.com/docker/for-mac/issues/7169)。
- 修复了在容器内挂载时 Docker Engine 套接字权限的问题。修复了 [docker/for-win#13898](https://github.com/docker/for-win/issues/13898)。
- Docker Scout：
...
- 构建视图：
  ...
  - 内联缓存和 Git 源现在在 **信息** 选项卡的 **构建时间** 部分中得到正确处理。
  - 现在在 **历史记录** 选项卡的过去构建中显示使用的构建器和调用构建的作者。
  - 对更好地链接 **历史记录** 选项卡上的过去构建进行了多项改进。
  - 对使构建名称更准确进行了多项改进。
  - 修复了当构建器无法访问时，**活动构建** 列表中卡住的构建。
  - 修复了在某些情况下阻止删除构建记录的问题。
  - 修复了构建名称可能为空的问题。
  - 修复了在启用资源节省器模式时构建视图的一般问题。

#### Mac

- 启用了 `Huge Pages` 并修复了 Rosetta 下的 PHP 分段错误。修复了 [docker/for-mac#7117](https://github.com/docker/for-mac/issues/7117)。
...
- 修复了当 `admin-settings.json` 语法无效时导致 Docker Desktop 可能死锁的错误。

#### Windows

- 修复了在某些语言环境下无法将 UTF-16 字符串编码为 UTF-8 的错误。修复了 [docker/for-win#13868](https://github.com/docker/for-win/issues/13868)。
- 修复了在应用程序重启时，使用 WSL 集成时凭据存储配置会重置的错误。修复了 [docker/for-win#13529](https://github.com/docker/for-win/issues/13529)。
...
- 将 CLI 二进制文件切换到与旧版 glibc 兼容的版本，例如 Ubuntu 20.04 中使用的版本，修复了 [docker/for-win#13824](https://github.com/docker/for-win/issues/13824)
...
- 管理员现在可以使用[设置管理](/enterprise/security/hardened-desktop/settings-management/_index.md) 控制对 **功能开发中** 选项卡中 Beta 和实验性功能的访问。
- 在页脚引入了四种新的版本更新状态。
...
- 修复了 Rosetta 不工作的几个问题。修复了 [[docker/for-mac#6973](https://github.com/docker/for-mac/issues/6973), [[docker/for-mac#7009](https://github.com/docker/for-mac/issues/7009), [[docker/for-mac#7068](https://github.com/docker/for-mac/issues/7068) 和 [[docker/for-mac#7075](https://github.com/docker/for-mac/issues/7075)
- 改进了 Rosetta 下 NodeJS 的性能。
- 修复了 **无法打开 /proc/self/exe** Rosetta 错误。
- 修复了 **登录时启动 Docker Desktop** 设置不起作用的错误。修复了 [docker/for-mac#7052](https://github.com/docker/for-mac/issues/7052)。
- 您现在可以通过 UI 启用 UDP 的内核网络路径使用。修复了 [docker/for-mac#7008](https://github.com/docker/for-mac/issues/7008)。
...
- 添加了 DLL 和 VBS 文件缺失的签名。

### 已知问题

#### Windows

- 在较旧的 Linux 发行版（例如 Ubuntu 20.04）上使用 WSL 2 集成时，Docker CLI 不起作用，该发行版使用的 `glibc` 版本低于 `2.32`。这将在未来的版本中修复。请参阅 [docker/for-win#13824](https://github.com/docker/for-win/issues/13824)。
...
- 修复了在 WSL 2 上的 Docker Desktop 在空闲时会意外关闭 dockerd 的错误。修复了 [docker/for-win#13789](https://github.com/docker/for-win/issues/13789)
...
- 修复了 4.25 中的回归问题，即如果交换文件损坏，Docker 将无法启动。损坏的交换文件将在下次启动时重新创建。
- 修复了禁用交换时的错误。修复了 [docker/for-mac#7045](https://github.com/docker/for-mac/issues/7045), [docker/for-mac#7044](https://github.com/docker/for-mac/issues/7044) 和 [docker/for-win#13779](https://github.com/docker/for-win/issues/13779)。
- `sysctl vm.max_map_count` 现在设置为 262144。请参阅 [docker/for-mac#7047](https://github.com/docker/for-mac/issues/7047)

#### Windows

- 修复了对于某些用户，托盘菜单中未显示 **切换到 Windows 容器** 的问题。请参阅 [docker/for-win#13761](https://github.com/docker/for-win/issues/13761)。
...
- Rosetta 现已对 macOS 13 或更高版本的所有用户正式可用。它在 Apple Silicon 上提供基于 Intel 的镜像的更快模拟。要使用 Rosetta，请参阅[设置](/manuals/desktop/settings-and-maintenance/settings.md)。在 macOS 14.1 及更高版本上默认启用 Rosetta。
- Docker Desktop 现在检测 WSL 版本是否过时。如果检测到过时的 WSL 版本，您可以允许 Docker Desktop 自动更新安装，也可以在 Docker Desktop 外手动更新 WSL。
- Windows 版 Docker Desktop 的新安装现在需要 Windows 版本 19044 或更高版本。
- 管理员现在可以在[设置管理](/enterprise/security/hardened-desktop/settings-management/_index.md) 中控制 Docker Scout 镜像分析。
...
- 修复了 `接受许可` 弹出窗口中的间距问题。
- 修复了在 **通知列表** 和 **通知详细信息** 视图之间导航时 **通知抽屉** 会改变大小的错误。
- containerd 集成：
  ...
  - 修复了注册表镜像不起作用的问题。
  - 修复了在多次并发调用同一容器的 `docker diff` 时无法正常工作的问题。
  - 修复了 `docker push` 在将层推送到同一注册表上的不同存储库时未重用层的问题。
- Docker Init：
  - 修复了生成的文件中包含的过时 Docker 文档链接
  - 添加了对 ASP.NET Core 8 的支持（除了 6 和 7）
- 修复了安装 Wasm shim 时导致失败的错误。
- 修复了 Docker Desktop 每 15 分钟退出[资源节省器模式](https://docs.docker.com/desktop/use-desktop/resource-saver/)，或者，如果计时器设置为超过 15 分钟，资源节省器模式永远不会启动的错误。
- 将 **启用后台 SBOM 索引** 选项提升到 **常规设置**。

#### Mac

- 在 macOS 上安装或更新 Docker Desktop 的最低操作系统版本现在是 macOS Monterey（版本 12）或更高版本。
- 如果用户与 `Docker.app` 的所有者不匹配，增强了无法完成更新时的错误消息。修复了 [docker/for-mac#7000](https://github.com/docker/for-mac/issues/7000)。
- 修复了当 `/var/run/docker.sock` 配置错误时，**重新应用配置** 可能不起作用的错误。
- 如果 `/usr/local/bin` 中已存在 `ECRCredentialHelper`，Docker Desktop 不会覆盖它。

#### Windows

- 修复了在 Windows 家庭版上托盘菜单中显示 **切换到 Windows 容器** 的问题。修复了 [docker/for-win#13715](https://github.com/docker/for-win/issues/13715)

#### Linux

- 修复了 `docker login` 中的错误。修复了 [docker/docker-credential-helpers#299](https://github.com/docker/docker-credential-helpers/issues/299)

### 已知问题

#### Mac

- 即使禁用了自动更新选项，升级到 MacOS 14 也可能导致 Docker Desktop 也更新到最新版本。
- 无法从命令行卸载 Docker Desktop。作为解决方法，您可以[从仪表板卸载 Docker Desktop](https://docs.docker.com/desktop/uninstall/)。

#### Windows

- 托盘菜单中的 **切换到 Windows 容器** 选项可能在 Windows 上不显示。作为解决方法，请编辑 [`settings.json` 文件](/manuals/desktop/settings-and-maintenance/settings.md) 并设置 `"displaySwitchWinLinContainers": true`。

#### 所有平台

- 如果交换文件大小设置为 0MB，Docker 操作（例如拉取镜像或登录）会失败并出现 'connection refused' 或 'timeout' 错误。作为解决方法，请在 **设置** 的 **资源** 选项卡中将交换文件大小配置为非零值。
...
- 修复了 Docker Desktop 会向 `notify.bugsnag.com` 发送多个请求的错误。修复了 [docker/for-win#13722](https://github.com/docker/for-win/issues/13722)。
- 修复了 PyTorch 的性能回归问题。
...
- 修复了 Windows 版 Docker Desktop 上的错误，即 Docker Desktop 仪表板无法正确显示容器日志。修复了 [docker/for-win#13714](https://github.com/docker/for-win/issues/13714)。
...
- 新的通知中心现已对所有用户开放，因此您可以收到有关新版本、安装进度更新等的通知。选择 Docker Desktop 仪表板右下角的铃铛图标可访问通知中心。
- Compose Watch 现已对所有用户开放。有关更多信息，请参阅[使用 Compose Watch](/manuals/compose/how-tos/file-watch.md)。
- 资源节省器现已对所有用户开放，并默认启用。要配置此功能，请导航到 **设置** 中的 **资源** 选项卡。有关更多信息，请参阅 [Docker Desktop 的资源节省器模式](use-desktop/resource-saver.md)。
...
- 为 **容器** 视图使用的资源使用统计轮询添加了超时。修复了 [docker/for-mac#6962](https://github.com/docker/for-mac/issues/6962)。
- containerd 集成：
  - 实现了推送/拉取/保存镜像事件。
  - 实现了拉取旧版 schema1 镜像。
  ...
  - 修复了在使用多个 COPY/ADD 指令时旧版构建器构建错误的问题。
...
- 修复了在 macOS 版本早于 12.5 的 Apple Silicon Mac 上的内核恐慌。修复了 [docker/for-mac#6975](https://github.com/docker/for-mac/issues/6975)。
- 修复了当 `filesharingDirectories` 中包含无效目录时 Docker Desktop 无法启动的错误。修复了 [docker/for-mac#6980](https://github.com/docker/for-mac/issues/6980)。
- 修复了安装程序创建 root 拥有目录的错误。修复了 [docker/for-mac#6984](https://github.com/docker/for-mac/issues/6984)。
- 修复了当缺少 `/Library/LaunchDaemons` 时，安装程序在设置 docker 套接字时失败的错误。修复了 [docker/for-mac#6967](https://github.com/docker/for-mac/issues/6967)。
- 修复了在 macOS 上将特权端口绑定到非本地主机 IP 时出现权限被拒绝的错误。修复了 [docker/for-mac#697](https://github.com/docker/for-mac/issues/6977)。
- 修复了 4.23 中引入的资源泄漏。与 [docker/for-mac#6953](https://github.com/docker/for-mac/issues/6953) 相关。

#### Windows

- 修复了当服务已经在运行时出现“Docker Desktop 服务未运行”弹出窗口的错误。请参阅 [docker/for-win#13679](https://github.com/docker/for-win/issues/13679)。
- 修复了导致 Docker Desktop 在 Windows 主机上无法启动的错误。修复了 [docker/for-win#13662](https://github.com/docker/for-win/issues/13662)。
- 修改了 Docker Desktop 资源节省器功能，以在 WSL 中没有容器运行时跳过减少内核内存，因为在某些情况下这会导致超时。相反，鼓励用户通过 .wslconfig 文件直接在 WSL 上启用 "autoMemoryReclaim"（自 WSL 1.3.10 起可用）。

### 已知问题

#### Mac

- 创建端口为 53 的容器会失败，并显示错误地址 `already in use`。作为解决方法，请通过在位于 `~/Library/Group Containers/group.com.docker/settings.json` 的 `settings.json` 文件中添加 `"kernelForUDP": false` 来停用网络加速。
...
- 修复了当 Docker Desktop 处于资源节省器模式时 `docker stats` 会挂起的错误。
- 修复了通过 Docker Desktop 仪表板中的 **设置** 关闭实验性功能无法完全关闭资源节省器模式的错误。
- 修复了 **容器列表** 操作按钮被裁剪的错误。
- containerd 镜像存储：
  - 修复了与某些镜像交互时出现的 `failed to read config content` 错误。
  - 修复了使用旧版经典构建器 (`DOCKER_BUILDKIT=0`) 构建带有 `FROM scratch` 指令的 Dockerfile 的问题。
  - 修复了使用旧版经典构建器 (`DOCKER_BUILDKIT=0`) 构建镜像时出现 `mismatched image rootfs errors` 的问题。
  - 修复了 `ONBUILD` 和 `MAINTAINER` Dockerfile 指令
  - 修复了健康检查。

#### Mac

- macOS 12.5 或更高版本的所有用户现在默认开启 VirtioFS。您可以在 **常规** 选项卡的 **设置** 中恢复此设置。
- 改进了单流 TCP 吞吐量。
- 恢复了 macOS 的健康检查，如果您的系统发生更改可能导致运行 Docker 二进制文件出现问题，会通知您。

#### Linux

- 修复了打开 Docker Desktop 应用程序两次时 GUI 被终止的错误。请参阅 [docker/desktop-linux#148](https://github.com/docker/desktop-linux/issues/148)。

#### Windows

- 修复了在切换到 Windows 容器或禁用 WSL 并切换到 Hyper-V 引擎后，非管理员用户会被提示输入凭据的错误。
...
- 修复了 [CVE-2023-5165](https://www.cve.org/cverecord?id=CVE-2023-5165)，该漏洞允许通过调试 shell 绕过增强型容器隔离。受影响的功能仅适用于 Docker Business 客户，并假设用户未被授予本地 root 或管理员权限的环境。
- 修复了 [CVE-2023-5166](https://www.cve.org/cverecord?id=CVE-2023-5166)，该漏洞允许通过精心制作的扩展图标 URL 窃取访问令牌。

### 已知问题

- 在 Docker Desktop 上绑定特权端口在 macOS 上不起作用。作为解决方法，您可以使用 `0.0.0.0` 在所有接口上暴露端口，或使用 `127.0.0.1` 使用 localhost。
...
- 缓解了影响 Docker Desktop 启动和资源节省器模式的几个问题。[docker/for-mac#6933](https://github.com/docker/for-mac/issues/6933)
...
- [资源使用情况](use-desktop/container.md) 已从实验性转为 GA。
...
- [设置管理](/enterprise/security/hardened-desktop/settings-management/_index.md) 现在允许您为组织关闭 Docker 扩展。
...
- 使用[快速搜索](/manuals/desktop/use-desktop/_index.md#quick-search)，您现在可以：
  - 查找驻留在您本地系统上的任何容器或 Compose 应用。此外，您可以访问环境变量并执行基本操作，例如启动、停止或删除容器。
...
- 资源节省器现在在 Mac 上可用。当没有容器运行时，它会优化 Docker Desktop 对系统资源的使用。要访问此功能，请确保您已在设置中[开启对实验性功能的访问](/manuals/desktop/settings-and-maintenance/settings.md)。

#### Windows

- 修复了当 vpnkit 预期不运行时，自诊断工具显示误报失败的错误。修复了 [docker/for-win#13479](https://github.com/docker/for-win/issues/13479)。
- 修复了搜索栏中无效正则表达式导致错误的错误。修复了 [docker/for-win#13592](https://github.com/docker/for-win/issues/13592)。
- 资源节省器现在在 Windows Hyper-V 上可用。当没有容器运行时，它会优化 Docker Desktop 对系统资源的使用。要访问此功能，请确保您已在设置中[开启对实验性功能的访问](/manuals/desktop/settings-and-maintenance/settings.md)。
...
- 添加了对新 Wasm 运行时的支持：slight、spin 和 wasmtime。用户可以在启用 containerd 镜像存储时按需下载 Wasm 运行时。
- 向 Docker init 添加了 Rust 服务器支持。
- [**构建** 视图](use-desktop/builds.md) 的 Beta 版发布，允许您检查构建和管理构建器。这可以在 **设置** 中的 **功能开发中** 选项卡中找到。
...
- 修复了 Docker 套接字权限。修复了 [docker/for-win#13447](https://github.com/docker/for-win/issues/13447) 和 [docker/for-mac#6823](https://github.com/docker/for-mac/issues/6823)。
- 修复了在暂停时退出应用程序导致 Docker Desktop 挂起的问题。
- 修复了导致 **容器** 视图中的 **日志** 和 **终端** 选项卡内容被固定工具栏覆盖的错误 [docker/for-mac#6814](https://github.com/docker/for-mac/issues/6814)。
- 修复了在容器运行对话框中输入标签与输入值重叠的错误。修复了 [docker/for-win#13304](https://github.com/docker/for-win/issues/13304)。
- 修复了用户无法选择 Docker 扩展菜单的错误。修复了 [docker/for-mac#6840](https://github.com/docker/for-mac/issues/6840) 和 [docker/for-mac#6855](https://github.com/docker/for-mac/issues/6855)

#### Mac

- 添加了 macOS 健康检查，如果系统发生更改可能导致运行 Docker 二进制文件出现问题，会通知用户。

#### Windows

- 修复了 WSL 2 上的错误，即如果 Desktop 被暂停、终止然后重新启动，除非首先使用 `wsl --shutdown` 关闭 WSL，否则启动会挂起。
- 修复了当 wsl.exe 不在 PATH 上时 WSL 引擎的问题 [docker/for-win#13547](https://github.com/docker/for-win/issues/13547)。
- 修复了 WSL 引擎检测 Docker Desktop 发行版驱动器缺失情况的能力 [docker/for-win#13554](https://github.com/docker/for-win/issues/13554)。
...
- containerd 镜像存储：修复了加载包含证明的镜像时 `docker load` 失败的错误。
- containerd 镜像存储：修复了构建期间的默认镜像导出器。

#### Windows

- 修复了在非西方语言环境下难以解析主机上 WSL 版本的错误。修复了 [docker/for-win#13518](https://github.com/docker/for-win/issues/13518) 和 [docker/for-win#13524](https://github.com/docker/for-win/issues/13524)。
...
- 修复了 HTTP 代理中的一个错误，即 HTTP 1.0 客户端可能收到 HTTP 1.1 响应。
- 在 WSL-2 上启用了 Docker Desktop 的增强型容器隔离 (ECI) 功能。此功能需要 Docker Business 订阅。
...
- 修复了阻止容器访问 169.254.0.0/16 IP 的错误。修复了 [docker/for-mac#6825](https://github.com/docker/for-mac/issues/6825)。
...
- 修复了无法将 WSL 数据移动到不同磁盘的错误。修复了 [docker/for-win#13269](https://github.com/docker/for-win/issues/13269)。
- 修复了当 Docker Desktop 关闭时，其 WSL 发行版（docker-desktop 和 docker-desktop-data）未停止的错误，导致主机内存被不必要地消耗。
- 添加了一个新设置，允许 Windows Docker 守护进程在运行 Windows 容器时使用 Docker Desktop 的内部代理。请参阅 [Windows 代理设置](/manuals/desktop/settings-and-maintenance/settings.md#proxies)。
...
- 改进了 `docker scout compare` 命令以比较两个镜像，现在也在 `docker scout diff` 下别名。
- 在 `docker-compose` 操作失败时，向仪表板错误添加了更多详细信息 ([docker/for-win#13378](https://github.com/docker/for-win/issues/13378))。
...
- Docker Desktop 现在停止在应用程序启动时覆盖 .docker/config.json `credsStore` 键。请注意，如果您使用自定义凭据帮助程序，则 CLI `docker login` 和 `docker logout` 不会影响 UI 是否登录到 Docker。通常，最好通过 UI 登录到 Docker，因为 UI 支持多因素身份验证。
- 添加了关于即将从 Docker Desktop 中移除 Compose V1 的警告。可以使用 `COMPOSE_V1_EOL_SILENT=1` 抑制。
- 在 Compose 配置中，YAML 中的布尔字段应为 `true` 或 `false`。不推荐使用的 YAML 1.1 值（如 “on” 或 “no”）现在会产生警告。
- 改进了镜像表的 UI，允许行使用更多可用空间。
- 修复了端口转发中的各种错误。
- 修复了 HTTP 代理中的一个错误，即没有服务器名称指示 (SNI) 记录的 HTTP 请求会被拒绝并出现错误。

#### Windows

- 恢复了在 Windows 上完全修补 etc/hosts（再次包含 `host.docker.internal` 和 `gateway.docker.internal`）。对于 WSL，此行为由 **常规** 选项卡中的新设置控制。修复了 [docker/for-win#13388](https://github.com/docker/for-win/issues/13388) 和 [docker/for-win#13398](https://github.com/docker/for-win/issues/13398)。
- 修复了在更新 Docker Desktop 时桌面上出现虚假的 `courgette.log` 文件的错误。修复了 [docker/for-win#12468](https://github.com/docker/for-win/issues/12468)。
- 修复了“放大”快捷方式 (ctrl+=)。修复了 [docker/for-win#13392](https://github.com/docker/for-win/issues/13392)。
- 修复了在第二次容器类型切换后托盘菜单未正确更新的错误。修复了 [docker/for-win#13379](https://github.com/docker/for-win/issues/13379)。

#### Mac

- 在 macOS Ventura 及更高版本上使用虚拟化框架时，提高了 VM 网络性能。Mac 版 Docker Desktop 现在使用 gVisor 代替 VPNKit。要继续使用 VPNKit，请在位于 `~/Library/Group Containers/group.com.docker/settings.json` 的 `settings.json` 文件中添加 `"networkType":"vpnkit"`。
- 修复了在卸载时显示错误窗口的错误。
- 修复了 `deprecatedCgroupv1` 设置被忽略的错误。修复了 [docker/for-mac#6801](https://github.com/docker/for-mac/issues/6801)。
- 修复了 `docker pull` 返回 `EOF` 的情况。

#### Linux

- 修复了 VM 网络在 24 小时后崩溃的错误。修复了 [docker/desktop-linux#131](https://github.com/docker/desktop-linux/issues/131)。

### 安全

#### 所有平台

- 修复了一个安全漏洞，该漏洞允许用户通过从其 Docker CLI 配置文件中删除 `credsStore` 键来绕过其组织配置的镜像访问管理 (IAM) 限制，从而避免通过 `registry.json` 强制登录。仅影响 Docker Business 客户。
...
- `docker init` 的初始 Beta 版发布，根据[路线图](https://github.com/docker/roadmap/issues/453)。
- 添加了新的 **学习中心** 选项卡以帮助用户开始使用 Docker。
- 向 Docker Compose 添加了一个实验性的文件监视命令，可在您编辑和保存代码时自动更新正在运行的 Compose 服务。
...
- 您现在可以直接从市场搜索扩展，也可以使用**全局搜索**。
- 修复了 `docker buildx` 容器构建器在 24 小时后失去网络访问权限的错误。
- 减少了提示用户对 Docker Desktop 提供反馈的频率。
- 移除了最小 VM 交换大小。
- 在 HTTP 代理排除列表中添加了对子域匹配、CIDR 匹配、`.` 和 `_.` 的支持。
- 修复了透明 TLS 代理中未设置服务器名称指示 (SNI) 字段时的错误。
- 修复了 Docker Desktop 引擎状态消息中的语法错误。

### For Windows

- 修复了 `docker run --gpus=all` 挂起的错误。修复了 [docker/for-win#13324](https://github.com/docker/for-win/issues/13324)。
- 修复了注册表访问管理策略更新未下载的错误。
- Docker Desktop 现在允许在 `C:` 上启用 BitLocker 时使用 Windows 容器。
- 带有 WSL 后端的 Docker Desktop 不再需要 `com.docker.service` 特权服务永久运行。有关更多信息，请参见 [Windows 的权限要求](https://docs.docker.com/desktop/windows/permission-requirements/)。

### For Mac

- 修复了主机上存储的属性未为 VirtioFS 用户缓存的性能问题。
- 首次启动 Mac 版 Docker Desktop 时，用户会看到一个安装窗口，以确认或调整需要特权访问的配置。有关更多信息，请参见 [Mac 的权限要求](https://docs.docker.com/desktop/mac/permission-requirements/)。
...
- 修复了 [CVE-2023-1802](https://www.cve.org/cverecord?id=CVE-2023-1802)，该漏洞导致 Artifactory 集成中的安全问题会在 HTTPS 检查失败时回退到通过纯 HTTP 发送注册表凭据。仅启用了 **访问实验性功能** 的用户会受到影响。修复了 [docker/for-win#13344](https://github.com/docker/for-win/issues/13344)。

#### Mac

- 移除了 `com.apple.security.cs.allow-dyld-environment-variables` 和 `com.apple.security.cs.disable-library-validation` 权限，这些权限允许通过 `DYLD_INSERT_LIBRARIES` 环境变量将任意动态库加载到 Docker Desktop。
...
- Docker Desktop 现在附带 Docker Scout。拉取并查看来自 Docker Hub 和 Artifactory 存储库的镜像分析，获取基础镜像更新和推荐的标签和摘要，并根据漏洞信息过滤您的镜像。要了解更多信息，请参阅 [Docker Scout](../scout/_index.md)。
...
- **容器文件资源管理器** 作为实验功能提供。直接从 GUI 调试容器内的文件系统。
- 您现在可以在**全局搜索**中搜索卷。
...
- 防止透明 HTTP 代理过度修改请求。修复了 Tailscale 扩展登录，参见 [tailscale/docker-extension#49](https://github.com/tailscale/docker-extension/issues/49)。
- 修复了透明 TLS 代理中未设置服务器名称指示 (SNI) 字段时的错误。
- 在 HTTP 代理排除列表中添加了对子域匹配、CIDR 匹配、`.` 和 `*.` 的支持。
- 确保在上传诊断时尊重 HTTP 代理设置。
...
- 修复了使用现有标签进行镜像标记时 containerd 集成的问题。
- 为 containerd 集成实现了镜像上的悬空过滤器。
- 修复了当容器的镜像不再存在时 `docker ps` 失败的问题（使用 containerd 集成）。

#### Mac

- 修复了在未安装特权帮助程序工具 `com.docker.vmnetd` 的系统上下载注册表访问管理策略的问题。
- 修复了当 `/Library/PrivilegedHelperTools` 不存在时 `com.docker.vmnetd` 无法安装的错误。
- 修复了“系统”代理无法处理“自动代理”/“pac 文件”配置的错误。
- 修复了在区分大小写的文件系统上 vmnetd 安装无法读取 `Info.Plist` 的错误。实际文件名是 `Info.plist`。修复了 [docker/for-mac#6677](https://github.com/docker/for-mac/issues/6677)。
- 修复了在每次启动时提示用户创建 docker 套接字符号链接的错误。修复了 [docker/for-mac#6634](https://github.com/docker/for-mac/issues/6634)。
- 修复了 **登录时启动 Docker Desktop** 设置不起作用的错误。修复了 [docker/for-mac#6723](https://github.com/docker/for-mac/issues/6723)。
- 修复了 UDP 连接跟踪和 `host.docker.internal`。修复了 [docker/for-mac#6699](https://github.com/docker/for-mac/issues/6699)。
- 改进了 kubectl 符号链接逻辑，以尊重 `/usr/local/bin` 中现有的二进制文件。修复了 [docker/for-mac#6328](https://github.com/docker/for-mac/issues/6328)。
...
- 修复了在 WSL 2 模式下（在 4.16 中引入）Docker Desktop 卡在“启动”阶段的问题。
- 修复了当在 `%LOCALAPPDATA%` 上启用文件系统压缩或加密时，Docker Desktop 无法启动 WSL 2 后端的问题。
- 修复了在启动时 Docker Desktop 无法报告缺少或过时（无法运行 WSL 版本 2 发行版）的 WSL 安装的问题。
- 修复了如果目标路径有空格，在 Visual Studio Code 中打开失败的错误。
- 修复了导致 `~/.docker/context` 损坏和错误消息“意外的 JSON 输入结束”的错误。您也可以删除 `~/.docker/context` 来解决此问题。
- 确保在 WSL 2 中使用的凭据帮助程序已正确签名。与 [docker/for-win#10247](https://github.com/docker/for-win/issues/10247) 相关。
- 修复了导致 WSL 集成代理被错误终止的问题。与 [docker/for-win#13202](https://github.com/docker/for-win/issues/13202) 相关。
- 修复了启动时损坏的上下文。修复了 [docker/for-win#13180](https://github.com/docker/for-win/issues/13180) 和 [docker/for-win#12561](https://github.com/docker/for-win/issues/12561)。

### For Linux

- 为 Docker Desktop for Linux 添加了 Docker Buildx 插件。
- 将 RPM 和 Arch Linux 发行版的压缩算法更改为 `xz`。
- 修复了在 Debian 包的根目录中留下剩余文件的错误。修复了 [docker/for-linux#123](https://github.com/docker/desktop-linux/issues/123)。

### 安全

#### 所有平台

- 修复了 [CVE-2023-0628](https://www.cve.org/cverecord?id=CVE-2023-0628)，该漏洞允许攻击者在开发环境容器初始化期间通过诱骗用户打开精心制作的恶意 `docker-desktop://` URL 在其中执行任意命令。
- 修复了 [CVE-2023-0629](https://www.cve.org/cverecord?id=CVE-2023-0629)，该漏洞允许非特权用户通过将 Docker 主机设置为 `docker.raw.sock` 或 Windows 上的 `npipe:////.pipe/docker_engine_linux`（通过 `-H` (`--host`) CLI 标志或 `DOCKER_HOST` 环境变量）来绕过增强型容器隔离 (ECI) 限制，并在没有 ECI 提供的额外强化功能的情况下启动容器。这不会影响已在运行的容器，也不会影响通过常规方法（不使用 Docker 的原始套接字）启动的容器。
...
- 修复了当在 `%LOCALAPPDATA%` 上启用文件系统压缩或加密时，Docker Desktop 无法启动 WSL 2 后端的问题。修复了 [docker/for-win#13184](https://github.com/docker/for-win/issues/13184)。
- 修复了在启动时 Docker Desktop 无法报告缺少或过时的 WSL 安装的问题。修复了 [docker/for-win#13184](https://github.com/docker/for-win/issues/13184)。
...
- 修复了当启用 containerd 集成功能时，`docker build` 和 `docker tag` 命令产生 `image already exists` 错误的问题。
- 修复了 Docker Desktop 4.16 引入的回归问题，该问题导致在 amd64 系统上使用目标平台 linux/386 的容器网络中断。修复了 [docker/for-mac/6689](https://github.com/docker/for-mac/issues/6689)。

#### Mac

- 修复了 `Info.plist` 的大小写问题，该问题导致 `vmnetd` 在区分大小写的文件系统上中断。修复了 [docker/for-mac/6677](https://github.com/docker/for-mac/issues/6677)。

#### Windows

- 修复了 Docker Desktop 4.16 引入的回归问题，导致在 WSL2 模式下卡在“启动”阶段。修复了 [docker/for-win/13165](https://github.com/docker/for-win/issues/13165)
...
- 修复了容器内的 `sudo` 因某些镜像出现安全相关错误而失败的问题。修复了 [docker/for-mac/6675](https://github.com/docker/for-mac/issues/6675) 和 [docker/for-win/13161](https://github.com/docker/for-win/issues/13161)。
...
- 修复了使用 `containerd` 集成时 `docker inspect` 不显示镜像标签的问题。
- 增加了正在运行和已停止容器图标之间的对比度，使色盲用户更容易扫描容器列表。
- 修复了在 Docker Desktop 重启之前，用户被重复提示输入新的 HTTP 代理凭据的错误。
- 添加了诊断命令 `com.docker.diagnose login` 以检查 HTTP 代理配置。
- 修复了 compose 堆栈上的操作无法正常工作的问题。修复了 [docker/for-mac#6566](https://github.com/docker/for-mac/issues/6566)。
...
- 修复了工具提示阻止操作按钮的问题。修复了 [docker/for-mac#6516](https://github.com/docker/for-mac/issues/6516)。
- 修复了 **容器** 视图中空白的“发生错误”容器列表。

#### Mac

- 在 macOS 上安装或更新 Docker Desktop 的最低操作系统版本现在是 macOS Big Sur（版本 11）或更高版本。
- 修复了当启用增强型容器隔离时，如果使用旧的 `osxfs` 实现进行文件共享，Docker 引擎无法启动的问题。
- 修复了在 VirtioFS 上创建的文件设置了可执行位的问题。修复了 [docker/for-mac#6614](https://github.com/docker/for-mac/issues/6614)。
- 重新添加了从命令行卸载 Docker Desktop 的方法。修复了 [docker/for-mac#6598](https://github.com/docker/for-mac/issues/6598)。
...
- 将 **设置** 中的磁盘映像大小更改为使用十进制系统（基数 10），以与 Finder 显示磁盘容量的方式一致。
- 修复了在网络负载下 Docker 崩溃的问题。修复了 [docker/for-mac#6530](https://github.com/docker/for-mac/issues/6530)。
- 修复了导致 Docker 在每次重启后提示用户安装 `/var/run/docker.sock` 符号链接的问题。
- 确保安装 `/var/run/docker.sock` 符号链接的登录项已签名。
- 修复了在出厂重置时删除 `$HOME/.docker` 的错误。

### For Windows

- 修复了在打印“加载元数据”时 `docker build` 挂起的错误。修复了 [docker/for-win#10247](https://github.com/docker/for-win/issues/10247)。
...
- 为 macOS 用户带来大幅性能改进，并提供了启用新的 VirtioFS 文件共享技术的选项。适用于 macOS 12.5 及以上版本。
- Docker Desktop for Mac 在安装或首次运行时不再需要安装特权辅助进程 `com.docker.vmnetd`。更多信息请参阅 [Mac 的权限要求](https://docs.docker.com/desktop/mac/permission-requirements/)。
- 新增了 [WebAssembly 功能](/manuals/desktop/features/wasm.md)。可与 [containerd 集成](/manuals/desktop/features/containerd.md) 配合使用。
- 改进了 Beta 和实验性设置的描述，以清晰地解释它们之间的区别以及用户如何访问它们。
- VM 的可用磁盘空间现在显示在 Docker Desktop 适用于 Mac 和 Linux 的仪表板页脚中。
- 如果可用空间低于 3GB，页脚现在会显示磁盘空间警告。
- 随着 Docker Desktop 界面变得更加符合 ADA 无障碍标准且视觉上更加统一，我们对界面进行了更改。
- 在 **扩展** 中添加了一个 **构建** 选项卡，其中包含构建扩展所需的所有资源。
- 新增了更轻松地共享扩展的功能，既可以使用 `docker extension share` CLI，也可以在扩展的 **管理** 选项卡中使用共享按钮。
- 市场 中的扩展现在显示安装次数。您也可以按安装次数对扩展进行排序。
- 开发环境 允许将 Git 仓库克隆到本地绑定挂载，因此您可以使用任何本地编辑器或 IDE。
...
- 现在在启用 containerd 集成的情况下，重启后会恢复容器。
- 修复了启用 containerd 集成时列出多平台镜像的问题。
- 更好地处理启用 containerd 集成时的悬空镜像。
- 为镜像实现了“引用”过滤器（启用 containerd 集成时）。
- 在容器、`docker pull` 等环境中，增加了通过 `proxy.pac` 自动选择上游 HTTP/HTTPS 代理的支持。
- 修复了拉取镜像时解析镜像引用的回归问题。修复了 [docker/for-win#13053](https://github.com/docker/for-win/issues/13053)、[docker/for-mac#6560](https://github.com/docker/for-mac/issues/6560) 和 [docker/for-mac#6540](https://github.com/docker/for-mac/issues/6540)。
...
- 当 Docker Desktop 使用“系统”代理时，如果 Windows 设置发生更改，Docker Desktop 现在会在不重启的情况下使用新的 Windows 设置。

#### 适用于 Linux

- 修复了 Linux 上的热重载问题。修复了 [docker/desktop-linux#30](https://github.com/docker/desktop-linux/issues/30)。
- 在 Linux 上禁用了托盘图标动画，这修复了一些用户的崩溃问题。
...
- 修复了启用注册表访问管理 时容器的 DNS 查找问题。

#### 适用于 Mac

- 修复了一个阻止 **镜像** 选项卡上的 **分析镜像** 按钮正常工作的问题。
- 修复了一个导致在 `/usr/local/lib` 不存在时无法为用户创建符号链接的缺陷。修复了 [docker/for-mac#6569](https://github.com/docker/for-mac/issues/6569)
...
- 将 Virtualization 框架设置为 macOS >= 12.5 的默认虚拟机监控程序。
- 对于 macOS >= 12.5，将之前的安装迁移到 Virtualization 框架虚拟机监控程序。
- 适用于 Docker Business 用户的增强型容器隔离 功能现在可以从常规设置中启用。

### 更新

- [Docker Engine v20.10.21](/manuals/engine/release-notes/20.10.md#201021),
  其中包含针对 [CVE-2022-39253](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-39253) 跟踪的 Git 漏洞的缓解措施，
  并更新了对 `image:tag@digest` 镜像引用的处理。
...
- Docker Desktop 现在需要一个大小为 /24 的内部网络子网。如果您之前使用的是 /28，它将自动扩展为 /24。如果您遇到网络问题，请检查 Docker 子网与您的基础架构之间是否存在冲突。修复了 [docker/for-win#13025](https://github.com/docker/for-win/issues/13025)。
- 修复了当 Git URL 包含大写字符时阻止用户创建开发环境的问题。
- 修复了诊断中报告的 `vpnkit.exe is not running` 错误。
- 将 qemu 回滚到 6.2.0，以修复运行模拟的 amd64 代码时出现 `PR_SET_CHILD_SUBREAPER is unavailable` 等错误。
- 在扩展 中启用了 [contextIsolation](https://www.electronjs.org/docs/latest/tutorial/context-isolation) 和 [sandbox](https://www.electronjs.org/docs/latest/tutorial/sandbox) 模式。现在扩展在单独的上下文中运行，这通过限制对大多数系统资源的访问来限制恶意代码可能造成的危害。
- 包含了 `unpigz` 以允许对拉取的镜像进行并行解压缩。
- 修复了与对选定容器执行操作相关的问题。修复了 https://github.com/docker/for-win/issues/13005
- 新增了允许您为容器或项目视图显示时间戳的功能。
- 修复了使用 Control+C 中断 `docker pull` 时可能出现的段错误。
- 增加了默认的 DHCP 租约时间，以避免 VM 的网络每两小时出现故障并断开连接。
- 移除了容器列表上的无限加载动画。修复了 https://github.com/docker/for-mac/issues/6486
- 修复了 **设置** 中已用空间显示不正确的错误。
- 修复了一个导致 Kubernetes 在启用 containerd 集成时无法启动的错误。
- 修复了一个导致 `kind` 在启用 containerd 集成时无法启动的错误。
- 修复了一个导致开发环境 在启用 containerd 集成时无法工作的错误。
- 在 containerd 集成中实现了 `docker diff`。
- 在 containerd 集成中实现了 `docker run —-platform`。
- 修复了一个导致不安全的注册表在启用 containerd 集成时无法工作的错误。

#### 适用于 Mac

- 修复了使用 Virtualization 框架的用户遇到的启动失败问题。
- 在 Mac 上默认重新添加了 `/var/run/docker.sock`，以增加与 `tilt` 和 `docker-py` 等工具的兼容性。
- 修复了一个阻止在新 Mac 安装上创建开发环境的问题（错误“Cannot connect to the Docker daemon at unix:///var/run/docker.sock. Is the docker daemon running?”）。
...
- 修复了 Compose v2 在 Windows 上未始终默认启用的问题。
- Docker Desktop 现在会在卸载时删除 `C:\Program Files\Docker` 文件夹。

### 已知问题

- 对于一些 Mac OS 用户，安装程序存在一个已知问题，会阻止安装 Docker Desktop 中实验性漏洞和包发现功能所需的新辅助工具。要解决此问题，需要创建一个符号链接，可以使用以下命令创建：`sudo ln -s /Applications/Docker.app/Contents/Resources/bin/docker-index /usr/local/bin/docker-index`
...
- 在 Mac 上默认重新添加了 `/var/run/docker.sock` 符号链接，以增加与 `tilt` 和 `docker-py` 等工具的兼容性。修复了 [docker/for-mac#6529](https://github.com/docker/for-mac/issues/6529)。
- 修复了一个阻止在新 Mac 安装上创建开发环境并导致 `error "Cannot connect to the Docker daemon at unix:///var/run/docker.sock. Is the docker daemon running?"` 的问题。
...
- 为 Docker Business 用户引入了两项新的安全功能：设置管理 和增强型容器隔离。详细了解 Docker Desktop 新的 [加固的 Docker Desktop 安全模型](/manuals/enterprise/security/hardened-desktop/_index.md)。
- 新增了新的开发环境 CLI `docker dev`，因此您可以通过命令行创建、列出和运行开发环境。现在将开发环境集成到自定义脚本中更加容易。
...
- 将 Docker Engine 和 Docker CLI 更新到 [v20.10.20](/manuals/engine/release-notes/20.10.md#201020)，
  其中包含针对 [CVE-2022-39253](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-39253) 跟踪的 Git 漏洞的缓解措施，
  更新了对 `image:tag@digest` 镜像引用的处理，以及对 [CVE-2022-36109](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-36109) 的修复。
...
- 在 Docker Desktop VM 内用于 cpu 模拟的 [Qemu 7.0.0](https://wiki.qemu.org/ChangeLog/7.0)。
...
- 如果 Docker Desktop 检测到 HTTP 或 HTTPS 代理密码已更改，它会提示开发者输入新密码。
- **对这些主机和域绕过代理设置** 设置现在可以正确处理域名的 HTTPS。
- **远程仓库** 视图和每日提示 现在可以与需要身份验证的 HTTP 和 HTTPS 代理一起工作。
- 我们为产品开发生命周期早期阶段的功能引入了暗发布。选择加入的用户可以随时在设置的“beta 功能”部分选择退出。
- 为扩展市场 添加了类别。
- 当有扩展更新可用时，在鲸鱼菜单和 **扩展** 选项卡上添加了一个指示器。
- 修复了因镜像名称没有命名空间（如 'my-extension'）而导致扩展卸载失败的问题。
- 在 **容器** 选项卡中明确显示端口映射。
- 将镜像磁盘使用信息的刷新频率更改为每天自动进行一次。
- 使 **容器** 和 **卷** 选项卡的标签样式保持一致。
- 修复了 **设置** 中 Grpcfuse 文件共享模式的启用问题。修复了 [docker/for-mac#6467](https://github.com/docker/for-mac/issues/6467)
- 对于运行 macOS < 12.5 的用户，Virtualization Framework 和 VirtioFS 已被禁用。
- **容器** 选项卡上的端口现在可以点击。
...
- 做了一个小改进，当点击侧边栏中的 **扩展** 时，可以导航到扩展市场。
- 在市场中添加了一个徽章以识别新的扩展。
- 修复了 kubernetes 在启用 containerd 集成时无法启动的问题。
- 修复了 `kind` 在启用 containerd 集成时无法启动的问题。
- 修复了开发环境在启用 containerd 集成时无法工作的问题。
- 在 containerd 集成中实现了 `docker diff`。
- 在 containerd 集成中实现了 `docker run —-platform`。
- 修复了不安全的注册表在启用 containerd 集成时无法工作的问题。
- 修复了一个导致 **设置** 中已用空间显示不正确的错误。
- Docker Desktop 现在从 Github releases 安装凭据助手。参见 [docker/for-win#10247](https://github.com/docker/for-win/issues/10247)、[docker/for-win#12995](https://github.com/docker/for-win/issues/12995)。
- 修复了导致用户在 7 天后被登出 Docker Desktop 的问题。

#### 适用于 Mac

- 为 Docker Desktop 添加了 **隐藏**、**隐藏其他**、**显示全部** 菜单项。参见 [docker/for-mac#6446](https://github.com/docker/for-mac/issues/6446)。
- 修复了一个导致从已安装的应用程序运行安装实用程序时该应用程序被删除的错误。修复了 [docker/for-mac#6442](https://github.com/docker/for-mac/issues/6442)。
...
- 新增了使用 containerd 来拉取和存储镜像的功能。这是一个实验性功能。
- Docker Desktop 现在可以运行未打标签的镜像。修复了 [docker/for-mac#6425](https://github.com/docker/for-mac/issues/6425)。
- 为 Docker Extension 的市场 添加了搜索功能。修复了 [docker/roadmap#346](https://github.com/docker/roadmap/issues/346)。
- 新增了放大、缩小或将 Docker Desktop 设置为实际大小的功能。这可以通过在 Mac 和 Windows 上分别使用键盘快捷键 ⌘ + / CTRL +、⌘ - / CTRL -、⌘ 0 / CTRL 0，或者通过 Mac 上的“视图”菜单来完成。
- 如果任何相关容器是可停止的，则添加了 compose 停止按钮。
- 现在可以从 **容器** 视图中删除单个 compose 容器。
- 移除了针对 Fedora 35 上 virtiofsd <-> qemu 协议不匹配的临时解决方案，因为不再需要它。Fedora 35 用户应将 qemu 包升级到最新版本（在撰写本文时为 qemu-6.1.0-15.fc35）。
- 为容器实现了集成终端。
- 默认添加了一个工具提示来显示所有外部链接的链接地址。
...
- 修复了 [CVE-2023-0626](https://www.cve.org/cverecord?id=CVE-2023-0626)，该漏洞允许通过 Electron 客户端中消息框路由的查询参数实现 RCE。
- 修复了 [CVE-2023-0625](https://www.cve.org/cverecord?id=CVE-2023-0625)，该漏洞允许通过扩展描述/更新日志实现 RCE，这可能被恶意扩展滥用。

#### 适用于 Windows

- 修复了 [CVE-2023-0627](https://www.cve.org/cverecord?id=CVE-2023-0627)，该漏洞允许绕过 4.11 版本中引入的 `--no-windows-containers` 安装标志。此标志允许管理员禁用 Windows 容器的使用。
- 修复了 [CVE-2023-0633](https://www.cve.org/cverecord?id=CVE-2023-0633)，其中对 Docker Desktop 安装程序的参数注入可能导致本地权限提升。

### 错误修复和次要增强

#### 适用于所有平台

- 恢复出厂设置后现在会启用 Compose V2。
- 在新安装的 Docker Desktop 上，Compose V2 现在默认启用。
- Compose 中环境变量的优先级顺序更加一致，并有清晰的[文档](/manuals/compose/how-tos/environment-variables/envvars-precedence.md)。
- 将内核升级到 5.10.124。
- 改进了因计算磁盘大小导致的整体性能问题。与 [docker/for-win#9401](https://github.com/docker/for-win/issues/9401) 相关。
- Docker Desktop 现在阻止未安装 Rosetta 的 ARM Mac 用户切换回只有 intel 二进制文件的 Compose V1。
- 将卷大小和 **创建时间** 列以及容器的 **启动时间** 列的默认排序顺序更改为降序。
- 通过始终仅将启动/停止和删除操作可见，并通过行菜单项访问其余操作，来重新组织容器行操作。
- 快速入门指南现在立即运行每个命令。
- 定义了容器/compose **状态** 列的排序顺序为：运行 > 部分运行 > 已暂停 > 部分暂停 > 已退出 > 部分退出 > 已创建。
- 修复了即使在 Docker Desktop 中存在镜像，镜像列表在 Docker Desktop 中显示为空的问题。与 [docker/for-win#12693](https://github.com/docker/for-win/issues/12693) 和 [docker/for-mac#6347](https://github.com/docker/for-mac/issues/6347) 相关。
- 根据是否显示系统容器来定义哪些镜像“正在使用”。如果未显示与 Kubernetes 和扩展相关的系统容器，则相关镜像不被定义为“正在使用”。
- 修复了一个导致某些语言的 Docker 客户端在 `docker exec` 上挂起的错误。修复了 [https://github.com/apocas/dockerode/issues/534](https://github.com/apocas/dockerode/issues/534)。
- 构建扩展时失败的生成命令不再导致 Docker Desktop 意外退出。
- 修复了一个导致扩展在未启用时在左侧菜单中显示为已禁用的错误。
- 修复了启用注册表访问管理并阻止访问 Docker Hub 时对私有注册表的 `docker login`。
...
- 在 macOS 上安装或更新 Docker Desktop 的最低版本现在是 10.15。修复了 [docker/for-mac#6007](https://github.com/docker/for-mac/issues/6007)。
- 修复了一个错误，该错误导致托盘菜单在下载更新后错误地显示“即将开始下载...”。修复了 [for-mac/issues#5677](https://github.com/docker/for-mac/issues/5677) 中报告的某些问题
- 修复了一个应用更新后未重启 Docker Desktop 的错误。
- 修复了一个错误，该错误导致当用户使用 virtualization.framework 和限制性防火墙软件时，计算机睡眠后与 Docker 的连接丢失。
- 修复了一个导致即使用户已退出应用程序，Docker Desktop 仍在后台运行的错误。修复了 [docker/for-mac##6440](https://github.com/docker/for-mac/issues/6440)
- 对于运行 macOS < 12.5 的用户，禁用了 Virtualization Framework 和 VirtioFS

#### 适用于 Windows

- 修复了更新期间显示版本可能不正确的错误。修复了 [for-win/issues#12822](https://github.com/docker/for-win/issues/12822)。
...
- 修复了阻止 VM 系统位置（例如 /var/lib/docker）进行绑定挂载的回归问题 [for-mac/issues#6433](https://github.com/docker/for-mac/issues/6433)
...
- Docker Desktop 现在完全支持 Docker Business 客户在 VMware ESXi 和 Azure VM 内部使用。有关更多信息，请参阅 [在 VM 或 VDI 环境中运行 Docker Desktop](/manuals/desktop/setup/vm-vdi.md)
- 在扩展市场 中添加了两个新的扩展（[vcluster](https://hub.docker.com/extensions/loftsh/vcluster-dd-extension) 和 [PGAdmin4](https://hub.docker.com/extensions/mochoa/pgadmin4-docker-extension)）。
- 在扩展市场中添加了排序扩展的功能。
- 修复了一个导致某些用户被过于频繁地要求提供反馈的错误。现在您每年只会被要求提供两次反馈。
- 为 Docker Desktop 添加了自定义主题设置。这允许您独立于设备设置将 Docker Desktop 指定为深色或浅色模式。修复了 [docker/for-win#12747](https://github.com/docker/for-win/issues/12747)
...
- 如果注册表访问管理阻止 Docker 引擎访问 Docker Hub，则改进了 `docker login` 的错误消息。
- 提高了主机与 Docker 之间的吞吐量。例如，提高了 `docker cp` 的性能。
- 收集诊断信息所需的时间更少。
- 在容器概览页面上选择或取消选择 compose 应用程序现在会选择/取消选择其所有容器。
- 容器概览页图像列上的标签名称是可见的。
- 向终端滚动条添加了搜索装饰，因此视口之外的匹配项也可见。
- 修复了搜索在容器页面上不能很好地工作的问题 [docker/for-win#12828](https://github.com/docker/for-win/issues/12828)。
- 修复了导致 **卷** 屏幕无限加载的问题 [docker/for-win#12789](https://github.com/docker/for-win/issues/12789)。
- 修复了容器 UI 中的一个问题，即调整列大小或隐藏列不起作用。修复了 [docker/for-mac#6391](https://github.com/docker/for-mac/issues/6391)。
- 修复了一个错误，当离开市场页面时，会丢失同时安装、更新或卸载多个扩展的状态。
- 修复了一个问题，即关于页面中的 compose 版本仅在重启 Docker Desktop 后才会从 v2 更新到 v1。
- 修复了一个用户因底层硬件不支持 WebGL2 渲染而无法看到日志视图的问题。修复了 [docker/for-win#12825](https://github.com/docker/for-win/issues/12825)。
- 修复了容器和镜像的 UI 不同步的错误。
...
- 修复了水平调整大小问题。修复了 [docker/for-win#12816](https://github.com/docker/for-win/issues/12816)。
- 如果在 UI 中配置了 HTTP/HTTPS 代理，它会自动将来自镜像构建和运行容器的流量发送到代理。这避免了在每个容器或构建中单独配置环境变量的需要。
...
- 修复了 UI 中的操作对从 WSL 创建的 Compose 应用程序失败的错误。修复了 [docker/for-win#12806](https://github.com/docker/for-win/issues/12806)。

#### 适用于 Mac

- 修复了因路径未初始化导致安装命令失败的错误。修复了 [docker/for-mac#6384](https://github.com/docker/for-mac/issues/6384)。
...
- 添加了使容器日志处理更容易的功能，例如正则表达式搜索以及在容器仍在运行时清除容器日志的能力。
- 在容器表上实现了反馈。添加了端口并分离了容器和镜像名称。
- 在扩展市场中添加了两个新的扩展，Ddosify 和 Lacework。

### 已移除

- 在重新设计主页期间移除了它。您可以在此处提供[反馈](https://docs.google.com/forms/d/e/1FAIpQLSfYueBkJHdgxqsWcQn4VzBn2swu4u_rMQRIMa8LExYb_72mmQ/viewform?entry.1237514594=4.10)。
...
- 在 **容器** 选项卡中为启动/暂停/停止选定的容器添加了额外的批量操作。
- 在 **容器** 选项卡中为 compose 项目添加了暂停和重启操作。
- 在 **容器** 选项卡中添加了图标和暴露的端口或退出代码信息。
- 外部 URL 现在可以使用诸如 `docker-desktop://extensions/marketplace?extensionId=docker/logs-explorer-extension` 之类的链接来引用扩展市场 中的扩展详细信息。
- Compose 应用程序的展开或折叠状态现在会被持久化。
- `docker extension` CLI 命令在 Docker Desktop 中默认可用。
- 增加了在扩展市场中显示的截图的大小。
- 修复了 Docker 扩展在其后端容器停止时无法加载的错误。修复了 [docker/extensions-sdk#16](https://github.com/docker/extensions-sdk/issues/162)。
- 修复了镜像搜索字段无故被清空的错误。修复了 [docker/for-win#12738](https://github.com/docker/for-win/issues/12738)。
- 修复了许可协议不显示并无声阻止 Docker Desktop 启动的错误。
- 修复了未发布扩展的显示镜像和标签，以实际显示来自已安装的未发布扩展的镜像和标签。
- 修复了支持屏幕上的重复页脚。
- 可以从 GitHub 仓库的子目录创建开发环境。
- 如果离线使用 Docker Desktop 时无法加载每日提示，则移除了错误消息。修复了 [docker/for-mac#6366](https://github.com/docker/for-mac/issues/6366)。

#### 适用于 Mac

- 修复了 macOS 上 bash 补全文件位置的错误。修复了 [docker/for-mac#6343](https://github.com/docker/for-mac/issues/6343)。
- 修复了用户名超过 25 个字符时 Docker Desktop 无法启动的错误。修复了 [docker/for-mac#6122](https://github.com/docker/for-mac/issues/6122)。
- 修复了因系统代理配置无效导致 Docker Desktop 无法启动的错误。修复了 [docker/for-mac#6289](https://github.com/docker/for-mac/issues/6289) 中报告的某些问题。
- 修复了启用实验性 virtualization framework 时 Docker Desktop 无法启动的错误。
- 修复了卸载 Docker Desktop 后托盘图标仍然显示的错误。

#### 适用于 Windows

- 修复了导致 Hyper-V 上 CPU 使用率过高的错误。修复了 [docker/for-win#12780](https://github.com/docker/for-win/issues/12780)。
- 修复了 Docker Desktop for Windows 无法启动的错误。修复了 [docker/for-win#12784](https://github.com/docker/for-win/issues/12784)。
- 修复了 `--backend=wsl-2` 安装程序标志未将后端设置为 WSL 2 的错误。修复了 [docker/for-win#12746](https://github.com/docker/for-win/issues/12746)。
...
- 在 `docker system prune` 期间，Docker 引擎偶尔会重启。这是当前引擎中使用的 buildkit 版本中的一个[已知问题](https://github.com/moby/buildkit/pull/2177)，将在未来版本中修复。
...
- 修复了空白仪表板 屏幕。修复了 [docker/for-win#12759](https://github.com/docker/for-win/issues/12759)。
...
- 在主页上为：Elasticsearch、MariaDB、Memcached、MySQL、RabbitMQ 和 Ubuntu 添加了额外的指南。
- 在 Docker Desktop 仪表板 添加了一个页脚，其中包含有关 Docker Desktop 更新状态和 Docker Engine 统计信息的一般信息
- 重新设计了容器表，添加了：
  - 将容器 ID 复制到剪贴板的按钮
  - 每个容器的暂停按钮
  - 容器表的列大小调整
...
- 修复了一个导致 Docker Desktop 在您暂停 Docker Desktop 时退出应用程序而挂起的问题。
- 修复了 PKI 过期后 Kubernetes 集群无法正确重置的问题。
- 修复了扩展市场 未使用定义的 http 代理的问题。
- 改进了 Docker Desktop 仪表板 中的日志搜索功能，允许使用空格。
- 在仪表板 中对按钮的鼠标中键单击现在表现为左键单击，而不是打开一个空白窗口。

#### 适用于 Mac

- 修复了一个问题，如果 `/opt` 已添加到文件共享目录列表中，则避免在主机上创建 `/opt/containerd/bin` 和 `/opt/containerd/lib`。

#### 适用于 Windows

- 修复了 WSL 2 集成中的一个错误，即如果将文件或目录绑定挂载到容器，然后容器退出，那么该文件或目录将被替换为同名的另一种类型的对象。例如，如果文件被替换为目录或目录被替换为文件，任何尝试绑定挂载新对象的操作都会失败。
- 修复了托盘图标和仪表板 UI 不显示且 Docker Desktop 未完全启动的错误。修复了 [docker/for-win#12622](https://github.com/docker/for-win/issues/12622)。

### 已知问题

#### 适用于 Linux

- 更改绑定挂载中文件的所有权失败。这是由于我们实现了运行 Docker 引擎的 VM 与主机之间文件共享的方式所致。我们计划在下一个版本中解决此问题。
...
- 修复了手动代理设置的问题，该问题在拉取镜像时造成问题。修复了 [docker/for-win#12714](https://github.com/docker/for-win/issues/12714) 和 [docker/for-mac#6315](https://github.com/docker/for-mac/issues/6315)。
- 修复了扩展禁用时 CPU 使用率高的问题。修复了 [docker/for-mac#6310](https://github.com/docker/for-mac/issues/6310)。
...
- Beta 版 [Docker 扩展](/manuals/extensions/_index.md) 和扩展 SDK。
- 创建了一个 Docker 主页，您可以在其中运行流行的镜像并发现如何使用它们。
- [Compose V2 现已正式发布](https://www.docker.com/blog/announcing-compose-v2-general-availability/)

### 错误修复和增强

- 修复了一个导致更新 Docker Desktop 时删除 Kubernetes 集群的错误。
...
- 修复了在代理后运行时仪表板 中显示远程仓库 的错误。
- 修复了 vpnkit 建立并阻塞客户端连接，即使服务器已消失的问题。参见 [docker/for-mac#6235](https://github.com/docker/for-mac/issues/6235)
- 对 Docker Desktop 中的 **卷** 选项卡进行了改进：
  - 显示卷大小。
  - 列可以调整大小、隐藏和重新排序。
  - 列的排序顺序和隐藏状态会被持久化，即使在 Docker Desktop 重启后也是如此。
  - 在选项卡之间切换时，行选择会被持久化，即使在 Docker Desktop 重启后也是如此。
- 修复了开发环境 选项卡中的一个错误，该错误未在向屏幕添加更多项目时添加滚动条。
- 标准化了仪表板 中的标题标题和操作。
- 添加了通过 HTTP 代理下载注册表访问管理 策略的支持。
- 修复了计算机长时间处于睡眠模式时出现空远程仓库的相关问题。
- 修复了悬空镜像 在清理过程中未被选中的错误，如果它们的名称未被标记为 "&lt;none>" 但其标签是。
- 改进了因需要 HTTP 代理而 `docker pull` 失败时的错误消息。
- 添加了在 Docker Desktop 中轻松清除搜索栏的功能。
- 将“容器/应用”选项卡重命名为“容器”。
- 修复了当 `C:\ProgramData\DockerDesktop` 是文件或符号链接时，Docker Desktop 安装程序中的静默崩溃。
- 修复了一个错误，即没有命名空间的镜像（例如 `docker pull <private registry>/image`）会被注册表访问管理错误地阻止，除非在设置中启用了对 Docker Hub 的访问。

#### 适用于 Mac

- Docker Desktop 的图标现在符合 Big Sur 风格指南。参见 [docker/for-mac#5536](https://github.com/docker/for-mac/issues/5536)
- 修复了重复的 Dock 图标和 Dock 图标未按预期工作的问题。修复了 [docker/for-mac#6189](https://github.com/docker/for-mac/issues/6189)。
- 改进了对 `Cmd+Q` 快捷键的支持。
...
- 目前，如果您正在运行 Kubernetes 集群，则在升级到 Docker Desktop 4.8.0 时它将被删除。我们计划在下一个版本中修复此问题。
...
- 修复了快速入门指南 最终屏幕上的崩溃问题。

#### 适用于 Windows

- 修复了更新因符号链接错误而失败的错误。修复了 [docker/for-win#12650](https://github.com/docker/for-win/issues/12650)。
- 修复了阻止使用 Windows 容器模式的错误。修复了 [docker/for-win#12652](https://github.com/docker/for-win/issues/12652)。
...
- 对新的 Kubernetes 集群使用 [cri-dockerd](https://github.com/Mirantis/cri-dockerd) 而不是 `dockershim`。从用户的角度来看，此更改是透明的，Kubernetes 容器像以前一样在 Docker 引擎上运行。`cri-dockerd` 允许 Kubernetes 使用标准的[容器运行时接口](https://github.com/kubernetes/cri-api#readme)（用于控制其他容器运行时的相同接口）来管理 Docker 容器。有关更多信息，请参阅 [Dockershim 的未来是 cri-dockerd](https://www.mirantis.com/blog/the-future-of-dockershim-is-cri-dockerd/)。
...
- UI 中的日志和终端现在在深色和浅色模式下遵循您的操作系统主题。
- 通过多选复选框轻松一次清理多个卷。
- 改进了登录反馈。

#### 适用于 Mac

- 修复了一个有时导致 Docker Desktop 显示空白白屏的问题。修复了 [docker/for-mac#6134](https://github.com/docker/for-mac/issues/6134)。
- 修复了一个在使用 Hyperkit 进行文件共享时，从睡眠状态唤醒后 gettimeofday() 性能下降的问题。修复了 [docker/for-mac#3455](https://github.com/docker/for-mac/issues/3455)。
- 修复了一个导致 Docker Desktop 在启动时使用 `osxfs` 进行文件共享时无响应的问题。

#### 适用于 Windows

- 修复了卷标题。修复了 [docker/for-win#12616](https://github.com/docker/for-win/issues/12616)。
- 修复了 WSL 2 集成中的一个错误，该错误导致重启 Docker Desktop 或切换到 Windows 容器后 Docker 命令停止工作。
...
- 防止 vpnkit-forwarder 中的无限循环用错误消息填充日志。
- 修复了未设置 HTTP 代理时的诊断信息上传。修复了 [docker/for-mac#6234](https://github.com/docker/for-mac/issues/6234)。
- 从自我诊断中移除了误报的“vm 未运行”错误。修复了 [docker/for-mac#6233](https://github.com/docker/for-mac/issues/6233)。
...
- Docker Desktop 仪表板 卷管理功能现在提供了使用多选复选框高效清理卷的功能。

#### 适用于 Mac

- Docker Desktop 4.6.0 为 macOS 用户提供了启用名为 VirtioFS 的新实验性文件共享技术的选项。在测试过程中，VirtioFS 已被证明可以大幅减少主机和 VM 之间同步更改所需的时间，从而带来显著的性能改进。有关更多信息，请参阅 [VirtioFS](/manuals/desktop/settings-and-maintenance/settings.md#beta-features)。
...
- [Go 1.17.8](https://github.com/golang/doc/go1.17)
...
- 修复了 [CVE-2022-0847](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-0847)，即“脏管道”漏洞，该漏洞可能允许攻击者从容器内部修改主机上容器镜像中的文件。
  如果使用 WSL 2 后端，您必须通过运行 `wsl --update` 来更新 WSL 2。
...
- 修复了设置 HTTPS 代理时上传诊断信息的问题。
- 使从系统托盘菜单检查更新会打开软件更新设置部分。

#### 适用于 Mac

- 修复了启动 Docker Desktop 后系统托盘菜单未显示所有菜单项的问题。修复了 [docker/for-mac#6192](https://github.com/docker/for-mac/issues/6192)。
- 修复了关于 Docker Desktop 不再在后台启动的回归问题。修复了 [docker/for-mac#6167](https://github.com/docker/for-mac/issues/6167)。
- 修复了 Docker Desktop Dock 图标缺失的问题。修复了 [docker/for-mac#6173](https://github.com/docker/for-mac/issues/6173)。
- 使用实验性的 `virtualization.framework` 时加快了块设备访问速度。参见[基准测试](https://github.com/docker/roadmap/issues/7#issuecomment-1050626886)。
- 为了更好的开箱即用性能，将默认 VM 内存分配增加到物理内存的一半（最小 2 GB，最大 8 GB）。

#### 适用于 Windows

- 修复了 UI 永远卡在 `starting` 状态的问题，尽管 Docker Desktop 从命令行运行正常。
- 修复了 Docker Desktop 系统托盘图标缺失的问题 [docker/for-win#12573](https://github.com/docker/for-win/issues/12573)
- 修复了使用最新 5.10.60.1 内核时 WSL 2 下的注册表访问管理问题。
- 修复了选择从 WSL 2 环境启动的 Compose 应用程序的容器时 UI 崩溃的问题。修复了 [docker/for-win#12567](https://github.com/docker/for-win/issues/12567)。
- 修复了从快速入门指南中的终端复制文本的问题。修复了 [docker/for-win#12444](https://github.com/docker/for-win/issues/12444)。

### 已知问题

#### 适用于 Mac

- 启用 VirtioFS 后，运行具有不同 Unix 用户 ID 的进程的容器可能会遇到缓存问题。例如，如果以 `root` 身份运行的进程查询一个文件，而以用户 `nginx` 身份运行的另一个进程立即尝试访问同一文件，则 `nginx` 进程将收到“Permission Denied”错误。
...
- 修复了 Docker Desktop 仪表板 中的崩溃问题，该问题会导致系统托盘菜单消失。

如果您在 Windows Home 上运行 Docker Desktop，安装 4.5.1 会自动将其切换回 WSL 2。如果您运行的是其他版本的 Windows，并且希望 Docker Desktop 使用 WSL 2 后端，则必须通过在 **设置 > 常规** 部分中启用 **使用基于 WSL 2 的引擎** 选项来手动切换。
或者，您可以编辑位于 `%APPDATA%\Docker\settings.json` 的 Docker Desktop 设置文件，并将 `wslEngineEnabled` 字段的值手动切换为 `true`。
...
- Docker Desktop 4.5.0 引入了新版本的 Docker 菜单，该菜单在所有操作系统上创建了一致的用户体验。有关更多信息，请参阅博客文章 [New Docker Menu & Improved Release Highlights with Docker Desktop 4.5](https://www.docker.com/blog/new-docker-menu-improved-release-highlights-with-docker-desktop-4-5/)
...
- 修复了 [CVE-2021-44719](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2021-44719)，该漏洞允许 Docker Desktop 被用于从容器访问主机上的任何用户文件，绕过共享文件夹的允许列表。
...
- 修复了一个导致 Docker Desktop 在用户退出 Docker Desktop 并启动应用程序后错误地提示用户登录的问题。
- 通过在 Linux 中设置 `fs.inotify.max_user_watches=1048576` 和 `fs.inotify.max_user_instances=8192` 来增加文件系统监视 的限制。修复了 [docker/for-mac#6071](https://github.com/docker/for-mac/issues/6071)。

#### 适用于 Mac

- 修复了当使用 `osxfs` 且没有主机目录与 VM 共享时，VM 在启动期间变得无响应的问题。
- 修复了一个不允许用户使用 Docker Desktop 仪表板 停止 Docker Compose 应用程序的问题，如果该应用程序是在不同版本的 Docker Compose 中启动的。例如，如果用户在 V1 中启动了 Docker Compose 应用程序，然后切换到 Docker Compose V2，尝试停止 Docker Compose 应用程序将失败。
- 修复了一个导致 Docker Desktop 在用户退出 Docker Desktop 并启动应用程序后错误地提示用户登录的问题。
- 修复了 **关于 Docker Desktop** 窗口不再工作的问题。
- 在 Mac M1 上将 CPU 数量限制为 8，以解决启动问题。修复了 [docker/for-mac#6063](https://github.com/docker/for-mac/issues/6063)。

#### 适用于 Windows

- 修复了与使用版本 2 启动的 compose 应用相关的问题，但仪表板只处理版本 1 的问题。

### 已知问题

#### 适用于 Windows

从头开始安装 Docker Desktop 4.5.0 有一个错误，默认将 Docker Desktop 设置为使用 Hyper-V 后端而不是 WSL 2。这意味着，Windows Home 用户将无法启动 Docker Desktop，因为 WSL 2 是唯一支持的后端。要解决此问题，您必须从计算机上卸载 4.5.0，然后下载并安装 Docker Desktop 4.5.1 或更高版本。或者，您可以编辑位于 `%APPDATA%\Docker\settings.json` 的 Docker Desktop settings.json 文件，并将 `wslEngineEnabled` 字段的值手动切换为 `true`。
...
- 通过浏览器登录后单击 **转到桌面**，有时不会将仪表板 置于前台。
- 登录后，当仪表板 获得焦点时，有时即使单击背景窗口，它也会保持在前台。作为解决方法，您需要在单击另一个应用程序窗口之前单击仪表板。
- 当通过 `registry.json` 文件启用组织限制时，每周提示会显示在强制登录对话框的顶部。
...
- 禁用了仪表板 快捷键，以防止即使在最小化或未聚焦时也捕获它们。修复了 [docker/for-win#12495](https://github.com/docker/for-win/issues/12495)。

### 已知问题

#### 适用于 Windows

- 通过浏览器登录后单击 **转到桌面**，有时不会将仪表板置于前台。
- 登录后，当仪表板获得焦点时，有时即使单击背景窗口，它也会保持在前台。作为解决方法，您需要在单击另一个应用程序窗口之前单击仪表板。
- 当通过 `registry.json` 文件启用组织限制时，每周提示会显示在强制登录对话框的顶部。
...
- 使用 Auth0 和单点登录轻松、安全地登录
  - 单点登录：拥有 Docker Business 订阅的用户现在可以配置 SSO，以使用其身份提供商进行身份验证来访问 Docker。有关更多信息，请参阅 [单点登录](/manuals/enterprise/security/single-sign-on/_index.md)。
  - 登录 Docker Desktop 现在会通过浏览器进行，以便您获得密码管理器自动填充的所有好处。
...
Docker Desktop 版本 4.3.0 和 4.3.1 中存在一个错误，该错误可能在登录时将敏感信息（访问令牌或密码）记录在用户的计算机上。
这仅影响用户在 Docker Desktop 4.3.0、4.3.1 上并且在 4.3.0、4.3.1 上时登录过的用户。获取此数据需要能够访问用户的本地文件。

### 错误修复和增强

#### 适用于所有平台

- 如果 `registry.json` 的 `allowedOrgs` 字段包含多个组织，Docker Desktop 会显示错误。如果您为不同的开发者组使用多个组织，则必须为每个组提供单独的 `registry.json` 文件。
- 修复了 Compose 中将容器名称分隔符从 `-` 恢复为 `_` 的回归问题。修复了 [docker/compose-switch](https://github.com/docker/compose-switch/issues/24)。

#### 适用于 Mac

- 修复了仪表板 中容器的内存统计信息。修复了 [docker/for-mac/#4774](https://github.com/docker/for-mac/issues/6076)。
- 向 `settings.json` 添加了一个已弃用的选项：`"deprecatedCgroupv1": true`，它将 Linux 环境切换回 cgroups v1。如果您的软件需要 cgroups v1，您应该更新它以兼容 cgroups v2。尽管 cgroups v1 应该继续工作，但很可能一些未来功能将依赖 cgroups v2。某些 Linux 内核错误也可能只能通过 cgroups v2 修复。
- 修复了一个问题，即在暂停 Docker Desktop 后将计算机置于睡眠模式，导致计算机退出睡眠模式后 Docker Desktop 无法从暂停中恢复。修复了 [for-mac#6058](https://github.com/docker/for-mac/issues/6058)。

#### 适用于 Windows

- 执行 `恢复出厂默认设置` 不再关闭 Docker Desktop。
...
- 通过浏览器登录后单击 **转到桌面**，有时不会将仪表板置于前台。
- 登录后，当仪表板获得焦点时，有时即使单击背景窗口，它也会保持在前台。作为解决方法，您需要在单击另一个应用程序窗口之前单击仪表板。
- 当仪表板打开时，即使它没有焦点或被最小化，它仍然会捕获键盘快捷键（例如，用于重启的 ctrl-r）
...
**Log4j 2 CVE-2021-44228**：我们更新了 `docker scan` CLI 插件。
这个新版本的 `docker scan` 能够检测 [Log4j 2
...
**Log4j 2 CVE-2021-44228**：我们为您更新了 `docker scan` CLI 插件。
...
- [Go 1.17.3](https://github.com/golang/go/releases/tag/go1.17)

### 错误修复和次要更改

#### 适用于所有平台

- 如果主机缺少 Internet 连接，则添加了自我诊断警告。
- 修复了阻止用户使用卷 UI 中的“另存为”选项从卷保存文件的问题。修复了 [docker/for-win#12407](https://github.com/docker/for-win/issues/12407)。
- Docker Desktop 现在使用 cgroupv2。如果您需要在容器中运行 `systemd`，则：
  - 确保您的 `systemd` 版本支持 cgroupv2。[它至少是 `systemd` 247](https://github.com/systemd/systemd/issues/19760#issuecomment-851565075)。考虑将任何 `centos:7` 镜像升级到 `centos:8`。
...
- 修复了当主目录路径包含正则表达式中使用的字符时，Docker Desktop 在启动期间失败的问题。修复了 [docker/for-win#12374](https://github.com/docker/for-win/issues/12374)。

### 已知问题

Docker Desktop 仪表板在基于 Hyper-V 的计算机上错误地将容器内存使用显示为零。
...
- 以下内部 DNS 名称已弃用，将在未来版本中移除：`docker-for-desktop`、`docker-desktop`、`docker.for.mac.host.internal`、`docker.for.mac.localhost`、`docker.for.mac.gateway.internal`。您现在必须使用 `host.docker.internal`、`vm.docker.internal` 和 `gateway.docker.internal`。
- 已移除：自定义 RBAC 规则已从 Docker Desktop 中移除，因为它为所有服务帐户提供了 `cluster-admin` 权限。修复了 [docker/for-mac/#4774](https://github.com/docker/for-mac/issues/4774)。
...
**暂停/恢复**：当您不主动使用 Docker Desktop 会话时，现在可以暂停它，从而节省机器上的 CPU 资源。

- 推出了 [Docker Public Roadmap#226](https://github.com/docker/roadmap/issues/226)

**软件更新**：所有 Docker 订阅用户（包括 Docker Personal 和 Docker Pro）现在都可以选择关闭自动检查更新。所有与更新相关的设置都已移至 **软件更新** 部分。

- 推出了 [Docker Public Roadmap#228](https://github.com/docker/roadmap/issues/228)

**窗口管理**：当您关闭并重新打开 Docker Desktop 时，Docker Desktop 仪表板 窗口的大小和位置会被保留。
...
- [Go 1.17.2](https://github.com/golang/doc/go1.17)
...
- 改进：自我诊断现在还会检查主机 IP 和 `docker networks` 之间的重叠。
- 修复了在 Docker Desktop 仪表板上显示更新可用性的指示器的位置。

#### 适用于 Mac

- 修复了导致 Docker Desktop 在单击致命错误对话框上的 **退出** 时停止响应的问题。
- 修复了一个罕见的启动失败问题，该问题影响将 `docker volume` 绑定挂载到主机上某个目录的用户。如果存在，此修复还将移除用户在相应主机目录上手动添加的 `DENY DELETE` ACL 条目。
- 修复了一个错误，即在升级时会忽略 `Docker.qcow2` 文件，而是使用新的 `Docker.raw`，导致容器和镜像消失。请注意，如果一个系统同时有两个文件（由于之前的错误），则将使用最近修改的文件，以避免最近的容器和镜像再次消失。要强制使用旧的 `Docker.qcow2`，请删除较新的 `Docker.raw` 文件。修复了 [docker/for-mac#5998](https://github.com/docker/for-mac/issues/5998)。
- 修复了一个在关闭期间子进程可能意外失败，从而触发意外致命错误弹出的错误。修复了 [docker/for-mac#5834](https://github.com/docker/for-mac/issues/5834)。

#### 适用于 Windows

- 修复了在致命错误对话框中单击“退出”时 Docker Desktop 有时挂起的问题。
- 修复了一个在已下载但尚未应用更新时频繁显示 **下载更新** 弹出窗口的问题 [docker/for-win#12188](https://github.com/docker/for-win/issues/12188)。
- 修复了安装新更新在应用程序有时间关闭之前就将其终止的问题。
- 修复：Docker Desktop 的安装现在即使在组策略阻止用户启动必备服务（例如 LanmanServer）的情况下也能工作 [docker/for-win#12291](https://github.com/docker/for-win/issues/12291)。
...
> 从 4.1.0 升级时，Docker 菜单不会更改为 **更新并重启**，因此您只需等待下载完成（图标更改），然后选择 **重启** 即可。此错误在 4.1.1 中已修复，对于未来的升级。

- 修复了一个错误，即在升级时会忽略 `Docker.qcow2` 文件，而是使用新的 `Docker.raw`，导致容器和镜像消失。如果一个系统同时有两个文件（由于之前的错误），则将使用最近修改的文件，以避免最近的容器和镜像再次消失。要强制使用旧的 `Docker.qcow2`，请删除较新的 `Docker.raw` 文件。修复了 [docker/for-mac#5998](https://github.com/docker/for-mac/issues/5998)。
- 修复了更新通知覆盖层在 **设置** 按钮和 Docker Desktop 仪表板 中的 **软件更新** 按钮之间有时不同步的问题。
- 修复了安装新下载的 Docker Desktop 更新的菜单项。当更新准备好安装时，**重启** 选项会更改为 **更新并重启**。

#### 适用于 Windows

- 修复了某些发行版（例如 Arch 或 Alpine）的 WSL 2 集成中的回归问题。修复了 [docker/for-win#12229](https://github.com/docker/for-win/issues/12229)
- 修复了更新通知覆盖层在设置按钮和仪表板中的软件更新按钮之间有时不同步的问题。
...
- **软件更新**：设置选项卡现在包含一个新部分，帮助您管理 Docker Desktop 更新。**软件更新** 部分会在有新更新时通知您，并允许您下载更新或查看新版本中包含的内容。
- **Compose V2** 您现在可以在常规设置中指定是否使用 Docker Compose V2。
- **卷管理**：卷管理现在适用于任何订阅（包括 Docker Personal）的用户。推出了 [Docker Public Roadmap#215](https://github.com/docker/roadmap/issues/215)
...
- 基础发行版更改为 debian:bullseye

### 错误修复和次要更改

#### 适用于 Windows

- 修复了与反恶意软件触发相关的问题，自我诊断避免调用 `net.exe` 实用程序。
- 修复了自我诊断中 WSL 2 Linux VM 的文件系统损坏问题。这可能是由 [microsoft/WSL#5895](https://github.com/microsoft/WSL/issues/5895) 引起的。
- 修复了 `SeSecurityPrivilege` 要求问题。参见 [docker/for-win#12037](https://github.com/docker/for-win/issues/12037)。
- 修复了 CLI 上下文与 UI 的同步问题。参见 [docker/for-win#11721](https://github.com/docker/for-win/issues/11721)。
- 向 `settings.json` 添加了密钥 `vpnKitMaxPortIdleTime`，以允许禁用或延长空闲网络连接超时。
- 修复了退出时的崩溃问题。参见 [docker/for-win#12128](https://github.com/docker/for-win/issues/12128)。
- 修复了 CLI 工具在 WSL 2 发行版中不可用的错误。
- 修复了从 Linux 容器切换到 Windows 容器时因 panic.log 上的访问权限而卡住的问题。参见 [for-win#11899](https://github.com/docker/for-win/issues/11899)。
...
- [Compose V2 RC3](https://github.com/docker/compose/releases/tag/v2.0.0-rc.3)
  ...
  - 修复了使用 `compose up --scale` 缩容时的 go panic。
...
- 修复了 Docker Desktop 仪表板 中复制粘贴不可用的错误。

#### 适用于 Windows

- 修复了 Docker Desktop 在使用 Hyper-V 引擎时无法正确启动的错误。参见 [docker/for-win#11963](https://github.com/docker/for-win/issues/11963)
...
Docker 已[宣布](https://www.docker.com/blog/updating-product-subscriptions/)更新和扩展产品订阅，以提高我们的开发者和企业的生产力、协作能力和增加安全性。

更新后的 [Docker 订阅服务协议](https://www.docker.com/legal/docker-subscription-service-agreement)包括对 **Docker Desktop** 条款的更改。

- Docker Desktop **对于小型企业（少于 250 名员工且年收入低于 1000 万美元）、个人使用、教育和非商业开源项目仍然免费**。
- 对于在大型企业中的专业使用，它需要付费订阅（**Pro、Team 或 Business**），每月费用低至 5 美元。
- 这些条款的生效日期为 2021 年 8 月 31 日。对于需要付费订阅才能使用 Docker Desktop 的用户，有一个宽限期至 2022 年 1 月 31 日。
- Docker Pro 和 Docker Team 订阅现在**包括 Docker Desktop 的商业使用**。
- 现有的 Docker Free 订阅已重命名为 **Docker Personal**。
- **没有更改** Docker Engine 或任何其他上游**开源** Docker 或 Moby 项目。

要了解这些更改如何影响您，请阅读 [FAQ](https://www.docker.com/pricing/faq)。
有关更多信息，请参阅 [Docker 订阅概述](../subscription/_index.md)。

### 更新

- [Compose V2 RC2](https://github.com/docker/compose-cli/releases/tag/v2.0.0-rc.2)
  - 修复了项目名称对于 `compose down` 不区分大小写的问题。参见 [docker/compose-cli#2023](https://github.com/docker/compose-cli/issues/2023)
  - 修复了非标准化的项目名称。
  - 修复了部分引用上的端口合并。
...
- 修复了从 git URL 进行构建时 SSH 不可用的错误。修复了 [for-mac#5902](https://github.com/docker/for-mac/issues/5902)

#### 适用于 Windows

- 修复了 CLI 工具在 WSL 2 发行版中不可用的错误。
- 修复了因 `panic.log` 上的访问权限导致从 Linux 容器切换到 Windows 容器时出错的问题。[for-win#11899](https://github.com/docker/for-win/issues/11899)
