...
请使用以下步骤备份和恢复您的镜像及容器数据。这在您需要重置虚拟机磁盘、将 Docker 环境迁移到新电脑，或从失败的 Docker Desktop 更新或安装中恢复时非常有用。
...
1. 使用 [`docker container commit`](/reference/cli/docker/container/commit.md) 将您的容器提交为镜像。
   ...
   提交容器会将文件系统更改和部分容器配置（如标签和环境变量）存储为本地镜像。请注意，环境变量可能包含密码或代理认证等敏感信息，因此在将生成的镜像推送到注册表时需格外小心。
...
2. 如有需要，使用 [`docker run`](/reference/cli/docker/container/run.md) 或 [Docker Compose](/manuals/compose/_index.md) 重新创建您的容器。
...
1. 备份 WSL 发行版。
   ...
   如果您运行任何 WSL Linux 发行版（Ubuntu、Alpine 等），请使用 [Microsoft 的指南](https://learn.microsoft.com/en-us/windows/wsl/faq#how-can-i-back-up-my-wsl-distributions-) 备份它们。

1. 恢复。

   重新安装 Docker Desktop 后，将 `docker_data.vhdx` 恢复到同一位置，并在需要时重新导入您的 WSL 发行版。
...
1. 恢复。
   ...
   重新安装 Docker Desktop 后，将 `Docker.raw` 恢复到同一位置。
