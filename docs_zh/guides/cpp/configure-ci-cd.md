...
description: 了解如何为 C++ 应用程序使用 GitHub Actions 配置 CI/CD。
aliases:
...
完成本指南中从[容器化 C++ 应用程序](containerize.md)开始的所有先前章节。你必须拥有一个 [GitHub](https://github.com/signup) 账户和一个经过验证的 [Docker](https://hub.docker.com/signup) 账户才能完成本节内容。
...
6. 在本地机器上的本地仓库中，运行以下命令将 origin 更改为你刚刚创建的仓库。确保将 `your-username` 更改为你的 GitHub 用户名，将 `your-repository` 更改为你创建的仓库名称。
...
2. 选择 **set up a workflow yourself**（自行设置工作流）。
   ...
   这会将你带到一个页面，用于在仓库中创建一个新的 GitHub Actions 工作流文件，默认位于 `.github/workflows/main.yml`。
...
在本节中，你学习了如何为 C++ 应用程序设置 GitHub Actions 工作流。
