...
你已完成 [构建你的 Rust 镜像](build-images.md) 并成功构建了镜像。

## 概述

容器是一个普通的操作系统进程，但 Docker 会隔离该进程，使其拥有自己的文件系统、自己的网络以及独立于主机的进程树。
...
运行此命令后，你会发现没有返回命令提示符。这是因为你的应用程序是一个服务器，它会循环运行并等待传入请求，直到你停止容器才会将控制权交还给操作系统。

打开一个新终端，然后使用 `curl` 命令向服务器发送请求。
...
如你所见，`curl` 命令失败了。这意味着你无法连接到本地主机的 8000 端口。这是正常的，因为你的容器是隔离运行的，包括网络也是隔离的。停止容器并以端口 8000 发布到本地网络的方式重新启动。

要停止容器，请按 ctrl-c。这将使你返回终端提示符。

要为容器发布端口，你需要在 `docker run` 命令中使用 `--publish` 标志（简写为 `-p`）。`--publish` 命令的格式是 `[主机端口]:[容器端口]`。因此，如果你想将容器内的 8000 端口映射到主机的 3001 端口，你需要向 `--publish` 标志传递 `3001:8000`。

你在容器中运行应用程序时没有指定端口，默认端口是 8000。如果你想让之前的请求能连接到 8000 端口，可以将主机的 3001 端口映射到容器的 8000 端口：
...
成功！你已成功连接到容器内运行在 8000 端口的应用程序。切换回容器运行的终端并停止它。

按 ctrl-c 停止容器。

## 以分离模式运行

到目前为止很好，但你的示例应用程序是一个 Web 服务器，你不需要连接到容器。Docker 可以让你的容器以分离模式（detached mode）或后台模式运行。为此，你可以使用 `--detach` 标志（简写为 `-d`）。Docker 会像之前一样启动你的容器，但这次会从容器中“分离”并返回终端提示符。
...
Docker 在后台启动了你的容器，并在终端打印了容器 ID。

再次确认你的容器正常运行。重新运行 curl 命令。
...
由于你的容器在后台运行，你怎么知道容器是否正在运行，或者你的机器上还有哪些其他容器在运行？要查看机器上运行的容器列表，运行 `docker ps`。这类似于在 Linux 中使用 ps 命令查看进程列表。
...
你可能想知道容器名称是从哪里来的。由于你启动容器时没有提供名称，Docker 生成了一个随机名称。你稍后会修复这个问题，但首先需要停止容器。要停止容器，运行 `docker stop` 命令，该命令会停止容器。你需要传递容器名称，或者使用容器 ID。
...
你现在应该看到列出了几个容器。这些是你启动并停止的容器，但尚未删除。

重启你刚刚停止的容器。找到你刚刚停止的容器名称，并在以下重启命令中替换为你的容器名称。
...
注意，你刚刚重启的容器已以分离模式启动。同时，观察容器状态为“Up X seconds”。当你重启容器时，它会使用最初启动时的相同标志或命令启动。

现在，停止并删除你所有的容器，然后解决随机命名问题。停止你刚刚启动的容器。找到你运行的容器名称，并在以下命令中替换为你的系统上的容器名称。
...
现在你已停止所有容器，接下来删除它们。删除容器后，它不再运行，也不再处于停止状态，但容器内的进程已停止，容器的元数据已被删除。

要删除容器，使用 `docker rm` 命令并传递容器名称。你可以使用单个命令传递多个容器名称。同样，在以下命令中替换为你的系统上的容器名称。
...
再次运行 `docker ps --all` 命令，查看 Docker 已删除所有容器。

现在，是时候解决随机命名问题了。标准做法是为容器命名，原因很简单，这样更容易识别容器中运行的内容以及它关联的应用程序或服务。

要命名容器，只需在 `docker run` 命令中传递 `--name` 标志即可。
...
在本节中，你了解了如何运行容器。你还了解了如何通过启动、停止和重启来管理容器。最后，你学习了如何为容器命名，使其更容易识别。
...
在下一节中，你将学习如何在容器中运行数据库，并将其连接到 Rust 应用程序。