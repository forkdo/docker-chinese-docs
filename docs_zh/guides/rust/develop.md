...
- 你已经完成了 Docker Desktop [学习中心](/manuals/desktop/use-desktop/_index.md) 的演练，以了解 Docker 概念。
...
对于示例应用程序，你将使用 [Awesome Compose](https://github.com/docker/awesome-compose/tree/master/react-rust-postgres) 中 react-rust-postgres 应用程序的后端变体。
...
2. 在克隆的仓库目录中，运行 `docker init` 以创建必要的 Docker 文件。参考以下示例来回答 `docker init` 的提示。
...
3. 在克隆的仓库目录中，在 IDE 或文本编辑器中打开 `Dockerfile` 以进行更新。

   `docker init` 处理了 Dockerfile 中的大部分指令，但你需要根据你的独特应用程序进行更新。除了 `src` 目录外，此应用程序还包括一个用于初始化数据库的 `migrations` 目录。将 `migrations` 目录的绑定挂载添加到 Dockerfile 的构建阶段。以下是更新后的 Dockerfile。
...
4. 在克隆的仓库目录中，运行 `docker build` 以构建镜像。

   ```console
   $ docker build -t rust-backend-image .
   ```

5. 使用以下选项运行 `docker run`，以在数据库所在的同一网络上将镜像作为容器运行。
...
请注意，该文件没有为这两个服务指定网络。当你使用 Compose 时，它会自动创建一个网络并将服务连接到它。有关更多信息，请参阅 [Compose 中的网络](/manuals/compose/how-tos/networking.md)。
...
如果你之前运行了任何其他容器，请现在 [停止](./run-containers.md#stop-start-and-name-containers) 它们。
