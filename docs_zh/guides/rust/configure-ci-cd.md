---
title: 为你的 Rust 应用配置 CI/CD
linkTitle: 配置 CI/CD
weight: 40
keywords: rust, CI/CD, local, development
description: 了解如何为你的应用配置 CI/CD
aliases:
...
完成本指南前面的部分，从 [开发你的 Rust 应用](develop.md) 开始。你需要一个 [GitHub](https://github.com/signup) 账号和一个 [Docker](https://hub.docker.com/signup) 账号才能完成本节内容。

## 概述

在本节中，你将学习如何设置并使用 GitHub Actions 来构建镜像并推送到 Docker Hub。你需要完成以下步骤：
...
6. 在你本地机器上的仓库中，运行以下命令将 origin 更改为刚创建的仓库。确保将 `your-username` 替换为你的 GitHub 用户名，将 `your-repository` 替换为你创建的仓库名称。
...
为你的 GitHub Actions 工作流设置构建、测试和推送镜像到 Docker Hub 的流程。

1. 前往 GitHub 上的仓库，然后选择 **Actions**（操作）标签页。
...
1. 选择 **Commit changes...**（提交更改...）并推送到 `main` 分支。

   推送提交后，工作流将自动启动。

2. 前往 **Actions**（操作）标签页。它将显示工作流。

   选择工作流可以查看所有步骤的详细信息。

3. 当工作流完成后，前往你的 [Docker Hub 仓库列表](https://hub.docker.com/repositories)。

   如果你在列表中看到新仓库，说明 GitHub Actions 已成功将镜像推送到 Docker Hub。

## 总结

在本节中，你学习了如何为你的 Rust 应用设置 GitHub Actions 工作流。
