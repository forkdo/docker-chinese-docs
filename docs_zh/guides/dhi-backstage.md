---
title: 使用 Docker Hardened Images 加固 Backstage 应用程序
description: 使用 Docker Hardened Images 加固 Backstage 开发者门户，涵盖原生模块编译、Socket Firewall 防护以及 distroless 运行时镜像。
summary: 学习如何使用 Docker Hardened Images（DHI）加固 Backstage 开发者门户，通过 better-sqlite3 处理原生模块编译，在依赖安装阶段添加 Socket Firewall 防护，并使用 DHI 定制生成 distroless 运行时镜像。
...
params:
  ...
  prerequisites:
    ...
    - 对多阶段 Dockerfile 和 Node.js 原生模块有基本了解
---
