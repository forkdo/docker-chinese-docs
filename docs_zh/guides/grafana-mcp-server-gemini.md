---
description: 通过 Docker MCP Toolkit 将 Gemini CLI 与 Grafana 集成，实现自然语言可观测性。
keywords: mcp, grafana, docker, gemini, devops
title: 通过 MCP 将 Gemini 连接到 Grafana
summary: |
  了解如何利用 Model Context Protocol (MCP)，直接从终端与 Grafana 仪表板和数据源交互。
params:
  tags: [ai]
