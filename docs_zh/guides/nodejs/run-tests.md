...
description: 了解如何在容器中运行你的 Node.js 测试。
aliases:
...
完成本指南之前的所有部分，从 [将 Node.js 应用容器化](containerize.md) 开始。

## 概述

测试是构建可靠软件的核心部分。无论你编写的是单元测试、集成测试还是端到端测试，跨环境一致地运行测试都至关重要。Docker 通过在本地、CI/CD 和镜像构建期间提供相同的环境，使这一过程变得简单。

## 在本地开发时运行测试

示例应用使用 Vitest 进行测试，它已经包含了 React 组件、自定义 Hook、API 路由、数据库操作和工具函数的测试。
...
### 向 Docker Compose 添加测试服务

要在容器化环境中运行测试，你需要在 `compose.yml` 文件中添加一个专用的测试服务。添加以下服务配置：
...
- **隔离的测试数据库**：使用独立的 `todoapp_test` 数据库进行测试
- **基于 Profile**：使用 `test` profile，因此仅在显式请求时运行
- **健康依赖**：在启动测试之前等待数据库健康
...
- **自定义 Hook** (`src/client/hooks/__tests__/`)：使用适当模拟进行 React Hook 测试
- **服务端路由** (`src/server/__tests__/routes/`)：API 端点测试
- **数据库层** (`src/server/database/__tests__/`)：PostgreSQL 数据库操作测试
- **工具函数** (`src/shared/utils/__tests__/`)：验证和辅助函数测试
- **集成测试** (`src/client/__tests__/`)：完整应用集成测试

## 在构建时运行测试

要在 Docker 构建过程中运行测试，你需要在 Dockerfile 中添加一个专用的测试阶段。如果尚未添加此阶段，请将以下内容添加到你的多阶段 Dockerfile 中：
...
- **灵活执行**：使用 `CMD` 而非 `RUN`，允许在构建期间或作为独立容器运行测试
- **覆盖率支持**：配置为运行带覆盖率报告的测试

### 构建并在镜像构建期间运行测试

要构建在构建过程中运行测试的镜像，你可以创建一个自定义 Dockerfile 或临时修改现有 Dockerfile：
...
在本节中，你学习了如何在使用 Docker Compose 进行本地开发时运行测试，以及如何在构建镜像时运行测试。
...
- [Compose 文件参考](/compose/compose-file/) – 了解 `compose.yaml` 中配置服务的完整语法和选项。
