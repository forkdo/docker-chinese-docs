---
title: 将 Django 应用容器化
linkTitle: Django
description: 学习如何使用 Docker 将 Django 应用容器化。
keywords: python, django, containerize, initialize, gunicorn, compose watch, uv
summary: |
  本指南介绍如何使用 Docker 将 Django 应用容器化。你将使用 uv 搭建项目骨架，
  使用 Docker Hardened Image 创建一个可用于生产的 Dockerfile，然后添加一个
  开发阶段以及 Compose Watch 以实现快速迭代。
params:
  tags: [security]
  time: 25 minutes
---
