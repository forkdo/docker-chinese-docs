---
title: 为你的 Go 应用配置 CI/CD
linkTitle: 配置 CI/CD
weight: 40
keywords: go, CI/CD, 本地, 开发
description: 了解如何为你的 Go 应用配置 CI/CD
aliases:
...
完成本指南前面的章节，从 [构建你的 Go 镜像](build-images.md) 开始。你需要一个 [GitHub](https://github.com/signup) 账号和一个 [Docker](https://hub.docker.com/signup) 账号才能完成本节内容。

## 概述

在本节中，你将学习如何设置并使用 GitHub Actions 来构建并将 Docker 镜像推送到 Docker Hub。你需要完成以下步骤：
...
为你的 GitHub Actions 工作流设置构建、测试并将镜像推送到 Docker Hub。

1. 转到 GitHub 上的仓库，然后选择 **Actions**（操作）标签页。

2. 选择 **set up a workflow yourself**（自己设置工作流）。

   这将带你进入一个页面，在该页面中你可以为仓库创建一个新的 GitHub Actions 工作流文件，默认位置为 `.github/workflows/main.yml`。
...
2. 转到 **Actions**（操作）标签页。它会显示工作流。

   选择工作流可以查看所有步骤的详细信息。

3. 当工作流完成后，转到你的 [Docker Hub 仓库列表](https://hub.docker.com/repositories)。
