---
title: 将 OpenCode 与 Docker Model Runner 配合使用
description: 配置 OpenCode 以使用 Docker Model Runner，从而可以使用本地模型进行编码。
summary: |
  通过兼容 OpenAI 的端点将 OpenCode 连接到 Docker Model Runner，
  选择编码模型，并使用更大的上下文窗口打包 `gpt-oss`。
