...
description: 了解如何为你的 React.js 应用配置基于 GitHub Actions 的 CI/CD。

---

## 前置条件

完成本指南的所有前置章节，从 [容器化 React.js 应用](containerize.md) 开始。
...
在本节中，你将使用 [GitHub Actions](https://docs.github.com/en/actions) 设置一个 **CI/CD 流水线**，自动完成以下任务：

- 在 Docker 容器内构建你的 React.js 应用。
- 在一致的环境中运行测试。
...
1. 在 [Docker Hub](https://hub.docker.com) 创建个人访问令牌（PAT）
   1. 进入 **Docker Hub 账号 → Account Settings → Security**。
   2. 生成一个新的具有 **Read/Write** 权限的访问令牌。
...
2. 在 [Docker Hub](https://hub.docker.com/repositories/) 创建仓库
   ...
   2. 仓库名称使用描述性名称 —— 例如：`reactjs-sample`。
   3. 创建后，复制并保存仓库名称 —— 你将在步骤 4 中用到它。

3. 在 [GitHub](https://github.com/new) 创建新仓库用于你的 React.js 项目

4. 将 Docker Hub 凭据作为 GitHub 仓库密钥添加
...
5. 将本地项目连接到 GitHub
   ...
   从项目根目录运行以下命令，将本地项目 `docker-reactjs-sample` 连接到刚创建的 GitHub 仓库：
   ...
   这表明本地仓库已正确关联，随时可以将源代码推送到 GitHub。
...
完成后，你的代码将出现在 GitHub 上，任何配置的 GitHub Actions 工作流都会自动运行。
...
添加工作流文件后，现在可以触发并观察 CI/CD 流程的实际运行。
...
2. 监控工作流执行
   ...
   2. 点击工作流运行以查看每一步：**build**、**test** 和（如果成功）**push**。
...
在本节中，你为容器化的 React.js 应用使用 GitHub Actions 设置了完整的 CI/CD 流水线。

你完成了以下工作：

- 为项目创建了一个新的 GitHub 仓库。
- 生成了安全的 Docker Hub 访问令牌并将其作为密钥添加到 GitHub。
- 定义了 GitHub Actions 工作流以：
   - 在 Docker 容器内构建应用。
   - 在一致的容器化环境中运行测试。
   - 如果测试通过，将生产就绪镜像推送到 Docker Hub。
- 触发并验证了通过 GitHub Actions 的工作流执行。
- 确认镜像已成功发布到 Docker Hub。

通过此设置，你的 React.js 应用现在已准备好在各种环境中进行自动化测试和部署 —— 提高信心、一致性和团队效率。
...
- [GitHub Actions 简介](/guides/gha.md) – 学习如何使用 GitHub Actions 自动化工作流  
...
接下来，了解如何在 Kubernetes 上本地测试和调试 React.js 工作负载，然后再进行部署。这有助于确保你的应用在类似生产的环境中按预期运行，减少部署时的意外情况。