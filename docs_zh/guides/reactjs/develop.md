...
在本节中，您将学习如何使用 Docker Compose 为您的容器化 React.js 应用程序设置生产和开发环境。此设置允许您通过 Nginx 提供静态生产构建，并使用 Compose Watch 在容器内通过实时重载的开发服务器进行高效开发。
...
使用 Compose Watch 将源文件更改自动同步到您的容器化开发环境中。这提供了一种无缝、高效的开发体验，无需手动重启或重建容器。
...
> - `strictPort: true` 确保如果端口不可用，Vite 会清晰地失败，而不是静默切换。
> 
> 有关完整详情，请参阅 [Vite 服务器配置文档](https://vitejs.dev/config/server-options.html)。
...
您应该会看到更新的文本立即出现，而无需手动重建容器。这证实了文件监视和自动同步按预期工作。
...
以下是您实现的目标：
- 创建了一个 `Dockerfile.dev`，以使用热重载简化本地开发
- 在您的 `compose.yaml` 文件中定义了独立的 `react-dev` 和 `react-prod` 服务
- 启用了使用 Compose Watch 的实时文件同步，以获得更流畅的开发体验
- 通过修改和预览组件，验证了实时更新是否无缝工作

借助此设置，您现在可以完全在容器内构建、运行和迭代您的 React.js 应用程序——在所有环境中高效且一致地进行。

---

## 相关资源

通过这些指南加深您的知识并改进您的容器化开发工作流：
...
在下一节中，您将学习如何在 Docker 容器内为您的 React.js 应用程序运行单元测试。这确保了所有环境的一致性测试，并消除了对本地机器设置的依赖。