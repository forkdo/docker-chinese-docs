---
title: 在 Docker Sandbox 中通过 Docker Model Runner 运行 Claude Code
description: 在隔离的 Docker Sandbox 中运行 Claude Code，并将请求路由到 Docker Model Runner，使智能体使用您主机上的本地模型。
summary: '将 Docker Sandboxes 与 Docker Model Runner 结合，在隔离的微虚拟机中运行 Claude Code，

  该微虚拟机通过兼容 Anthropic 的 API 与您主机上的本地模型通信。
