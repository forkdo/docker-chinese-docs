---
title: 为你的 R 应用配置 CI/CD
linkTitle: 配置 CI/CD
weight: 40
keywords: ci/cd, github actions, R, shiny
description: 了解如何使用 GitHub Actions 为你的 R 应用配置 CI/CD。
aliases:
...
完成本指南之前的所有章节，从 [容器化 R 应用](containerize.md) 开始。你需要拥有 [GitHub](https://github.com/signup) 账号和 [Docker](https://hub.docker.com/signup) 账号才能完成本节内容。

## 概述

在本节中，你将学习如何设置并使用 GitHub Actions 来构建和测试你的 Docker 镜像，并将其推送到 Docker Hub。你需要完成以下步骤：
...
3. 创建一个新的 **Repository variable**，命名为 `DOCKER_USERNAME`，值为你的 Docker ID。

4. 为 Docker Hub 创建一个新的 [Personal Access Token (PAT)](/manuals/security/access-tokens.md#create-an-access-token)。你可以将其命名为 `docker-tutorial`。确保访问权限包含读取和写入。

5. 将 PAT 作为 **Repository secret** 添加到你的 GitHub 仓库中，名称为 `DOCKERHUB_TOKEN`。

6. 在你本地机器上的仓库中，运行以下命令将 origin 更改为刚才创建的仓库。请确保将 `your-username` 替换为你的 GitHub 用户名，将 `your-repository` 替换为你创建的仓库名称。
...
为你的 GitHub Actions 工作流设置构建、测试并将镜像推送到 Docker Hub 的流程。

1. 前往 GitHub 上的仓库，然后选择 **Actions** 标签页。

2. 选择 **set up a workflow yourself**。

   这将带你进入一个在仓库中创建新的 GitHub Actions 工作流文件的页面，默认路径为 `.github/workflows/main.yml`。
...
在本节中，你学习了如何为你的 R 应用设置 GitHub Actions 工作流。
