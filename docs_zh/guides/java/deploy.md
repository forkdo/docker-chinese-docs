...
- 完成本指南之前的所有章节，从 [容器化你的应用](containerize.md) 开始。
- 在 Docker Desktop 中[启用 Kubernetes](/manuals/desktop/use-desktop/kubernetes.md#enable-kubernetes)。

## 概述

在本节中，你将学习如何使用 Docker Desktop 将你的应用部署到开发机器上的完整 Kubernetes 环境中。这让你可以在本地测试和调试你的工作负载，然后再部署到生产环境。

## 创建 Kubernetes YAML 文件

在你的 `spring-petclinic` 目录中，创建一个名为 `docker-java-kubernetes.yaml` 的文件。在 IDE 或文本编辑器中打开该文件，并添加以下内容。将 `DOCKER_USERNAME/REPO_NAME` 替换为你的 Docker 用户名和在 [为你的 Java 应用配置 CI/CD](configure-ci-cd.md) 中创建的仓库名称。
...
- 一个 Deployment，描述了一组可扩展的相同 Pod。在本例中，你将得到一个 Pod 副本。该 Pod 在 `template` 下描述，其中只有一个容器。该容器是从 GitHub Actions 在 [为你的 Java 应用配置 CI/CD](configure-ci-cd.md) 中构建的镜像创建的。
- 一个 NodePort 服务，它将把主机上的 30001 端口的流量路由到 Pod 内部的 8080 端口，允许你从网络访问你的应用。
...
3. 在终端中，使用 curl 访问该服务。注意，在此示例中没有部署数据库。
