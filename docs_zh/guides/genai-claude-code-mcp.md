---
description: 了解如何结合使用 Claude Code 与 Docker MCP Toolkit，通过 Docker Hub MCP 服务器从自然语言生成可用于生产的 Docker Compose 文件。
keywords: mcp, model context protocol, docker, docker desktop, claude code, docker hub, compose, automation
title: 使用 Claude Code 与 Docker MCP Toolkit 生成 Docker Compose 文件
summary: |
  本指南展示如何把 Claude Code 接入 Docker MCP Toolkit，使其能够搜索 Docker Hub 镜像并从自然语言生成完整的 Docker Compose 技术栈。
  你将启用 Docker Hub MCP 服务器、连接 Claude Code、验证 MCP 访问，并通过对话式提示创建一个 Node.js + PostgreSQL 技术栈。
aliases:
