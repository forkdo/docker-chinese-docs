---
title: 为你的 Bun 应用配置 CI/CD
linkTitle: 配置 CI/CD
weight: 40
keywords: ci/cd, github actions, bun, shiny
description: 了解如何使用 GitHub Actions 为你的 Bun 应用配置 CI/CD。
aliases:
...
完成本指南中之前的所有部分，从 [容器化 Bun 应用](containerize.md) 开始。你需要一个 [GitHub](https://github.com/signup) 账号和一个 [Docker](https://hub.docker.com/signup) 账号才能完成本节内容。
...
5. 将 PAT 作为 **Repository secret** 添加到你的 GitHub 仓库中，命名为 `DOCKERHUB_TOKEN`。
...
1. 进入 GitHub 上的仓库，然后选择 **Actions** 标签页。
...
2. 进入 **Actions** 标签页。它将显示工作流。

   选择工作流将显示所有步骤的详细信息。

3. 当工作流完成后，前往你的 [Docker Hub 仓库页面](https://hub.docker.com/repositories)。

   如果你在列表中看到新仓库，说明 GitHub Actions 成功将镜像推送到 Docker Hub。

## 总结

在本节中，你学习了如何为你的 Bun 应用设置 GitHub Actions 工作流。
