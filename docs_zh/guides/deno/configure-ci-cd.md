...
完成本指南的所有先前部分，从[容器化 Deno 应用程序](containerize.md)开始。您必须拥有 [GitHub](https://github.com/signup) 账户和 [Docker](https://hub.docker.com/signup) 账户才能完成本部分。

## 概述

在本部分中，您将学习如何设置和使用 GitHub Actions 来构建和测试您的 Docker 镜像，并将其推送到 Docker Hub。您将完成以下步骤：
...
4. 为 Docker Hub 创建一个新的[个人访问令牌 (PAT)](/manuals/security/access-tokens.md#create-an-access-token)。您可以将此令牌命名为 `docker-tutorial`。确保访问权限包括 Read（读取）和 Write（写入）。

5. 将 PAT 作为 **Repository secret**（仓库机密）添加到您的 GitHub 仓库中，名称为
   `DOCKERHUB_TOKEN`。

6. 在您机器上的本地仓库中，运行以下命令将 origin 更改为您刚刚创建的仓库。确保将 `your-username` 更改为您 GitHub 的用户名，将 `your-repository` 更改为您创建的仓库名称。
...
1. 转到您在 GitHub 上的仓库，然后选择 **Actions**（操作）选项卡。

2. 选择 **set up a workflow yourself**（自己设置工作流）。

   这将带您进入一个页面，用于在您的仓库中创建一个新的 GitHub Actions 工作流文件，默认路径为 `.github/workflows/main.yml`。

3. 在编辑器窗口中，复制并粘贴以下 YAML 配置，并提交更改。
...
在本部分中，您学习了如何为您的 Deno 应用程序设置 GitHub Actions 工作流。
