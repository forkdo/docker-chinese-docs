...
每个服务还可以包含一个 `build` 部分，该部分定义如何为服务创建 Docker 镜像。Compose 支持使用此服务定义构建 Docker 镜像。如果不使用，`build` 部分将被忽略，Compose 文件仍然被视为有效。构建支持是 Compose 规范的一个可选方面，在 [Compose 构建规范](build.md) 文档中有详细描述。

每个服务都定义了运行其容器的运行时约束和要求。`deploy` 部分将这些约束分组，并允许平台调整部署策略，以最佳地匹配容器的需求与可用资源。部署支持是 Compose 规范的一个可选方面，在 [Compose 部署规范](deploy.md) 文档中有详细描述。如果未实现，`deploy` 部分将被忽略，Compose 文件仍然被视为有效。
...
当 `attach` 定义并设置为 `false` 时，Compose 不会收集服务日志，直到您显式请求它。
...
`blkio_config` 定义一组配置选项，用于设置服务的块 I/O 限制。
...
`cap_add` 指定要添加的额外容器 [capabilities](https://man7.org/linux/man-pages/man7/capabilities.7.html) 字符串。
...
`cap_drop` 指定要删除的容器 [capabilities](https://man7.org/linux/man-pages/man7/capabilities.7.html) 字符串。
...
> [!NOTE]
>
> 与 Dockerfile 中的 `CMD` 指令不同，`command` 字段不会自动在镜像定义的 [`SHELL`](/reference/dockerfile.md#shell-form) 指令的上下文中运行。如果您的 `command` 依赖于特定于 shell 的功能，例如环境变量扩展，则需要显式地在 shell 中运行它。例如：
>
> ```yaml
...
`configs` 让服务调整其行为，而无需重建 Docker 镜像。服务只能在通过 `configs` 属性显式授权时访问配置。支持两种不同的语法变体。

如果 `config` 在平台上不存在或未在 Compose 文件的 [`configs` 顶级元素](configs.md) 中定义，Compose 会报告错误。

为 configs 定义了两种语法：短语法和长语法。

您可以授予服务访问多个 configs 的权限，并且可以混合使用长语法和短语法。

#### 短语法

短语法变体仅指定配置名称。这授予容器访问配置的权限，并将其作为文件挂载到服务容器的文件系统中。容器内的挂载点位置在 Linux 容器中默认为 `/<config_name>`，在 Windows 容器中默认为 `C:\<config-name>`。
...
- `mode`: 挂载在服务任务容器中的文件的 [权限](https://wintelguy.com/permissions-calc.pl)，以八进制表示法表示。默认值为全局可读 (`0444`)。可写位必须被忽略。可执行位可以设置。
...
如果您的服务使用 Windows 容器，您可以对 `credential_spec` 使用 `file:` 和 `registry:` 协议。Compose 还支持用于自定义用例的其他协议。
...
为服务配置 gMSA 凭据规范时，您只需使用 `config` 指定凭据规范，如下例所示：
...
- `restart`: 当设置为 `true` 时，Compose 在更新依赖服务后重新启动此服务。这适用于由 Compose 操作控制的显式重新启动，不包括容器运行时在容器死亡后的自动重新启动。在 Docker Compose 版本 [2.17.0](https://github.com/docker/compose/releases/tag/v2.17.0) 中引入。
...
语法是 `<portnum>/[<proto>]` 或 `<startport-endport>/[<proto>]` 表示端口范围。
如果未显式设置，则使用 `tcp` 协议。
...
`blkio_config.device_read_bps`, `blkio_config.device_read_iops`, `blkio_config.device_write_bps`, `blkio_config.device_write_iops`, `devices` 和 `volumes` 下的项目也视为映射，其中键是容器内的目标路径。
...
此功能有用的一个例子是，当多个容器（以不同用户身份运行）需要读取或写入共享卷上的同一文件时。该文件可以由所有容器共享的组拥有，并在 `group_add` 中指定。
...
在创建的容器内运行 `id` 必须显示该用户属于 `mail` 组，如果不声明 `group_add`，情况就不会如此。
...
`image` 指定要从中启动容器的镜像。`image` 必须遵循 Open Container Specification 的 [可寻址镜像格式](https://github.com/opencontainers/org/blob/master/docs/docs/introduction/digests.md)，即 `[<registry>/][<project>/]<image>[:<tag>|@<digest>]`。
...
链接不是启用服务通信所必需的。当未设置特定的网络配置时，任何服务都可以在 `default` 网络上通过该服务的名称访问任何其他服务。
如果服务指定了它们连接的网络，`links` 不会覆盖网络配置。未连接到共享网络的服务无法相互通信。Compose 不会就配置不匹配发出警告。
...
`driver` 名称指定服务容器的日志记录驱动程序。默认值和可用值是特定于平台的。驱动程序特定的选项可以用 `options` 作为键值对来设置。
...
> [!NOTE]
> 容器运行时可能会拒绝此值，例如 Docker Engine >= v25.0。在这种情况下，您应该改用 [networks.mac_address](#mac_address)。
...
`mem_swappiness` 定义为百分比，值在 0 到 100 之间，用于主机内核换出容器使用的匿名内存页。
...
`memswap_limit` 定义容器允许换出到磁盘的内存量。这是一个修饰符属性，仅在同时设置了 [`memory`](deploy.md#memory) 时才有意义。使用交换空间允许容器在用尽所有可用内存时将多余的内存需求写入磁盘。经常将内存交换到磁盘的应用程序会受到性能影响。

- 如果 `memswap_limit` 设置为正整数，则必须同时设置 `memory` 和 `memswap_limit`。`memswap_limit` 表示可以使用的内存和交换空间的总量，`memory` 控制非交换内存的使用量。因此，如果 `memory`="300m" 且 `memswap_limit`="1g"，容器可以使用 300m 内存和 700m (1g - 300m) 交换空间。
- 如果 `memswap_limit` 设置为 0，则该设置被忽略，值被视为未设置。
- 如果 `memswap_limit` 设置为与 `memory` 相同的值，并且 `memory` 设置为正整数，则容器无法访问交换空间。
- 如果 `memswap_limit` 未设置，并且 `memory` 已设置，则容器可以使用与 `memory` 设置一样多的交换空间（如果主机容器配置了交换内存）。例如，如果 `memory`="300m" 且 `memswap_limit` 未设置，容器总共可以使用 600m 内存和交换空间。
...
`models` 定义服务在运行时应使用的 AI 模型。每个引用的模型必须在 [`models` 顶级元素](models.md) 下定义。
...
当服务链接到模型时，Docker Compose 会注入环境变量以将连接详细信息和模型标识符传递给容器。这允许应用程序在运行时动态定位和与模型通信，而无需硬编码值。
...
如果省略任何一个，Compose 会根据模型键自动生成环境变量名称，使用以下规则：
...
services:
  some-service:
    networks:
      ...
      - other-network
```
有关 `networks` 顶级元素的更多信息，请参阅 [网络](networks.md)。

#### 隐式默认网络

如果 `networks` 在 Compose 文件中为空或不存在，Compose 会考虑一个隐式定义，表示服务连接到 `default` 网络：
...
`aliases` 声明服务在网络上替代的主机名。同一网络上的其他容器可以使用服务名称或别名来连接到服务的容器之一。

由于 `aliases` 是网络范围的，同一服务在不同网络上可以有不同的别名。

> [!NOTE]
> 网络范围的别名可以由多个容器共享，甚至可以由多个服务共享。
...
[顶级网络部分](networks.md) 中的相应网络配置必须具有 `ipam` 属性，其中包含覆盖每个静态地址的子网配置。
...
`link_local_ips` 指定链路本地 IP 地址列表。链路本地 IP 是属于众所周知的子网的特殊 IP，纯粹由操作员管理，通常取决于部署它们的架构。
...
`driver_opts` 指定一个键值对选项列表以传递给驱动程序。这些选项是驱动程序相关的。有关更多信息，请查阅驱动程序的文档。
...
如果容器运行时在服务级别接受 `mac_address` 属性，则将其应用于具有最高 `priority` 的网络。在其他情况下，使用属性 `networks.mac_address`。
...
`oom_score_adj` 调整容器在内存不足时被平台杀死的偏好程度。值必须在 -1000,1000 范围内。
...
长形式语法允许您配置短形式无法表达的附加字段。

- `target`: 容器端口。
- `published`: 公开暴露的端口。定义为字符串，可以使用 `start-end` 语法设置为范围。这意味着实际端口在设置的范围内分配一个剩余的可用端口。
...
- `user`: 运行命令的用户。如果未设置，命令将以与主服务命令相同的用户身份运行。
- `privileged`: 允许 `post_start` 命令以特权访问运行。
- `working_dir`: 运行命令的工作目录。如果未设置，则在与主服务命令相同的工作目录中运行。
- `environment`: 专门为 `post_start` 命令设置环境变量。虽然该命令继承为服务主命令定义的环境变量，但此部分允许您添加新变量或覆盖现有变量。
...
依赖服务 `app` 接收到以服务名称为前缀的附加环境变量，因此它可以访问该资源。
...
`options` 特定于所选的提供程序，并且不受 compose 规范验证。

### `pull_policy`

`pull_policy` 定义 Compose 开始拉取镜像时所做的决策。可能的值是：

- `always`: Compose 始终从注册表拉取镜像。
- `never`: Compose 不从注册表拉取镜像，依赖于平台缓存的镜像。如果没有缓存的镜像，则报告失败。
- `missing`: 仅当镜像在平台缓存中不可用时，Compose 才会拉取镜像。如果您不使用 [Compose 构建规范](build.md)，这是默认选项。`if_not_present` 被视为此值的别名，以实现向后兼容性。即使使用 `missing` 拉取策略，`latest` 标签也始终被拉取。
...
例如，`runtime` 可以是 [OCI 运行时规范实现](https://github.com/opencontainers/runtime-spec/blob/master/implementations.md) 的名称，例如 "runc"。
...
如果密钥在平台上不存在或未在 Compose 文件的 [`secrets` 顶级部分](secrets.md) 中定义，Compose 会报告错误。

在顶级 `secrets` 中定义密钥并不意味着授予任何服务对其的访问权限。此类授权必须在服务规范中作为 [secrets](secrets.md) 服务元素显式进行。

#### 短语法

短语法变体仅指定密钥名称。这授予容器访问密钥的权限，并将其作为只读挂载到容器内的 `/run/secrets/<secret_name>`。源名称和目标挂载点都设置为密钥名称。
...
长语法在如何在服务容器中创建密钥方面提供了更精细的控制。

- `source`: 平台上存在的密钥名称。
- `target`: 要挂载到服务任务容器中 `/run/secrets/` 下的文件名，或者如果需要备用位置，则为文件的绝对路径。如果未指定，则默认为 `source`。
- `uid` 和 `gid`: 挂载的文件在服务任务容器中 `/run/secrets/` 下拥有的数字 uid 或 gid。
- `mode`: 挂载在服务任务容器中 `/run/secrets/` 下的文件的 [权限](https://wintelguy.com/permissions-calc.pl)，以八进制表示法表示。
  默认值为全局可读权限（模式 `0444`）。
...
`shm_size` 配置服务容器允许的共享内存大小（Linux 上的 `/dev/shm` 分区）。
它被指定为 [字节值](extension.md#specifying-byte-values)。
...
短语法使用一个冒号分隔值的字符串来指定卷挂载 (`VOLUME:CONTAINER_PATH`)，或访问模式 (`VOLUME:CONTAINER_PATH:ACCESS_MODE`)。

- `VOLUME`: 可以是托管容器的平台上的主机路径（绑定挂载），也可以是卷名称。
- `CONTAINER_PATH`: 卷在容器中挂载的路径。
- `ACCESS_MODE`: 逗号分隔 `,` 的选项列表：
  - `rw`: 读写访问。如果未指定任何访问模式，则为默认值。
  - `ro`: 只读访问。
...
> [!NOTE]
>
> 对于绑定挂载，如果源路径在主机上不存在，短语法会在源路径上创建一个目录。这是为了与 `docker-compose` 旧版保持向后兼容性。
...
- `source`: 挂载的源，绑定挂载的主机路径，镜像挂载的 Docker 镜像引用，或在 [顶级 `volumes` 键](volumes.md) 中定义的卷名称。不适用于 tmpfs 挂载。
- `target`: 卷在容器中挂载的路径。
- `read_only`: 将卷设置为只读的标志。
- `bind`: 用于配置附加绑定选项：
  - `propagation`: 绑定使用的传播模式。
  - `create_host_path`: 如果源路径上没有任何内容，则在源路径上创建目录。默认为 `true`。
  - `selinux`: SELinux 重新标记选项 `z`（共享）或 `Z`（私有）。
- `volume`: 配置附加卷选项：
  - `nocopy`: 在创建卷时禁止从容器复制数据的标志。
  - `subpath`: 卷内部要挂载的路径，而不是卷根目录。
- `tmpfs`: 配置附加 tmpfs 选项：
...
- `image`: 配置附加镜像选项：
  - `subpath`: 要挂载的源镜像内部的路径，而不是镜像根目录。在 [Docker Compose 版本 2.35.0](https://github.com/docker/compose/releases/tag/v2.35.0) 中可用。
...
您还可以使用 `container:` 前缀挂载来自不由 Compose 管理的容器的卷。
